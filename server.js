...
try { db.exec("ALTER TABLE users ADD COLUMN rejection_note TEXT"); } catch {}
try { db.exec("ALTER TABLE users ADD COLUMN rejected_at TEXT"); } catch {}
...
app.post("/api/application-drafts/save",(req,res)=>{
 ...
 db.prepare(`INSERT INTO application_drafts(email,name,first_name,last_name,whatsapp_phone,mpesa_phone,newsletter_opt_in,location,google_maps_location,intro_note,dog_name,dog_breed,dog_gender,dog_dob,household_dogs,household_adults,children_0_8,children_9_13,children_14_plus,household_changes,household_note,last_step)
   VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
   ON CONFLICT(email) DO UPDATE SET name=excluded.name,first_name=excluded.first_name,last_name=excluded.last_name,whatsapp_phone=excluded.whatsapp_phone,mpesa_phone=COALESCE(NULLIF(excluded.mpesa_phone,''),application_drafts.mpesa_phone),newsletter_opt_in=excluded.newsletter_opt_in,location=COALESCE(NULLIF(excluded.location,''),application_drafts.location),google_maps_location=COALESCE(NULLIF(excluded.google_maps_location,''),application_drafts.google_maps_location),intro_note=COALESCE(NULLIF(excluded.intro_note,''),application_drafts.intro_note),dog_name=COALESCE(NULLIF(excluded.dog_name,''),application_drafts.dog_name),dog_breed=COALESCE(NULLIF(excluded.dog_breed,''),application_drafts.dog_breed),dog_gender=COALESCE(NULLIF(excluded.dog_gender,''),application_drafts.dog_gender),dog_dob=COALESCE(NULLIF(excluded.dog_dob,''),application_drafts.dog_dob),household_dogs=COALESCE(excluded.household_dogs,application_drafts.household_dogs),household_adults=COALESCE(NULLIF(excluded.household_adults,''),application_drafts.household_adults),children_0_8=COALESCE(excluded.children_0_8,application_drafts.children_0_8),children_9_13=COALESCE(excluded.children_9_13,application_drafts.children_9_13),children_14_plus=COALESCE(excluded.children_14_plus,application_drafts.children_14_plus),household_changes=COALESCE(NULLIF(excluded.household_changes,''),application_drafts.household_changes),household_note=COALESCE(NULLIF(excluded.household_note,''),application_drafts.household_note),last_step=MAX(application_drafts.last_step,excluded.last_step),updated_at=CURRENT_TIMESTAMP`)
   .run(cleanEmail,name,firstName,lastName,whatsRaw,formattedMpesa||"",b.newsletterOptIn?1:0,String(b.location||"").trim(),String(b.googleMapsLocation||"").trim(),String(b.introNote||"").trim(),String(b.dogName||"").trim(),String(b.dogBreed||"").trim(),String(b.dogGender||"").trim(),String(b.dogDob||"").trim(),b.householdDogs===""||b.householdDogs==null?null:Number(b.householdDogs),b.householdAdults===""||b.householdAdults==null?null:String(b.householdAdults).trim(),b.children0to8===""||b.children0to8==null?null:Number(b.children0to8),b.children9to13===""||b.children9to13==null?null:Number(b.children9to13),b.children14plus===""||b.children14plus==null?null:Number(b.children14plus),String(b.householdChanges||"").trim(),String(b.householdNote||"").trim(),step);
...
app.post("/api/auth/register", (req,res)=>{
 const {name,email,firstName,lastName,whatsappPhone,mpesaPhone,password,newsletterOptIn,applicationSignup,location,googleMapsLocation,introNote,householdDogs,householdAdults,children0to8,children9to13,children14plus,householdChanges,householdNote}=req.body;
 const cleanFirst=String(firstName||"").trim(),cleanLast=String(lastName||"").trim(),displayName=String(name||[cleanFirst,cleanLast].filter(Boolean).join(" ")).trim();
 const whatsRaw=String(whatsappPhone||"").trim(),mpesaRaw=String(mpesaPhone||"").trim(),cleanEmail=String(email||"").trim().toLowerCase();
 if(applicationSignup&&(!cleanFirst||!cleanLast))return res.status(400).json({error:"Please complete your first name and last name."});
 if(!displayName||!cleanEmail||!whatsRaw||!password)return res.status(400).json({error:"Please complete name, WhatsApp number, email and password."});
 if(!validWhatsappPhone(whatsRaw))return res.status(400).json({error:"WhatsApp number may contain numbers, spaces and a leading + only."});
 ...
 try{
  ...
  id=db.prepare(`INSERT INTO users(role,name,first_name,last_name,email,phone,whatsapp_phone,mpesa_phone,newsletter_opt_in,last_login_at,password_hash,client_status,location,google_maps_location,client_intro_note,household_dogs,household_adults,children_0_8,children_9_13,children_14_plus,household_changes,household_note)
   VALUES('client',?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP,?,?,?,?,?,?,?,?,?,?,?,?)`)
   .run(displayName,cleanFirst||null,cleanLast||null,cleanEmail,whats,whats,mpesa,newsletterOptIn?1:0,hash,applicationSignup?"applicant":"current",String(location||"").trim(),String(googleMapsLocation||"").trim(),String(introNote||"").trim(),householdDogs===""||householdDogs==null?null:Number(householdDogs),householdAdults===""||householdAdults==null?null:String(householdAdults).trim(),children0to8===""||children0to8==null?null:Number(children0to8),children9to13===""||children9to13==null?null:Number(children9to13),children14plus===""||children14plus==null?null:Number(children14plus),String(householdChanges||"").trim(),String(householdNote||"").trim()).lastInsertRowid;
 }catch(e){
 ...
 try{logActivity({userId:Number(id),actorUserId:Number(id),actorRole:"client",action:"account_created",details:`New client account created — ${displayName}.`})}catch(e){console.error(e)}
 try{notifyTrainer({userId:Number(id),kind:"new_account",message:`New client account created — ${displayName}.`})}catch(e){console.error(e)}
...
function bookingTravelBuffer(row){return row&&row.location_type==="home"?Math.max(30,Number(row.travel_minutes||0)):0}
function bookingOccupiedStart(row){return row&&row.location_type==="home"?addMinutes(row.start_at,-bookingTravelBuffer(row)):row.start_at}
function candidateOccupiedStart(locationType,startAt,travelMinutes){return locationType==="home"?addMinutes(startAt,-Math.max(30,Number(travelMinutes||0))):startAt}
function scheduleAmyConflict(startAt,endAt){const date=String(startAt||"").slice(0,10);return activeScheduleBlocks().find(r=>{if(r.target!=="amy"||!scheduleBlockAppliesOnDate(r,date))return false;const x=scheduleBlockTimes(r,date);return overlaps(startAt,endAt,x.start,x.end)})||null}
...
app.get("/api/availability",async(req,res)=>{
  ...
  for(let cursor=ws;cursor<we;cursor+=30*60000){
      ...
      const existing=db.prepare("SELECT start_at,buffer_end_at,location_type,travel_minutes FROM bookings WHERE status NOT IN ('cancelled','expired') AND payment_status IN ('pending','paid','demo_paid','credit_paid')").all();
      ...
      const occupiedStart=candidateOccupiedStart(locationType,start,travel);
      const badBooking=existing.some(x=>overlaps(occupiedStart,bufferEnd,bookingOccupiedStart(x),x.buffer_end_at));
...
app.get("/api/trainer/availability",requireTrainer,async(req,res)=>{
  expireTimedHolds();const {date,locationType,address,service,extraMinutes,overrideLocation}=req.query;if(!date||!/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/.test(date))return res.status(400).json({error:"Please provide a complete date."});if(!privateBookingDateAllowed(`${date}T00:00:00`))return res.status(400).json({error:"Private appointments can be booked from tomorrow onwards."});if(!["consultation","standard","extra"].includes(service))return res.status(400).json({error:"Please choose a training type."});if(!["arena","home"].includes(locationType))return res.status(400).json({error:"Please choose a location."});const override=String(overrideLocation)==="1";const total=({consultation:90,standard:60,extra:90}[service]||60)+Math.max(0,Number(extraMinutes||0)),working=workingWindowForDate(date);if(!working.enabled||!working.start_time||!working.end_time)return res.json([]);const ws=wallClockMs(isoDateTime(date,working.start_time)),we=wallClockMs(isoDateTime(date,working.end_time)),existing=db.prepare("SELECT start_at,buffer_end_at,location_type,travel_minutes FROM bookings WHERE status NOT IN ('cancelled','expired') AND payment_status IN ('pending','paid','demo_paid','credit_paid')").all(),blocked=db.prepare("SELECT start_at,end_at FROM availability_blocks").all(),classSessions=db.prepare("SELECT session_date,start_time,end_time FROM class_sessions").all(),out=[];let travel=0;if(locationType==="home"&&address)travel=await routeTravelMinutes(address,"Nairobi, Kenya");const buffer=locationType==="arena"?0:Math.max(30,travel);for(let cursor=ws;cursor<we;cursor+=30*60000){const start=wallClockIso(cursor),end=addMinutes(start,total),bufferEnd=addMinutes(end,buffer),occupiedStart=candidateOccupiedStart(locationType,start,travel);if(String(end).slice(0,10)!==date||wallClockMs(end)>we)continue;if(recurringBlockConflict(start,end)||scheduleAmyConflict(occupiedStart,bufferEnd))continue;if(!override&&(serviceUnavailable(locationType,start,bufferEnd)||scheduleBlockConflict(locationType,start,bufferEnd)))continue;if(existing.some(x=>overlaps(occupiedStart,bufferEnd,bookingOccupiedStart(x),x.buffer_end_at)))continue;if(activeRescheduleHoldConflict(occupiedStart,bufferEnd))continue;if(blocked.some(x=>overlaps(occupiedStart,bufferEnd,x.start_at,x.end_at)))continue;if(classSessions.some(x=>overlaps(occupiedStart,bufferEnd,isoDateTime(x.session_date,x.start_time),isoDateTime(x.session_date,x.end_time))))continue;out.push({start,end,travelMinutes:travel,bufferMinutes:buffer,overrideUsed:override})}res.json(out)
});
...
app.post("/api/bookings/private",requireAuth,requireApprovedClient,async(req,res)=>{
  ...
  const occupiedStart=candidateOccupiedStart(locationType,startAt,travel);
  const existing=db.prepare("SELECT start_at,buffer_end_at,location_type,travel_minutes FROM bookings WHERE status!='cancelled' AND payment_status IN ('pending','paid','demo_paid','credit_paid')").all();
  if(existing.some(x=>overlaps(occupiedStart,bufferEnd,bookingOccupiedStart(x),x.buffer_end_at))||activeRescheduleHoldConflict(occupiedStart,bufferEnd)) return res.status(409).json({error:"That time is no longer available, including the travel time needed around home visits."});
...
app.post("/api/my/classes/:ref/resume-payment",requireAuth,(req,res)=>{
  ...
  if(e.manual_payment_status==='submitted')return res.status(409).json({error:"Your M-Pesa reference has already been submitted to Amy for verification."});
...
app.get("/api/my/profile",requireAuth,(req,res)=>{
  const user=db.prepare("SELECT id,role,name,first_name,last_name,email,phone,whatsapp_phone,mpesa_phone,newsletter_opt_in,kra_pin,last_login_at,COALESCE(client_status,'current') client_status,location,google_maps_location,rejection_note,rejected_at FROM users WHERE id=?").get(req.user.id);
  const applicationDeposit=db.prepare("SELECT amount,manual_payment_code,manual_payment_status,submitted_at,verified_at,refund_amount,refund_method,refund_reference,refunded_at,decision_status FROM application_deposits WHERE user_id=?").get(req.user.id)||null;
...
app.put("/api/my/profile",requireAuth,(req,res)=>{
  ...
  const name=String(req.body.name||[firstName,lastName].filter(Boolean).join(' ')||before.name).trim(), whats=String(req.body.whatsappPhone??before.whatsapp_phone??before.phone??"").trim(), rawMpesa=String(req.body.mpesaPhone??before.mpesa_phone??whats).trim(), normalizedMpesa=normalizeKenyanMpesaPhone(rawMpesa), kra=String(req.body.kraPin??before.kra_pin??"").trim()||null,location=String(req.body.location??before.location??'').trim(),googleMapsLocation=String(req.body.googleMapsLocation??before.google_maps_location??'').trim();
  if(!validWhatsappPhone(whats))return res.status(400).json({error:'Please enter a valid WhatsApp number using numbers, spaces and an optional leading +.'});
  if(rawMpesa && !normalizedMpesa)return res.status(400).json({error:'Please enter a valid Kenyan M-Pesa number beginning 07, 01, +254 7 or +254 1.'});
  const mpesa=normalizedMpesa||whats;
...
app.post("/api/trainer/clients/:id/reject-application",requireTrainer,(req,res)=>{
 ...
 const tx=db.transaction(()=>{
   db.prepare("UPDATE users SET client_status='rejected',status_override=NULL,rejection_note=?,rejected_at=CURRENT_TIMESTAMP WHERE id=?").run(rejectionNote||null,u.id);
...
app.get("/api/trainer/summary",requireTrainer,(req,res)=>{
  ...
  res.json({
    ...
    vaccinationAttention:db.prepare(`SELECT p.id pet_id,p.name pet_name,p.breed,p.vaccination_status,p.vaccination_verified_at,u.id user_id,u.name client_name,(SELECT COUNT(*) FROM pet_files f WHERE f.pet_id=p.id AND f.kind='vaccination') vaccination_count FROM pets p JOIN users u ON u.id=p.user_id WHERE COALESCE(p.archived,0)=0 AND COALESCE(u.client_status,'current')='current' AND p.vaccination_status IN ('pending','rejected','not_provided') ORDER BY p.name`).all(),
...
app.put("/api/trainer/classes/:id",requireTrainer,(req,res)=>{
  ...
  const existingSessions=db.prepare("SELECT * FROM class_sessions WHERE class_id=? ORDER BY session_date,start_time").all(current.id);
  const today=nairobiDateKey(0),pastSessions=existingSessions.filter(x=>x.session_date<today);
  ...
  if(pastSessions.length){const pastDates=pastSessions.map(x=>x.session_date);if(pastDates.some(d=>!dates.includes(d)))return res.status(409).json({error:"Completed class dates are part of the course history and cannot be moved or removed."});const originalStart=pastSessions[0].start_time,originalEnd=pastSessions[0].end_time;if(startTime!==originalStart||endTime!==originalEnd)return res.status(409).json({error:"The course time is locked once a session has taken place. Future dates can still be changed."});}
...
app.post("/api/trainer/clients/:userId/provisional-booking",requireTrainer,async(req,res)=>{
  ...
  const {petId,service,locationType,address,startAt,requestedDate,repeatType,sessionCount,customDates,packageName,packagePrice,overrideLocation}=req.body;
  ...
  const travel=locationType==='home'?await routeTravelMinutes(address,'Nairobi, Kenya'):0,buffer=locationType==='arena'?0:Math.max(30,travel),proposed=[],override=!!overrideLocation;
  for(const st of dates){const en=addMinutes(st,duration),be=addMinutes(en,buffer),occupiedStart=candidateOccupiedStart(locationType,st,travel);if(!withinWorkingHours(st,en)||recurringBlockConflict(st,en)||scheduleAmyConflict(occupiedStart,be)||(!override&&(serviceUnavailable(locationType,st,be)||scheduleBlockConflict(locationType,st,be)))||activeRescheduleHoldConflict(occupiedStart,be))return res.status(409).json({error:`${st.slice(0,10)} ${st.slice(11,16)} is unavailable.`});const existing=db.prepare("SELECT * FROM bookings WHERE status NOT IN ('cancelled','expired') AND payment_status IN ('pending','paid','demo_paid','credit_paid')").all();if(existing.some(x=>overlaps(occupiedStart,be,bookingOccupiedStart(x),x.buffer_end_at)))return res.status(409).json({error:`${st.slice(0,10)} ${st.slice(11,16)} conflicts with another booking or required travel time.`});const classes=db.prepare("SELECT * FROM class_sessions").all();if(classes.some(x=>overlaps(occupiedStart,be,isoDateTime(x.session_date,x.start_time),isoDateTime(x.session_date,x.end_time))))return res.status(409).json({error:`${st.slice(0,10)} ${st.slice(11,16)} overlaps a class or required travel time.`});proposed.push({st,en,be})}
...
app.get("/api/trainer/bookings/:id/reschedule-availability",requireTrainer,async(req,res)=>{
  ...
  const existing=db.prepare("SELECT id,start_at,buffer_end_at,location_type,travel_minutes FROM bookings WHERE id!=? AND status NOT IN ('cancelled','expired') AND payment_status IN ('pending','paid','demo_paid','credit_paid')").all(b.id);
  const blocked=db.prepare("SELECT start_at,end_at FROM availability_blocks").all();
  const classSessions=db.prepare("SELECT session_date,start_time,end_time FROM class_sessions").all();
  const slots=[];
  ...
  for(let cursor=ws;cursor<we;cursor+=30*60000){
    ...
    const occupiedStart=candidateOccupiedStart(b.location_type,start,travel);
    if(existing.some(x=>overlaps(occupiedStart,bufferEnd,bookingOccupiedStart(x),x.buffer_end_at)))continue;
...
function rescheduleBookingInternal(req,res,actor){
  ...
  const occupiedStart=candidateOccupiedStart(b.location_type,startAt,b.travel_minutes||0);
  if(others.some(o=>overlaps(occupiedStart,bufferEnd,bookingOccupiedStart(o),o.buffer_end_at)))return res.status(409).json({error:"That new time conflicts with another booking or the travel time around a home visit."});
...
app.get("/api/trainer/clients",requireTrainer,(req,res)=>{
  expireTimedHolds();
  const users=db.prepare("SELECT id,name,first_name,last_name,email,phone,whatsapp_phone,mpesa_phone,newsletter_opt_in,kra_pin,location,google_maps_location,client_intro_note,rejection_note,rejected_at,created_at,last_login_at,status_override,COALESCE(client_status,'current') client_status FROM users WHERE role='client' ORDER BY name").all();
...
app.post("/api/trainer/clients/:id/messages",requireTrainer,(req,res)=>{const user=db.prepare("SELECT id,name FROM users WHERE id=? AND role='client'").get(req.params.id);if(!user)return res.status(404).json({error:'Client not found.'});const body=String(req.body.body||'').trim();if(!body)return res.status(400).json({error:'Please enter a message.'});const id=db.prepare("INSERT INTO client_messages(user_id,reply_to_type,reply_to_id,body,sender_role,seen_by_trainer) VALUES(?, 'trainer', NULL, ?, 'trainer', 1)").run(user.id,body).lastInsertRowid;logActivity({userId:user.id,actorUserId:req.user.id,actorRole:'trainer',action:'message_sent',details:'Amy sent a message to the client.'});res.json({ok:true,id});});
