...
function clientFinancialSummary(b){
 ...
 for(const x of cls){if(["refund_partial","refunded"].includes(x.payment_status)&&Number(x.refund_amount||0)>0&&recent(x.refund_recorded_at))events.push({at:x.refund_recorded_at,text:`${x.payment_status==="refund_partial"?"Partial refund":"Refund"} ${money(x.refund_amount)} for cancelled class ${esc(x.title||"")}`});if(["credit_partial","credited"].includes(x.payment_status)&&recent(x.refund_recorded_at))events.push({at:x.refund_recorded_at,text:`${x.payment_status==="credit_partial"?"Partial credit":"Credit"} for cancelled class ${esc(x.title||"")}`});if(x.payment_status==="no_refund"&&x.refund_client_note&&recent(x.refund_recorded_at))events.push({at:x.refund_recorded_at,text:`No refund or credit for cancelled class ${esc(x.title||"")} — ${esc(x.refund_client_note)}`})}
 events.sort((x,y)=>String(y.at||"").localeCompare(String(x.at||"")));const latest=events.slice(0,3);
 return latest.length?`<div class="notice account-update"><b>Recent account updates</b>${latest.map(x=>`<div class="account-update-line">${x.text}</div>`).join("")}</div>`:"";
}
...
function trainerAgenda(){
 ...
 return `<div class="trainer-agenda"><div class="agenda-date"><div><div class="eyebrow">Daily agenda</div><h3>${displayDate(key,{weekday:"long",day:"numeric",month:"long"})}</h3></div><span class="agenda-working-hours">${w.start_time}–${w.end_time}</span></div><div class="agenda-timeline">${merged.map(r=>{const body=`<div class="agenda-time">${r.st}–${r.en}</div><div class="agenda-copy"><span class="agenda-title">${esc(r.title)}</span>${r.detail?`<p>${esc(r.detail)}</p>`:""}</div>`;const cls=`agenda-item ${r.kind}${r.kind==="booking"&&r.locationType?` booking-${r.locationType}`:""}${r.lunch?" lunch-item":""}`;return r.kind==="booking"?`<button class="${cls} clickable-agenda" onclick="openTrainerBooking(${r.id})">${body}</button>`:r.kind==="class"?`<button class="${cls} clickable-agenda class-agenda-link" onclick="openClassFromAgenda(${Number(r.id)||0})">${body}</button>`:`<div class="${cls}">${body}</div>`}).join("")}</div></div>`;
}
...
function filterQuickResources(value){
 ...
 document.querySelectorAll(".quick-resource-row").forEach(el=>{el.style.display=!q||String(el.dataset.search||"").includes(q)?"":"none"});
}
...
function attentionAdminView(){
 ...
 const none=!(t.vaccinationAttention||[]).length&&!(t.cancellationAttention||[]).length&&!(t.classRefundAttention||[]).length&&!(t.rescheduleAttention||[]).length&&!(t.manualPaymentAttention||[]).length&&!(t.classManualPaymentAttention||[]).length&&!newClients.length;
