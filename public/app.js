...
function portalView(){
 ...
 return `<section class="screen client-dashboard"><div class="eyebrow">Client Portal</div><h2>Welcome, ${esc(state.user.first_name||state.user.name.split(" ")[0])}</h2>
 ${String(state.user?.client_status||"current")==="rejected"?`<div class="notice bad rejection-client-banner"><b>Dear ${esc(state.user.first_name||state.user.name.split(" ")[0])},</b><br>Amy has reviewed your application and decided not to accept you as a client.${state.user.rejection_note?`<br><br><b>Reason:</b> ${esc(state.user.rejection_note)}`:""}<br><br>Please respect this decision.${Number(state.profile?.applicationDeposit?.refund_amount||0)>0?` Your KES 700 will be refunded shortly.`:""}</div>`:applicant?`<div class="notice application-status-banner"><b>New Client Application</b><br>Your account is open for profile and dog information. Booking is locked until Amy approves your application.</div>`:""}
...
function applicationDepositCopy(){
 return `<div class="application-deposit-copy"><h3>You are starting a New Client Application</h3><p>Amy asks that you pay <b>KES 1,000</b> to start a client account as this initial phase takes a lot of her time. Of this <b>KES 1,000</b>, only <b>KES 300</b> will be non-refundable (administration fee).</p><p>The remaining <b>KES 700</b> is held pending Amy’s decision:</p><ul><li>if your application is approved, <b>KES 700</b> is added to your account as a credit which can be spent on her services</li><li>if your application is not approved, <b>KES 700</b> is refunded</li></ul><p>Paying the application deposit does not guarantee approval or a booking.</p><p class="application-vaccination-reminder">Please have your dog's valid vaccination record ready (as a photo or document, or in a place you can photograph it) as it is useful if you can upload it during the sign up process.</p><p class="application-duration"><b>The sign up takes roughly 5 minutes to complete.</b></p></div>`;
}
...
function attentionAdminView(){
 ...
 const none=!(t.vaccinationAttention||[]).length&&!(t.cancellationAttention||[]).length&&!(t.classRefundAttention||[]).length&&!(t.rescheduleAttention||[]).length&&!(t.manualPaymentAttention||[]).length&&!(t.classManualPaymentAttention||[]).length&&!(t.applicationDepositAttention||[]).length;
 ...
 ${(t.manualPaymentAttention||[]).map(x=>`<div class="card"><b>Manual M-Pesa to verify · ${esc(x.client_name)}</b><p>${esc(x.pet_name||'')} · ${esc(x.booking_ref)} · M-Pesa Ref ${esc(x.manual_payment_code)} · Reported ${money(x.manual_payment_amount)}${Number(x.credit_applied||0)>0?` · Credit used ${money(x.credit_applied)}`:''} · Total fee ${money(x.price)}</p><div class="actions"><button class="primary compact-button" onclick="verifyManualPayment(${x.id},true)">Verify payment</button><button class="danger compact-button" onclick="verifyManualPayment(${x.id},false)">Reject</button></div></div>`).join('')}
 ${(t.classManualPaymentAttention||[]).map(x=>`<div class="card"><b>Class manual M-Pesa to verify · ${esc(x.client_name)}</b><p>${esc(x.pet_name||'')} · ${esc(x.title||'Class')} · ${esc(x.booking_ref)} · M-Pesa Ref ${esc(x.manual_payment_code)} · Reported ${money(x.manual_payment_amount)}${Number(x.credit_applied||0)>0?` · Credit used ${money(x.credit_applied)}`:''} · Total fee ${money(x.price)}</p><div class="actions"><button class="primary compact-button" onclick="verifyClassManualPayment(${x.id},true)">Verify payment</button><button class="danger compact-button" onclick="verifyClassManualPayment(${x.id},false)">Reject</button></div></div>`).join('')}
