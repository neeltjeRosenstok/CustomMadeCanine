...
function clientFinancialSummary(b){
 const priv=Array.isArray(b?.privateBookings)?b.privateBookings:[],cls=Array.isArray(b?.classBookings)?b.classBookings:[];
 const events=[];
 for(const x of priv){
  if(["refund_partial","refunded"].includes(x.payment_status)&&Number(x.refund_amount||0)>0)events.push({at:x.refund_recorded_at||x.start_at,text:`${x.payment_status==="refund_partial"?"Partial refund":"Refund"} ${money(x.refund_amount)} · ${esc(x.booking_ref)}`});
  if(["credit_partial","credited"].includes(x.payment_status)&&Number(x.credit_amount||0)>0)events.push({at:x.refund_recorded_at||x.start_at,text:`${x.payment_status==="credit_partial"?"Partial credit":"Credit"} ${money(x.credit_amount)} added to your account · ${esc(x.booking_ref)}`});
 }
 for(const x of cls){
  if(["refund_partial","refunded"].includes(x.payment_status)&&Number(x.refund_amount||0)>0)events.push({at:x.refund_recorded_at||x.created_at,text:`${x.payment_status==="refund_partial"?"Partial refund":"Refund"} ${money(x.refund_amount)} · ${esc(x.booking_ref)}`});
  if(["credit_partial","credited"].includes(x.payment_status)&&Number(x.credit_amount||0)>0)events.push({at:x.refund_recorded_at||x.created_at,text:`${x.payment_status==="credit_partial"?"Partial credit":"Credit"} ${money(x.credit_amount)} added to your account · ${esc(x.booking_ref)}`});
 }
 events.sort((x,y)=>String(y.at||"").localeCompare(String(x.at||"")));
 return events[0]?`<div class="notice account-update"><b>Latest account update</b><br>${events[0].text}</div>`:"";
}
...
async function submitManualPaymentFromPayment(){
 ...
 const code=(document.getElementById('manualMpesaCode')?.value||'').replace(/\s+/g,'').toUpperCase();
 ...
 if(!Number.isFinite(amount)||amount<=0)return appAlert('There is no payment amount due.');
 try{
   await api(`/api/my/bookings/${c.id}/manual-payment`,{method:'POST',body:JSON.stringify({confirmationCode:code,amount})});
   await appAlert('Thank you. Your M-Pesa reference has been submitted to Amy for verification. Your booking is being held while she checks the payment. Please WhatsApp Amy to make sure she sees this.');
   state.portalTab='bookings';await portal();
 }catch(e){appAlert(e.message)}
}
