...
function classesAdminView(){
 ...
 return `<section class="screen admin-screen"><button class="back-dashboard" onclick="dashboardBack()">← Back to Dashboard</button><div class="admin-head"><div><div class="eyebrow">Amy's workspace</div><h2>Classes</h2><p>Courses, age ranges, dates, places and dogs enrolled.</p></div><button class="secondary" onclick="addClass()">＋ New course</button></div><div class="class-admin-layout"><div class="admin-list class-admin-list">${rows.map(x=>{const n=new Set((x.enrolments||[]).filter(e=>e.enrolment_status==="active").map(e=>e.pet_id||`enrol-${e.id}`)).size;return `<button class="admin-list-row class-course-row ${Number(active?.id)===Number(x.id)?"selected":""}" onclick="state.selectedClassAdmin=${x.id};render()"><span class="class-list-main"><span class="class-list-title">${esc(x.title)}</span><small>${displayDate(x.start_date,{day:"numeric",month:"short",year:"numeric"})}</small></span><span class="class-list-enrolled"><span>${n}/${x.capacity}</span><small>enrolled</small></span></button>`}).join('')||'<p>No classes created yet.</p>'}</div><div class="admin-detail">${classAdminDetail(active)}</div></div></section>`;
}
function classAdminDetail(c){
 ...
 const byPet=new Map();
 for(const e of rows){
   const key=e.pet_id!=null?`pet-${e.pet_id}`:`enrol-${e.id}`;
   if(!byPet.has(key))byPet.set(key,[]);
   byPet.get(key).push(e);
 }
 const cards=[...byPet.values()].map(group=>{
   group.sort((x,y)=>String(y.activity_at||y.created_at||"").localeCompare(String(x.activity_at||x.created_at||"")));
   const active=group.find(e=>e.enrolment_status==="active");
   const current=active||group[0];
   const prior=group.filter(e=>e.id!==current.id);
   const events=[...(current.events||[]),...prior.flatMap(e=>e.events||[])]
     .sort((x,y)=>String(y.created_at||"").localeCompare(String(x.created_at||"")));
   return {...current,_events:events,_active:!!active,_sortAt:String(current.activity_at||current.created_at||"")};
 }).sort((x,y)=>String(y._sortAt).localeCompare(String(x._sortAt)));

 const activeCards=cards.filter(x=>x._active),inactiveCards=cards.filter(x=>!x._active);
 const cardHtml=(e)=>{
   const months=ageMonthsOnClient(e.date_of_birth,c.start_date);
   const stateLabel=e.enrolment_status==="active"?"Active":e.enrolment_status==="cancelled_by_client"?"Cancelled by client":e.enrolment_status==="cancelled_by_trainer"?"Cancelled by Amy":e.enrolment_status==="rejected"?"Cancelled":"Inactive";
   const financial=e.payment_status==="refund_pending"?"Refund / credit decision required":e.payment_status==="refunded"?`Refunded ${money(e.refund_amount||0)}`:e.payment_status==="refund_partial"?`Partial refund ${money(e.refund_amount||0)}`:e.payment_status==="credited"?"Full client credit":e.payment_status==="credit_partial"?"Partial client credit":e.payment_status==="no_refund"?"No refund or credit":e.payment_status==="demo_paid"?"Paid":e.payment_status==="paid"?"Paid":e.payment_status==="credit_paid"?"Paid with credit":e.payment_status==="pending"?"Awaiting payment":esc(e.payment_status||"");
   const history=(e._events||[]).slice(0,12);
   return `<article class="class-dog-card ${e._active?"active-dog":"inactive-dog"} ${e.payment_status==="refund_pending"?"needs-financial":""}" data-class-enrolment-id="${e.id}">
     <div class="class-dog-card-head"><div><h4>${esc(e.pet_name||"Dog")}</h4><p>${esc(e.client_name||"")}</p></div><span class="class-dog-state ${e._active?"active":"inactive"}">${esc(stateLabel)}</span></div>
     <div class="class-dog-info"><p>${esc(e.breed||"Breed not recorded")}${e.date_of_birth?` · DOB ${displayDate(e.date_of_birth,{day:"numeric",month:"short",year:"numeric"})}${months!=null?` · ${months} months at start`:""}`:" · DOB not recorded"}</p><p>${esc(e.email||"")}${e.phone?` · ${esc(e.phone)}`:""}</p><p>Vaccination: ${e.vaccination_status==="verified"?"Verified ✓":esc(e.vaccination_status||"Not provided")}</p></div>
     <div class="class-dog-status-line"><span>${esc(financial)}</span>${e.rejected_reason?`<span>${esc(e.rejected_reason)}</span>`:""}</div>
     ${e._active?`<div class="class-dog-actions"><button class="secondary compact-button" onclick="rejectClassDog(${e.id})">Cancel enrolment</button></div>`:""}
     ${e.payment_status==="refund_pending"?`<div class="class-refund-actions"><button class="secondary compact-button" onclick="decideClassRefund(${e.id},'full',${Number(c.price||0)})">Full refund</button><button class="secondary compact-button" onclick="decideClassRefund(${e.id},'partial',${Number(c.price||0)})">Partial refund</button><button class="secondary compact-button" onclick="decideClassRefund(${e.id},'credit_full',${Number(c.price||0)})">Full credit</button><button class="secondary compact-button" onclick="decideClassRefund(${e.id},'credit_partial',${Number(c.price||0)})">Partial credit</button><button class="secondary compact-button" onclick="decideClassRefund(${e.id},'none',${Number(c.price||0)})">No refund or credit</button></div>`:""}
     ${history.length?`<details class="class-dog-history"><summary>History · ${history.length}</summary><div>${history.map(h=>`<p><time>${esc(String(h.created_at||"").replace("T"," ").slice(0,16))}</time> · ${esc(String(h.event_type||"").replaceAll("_"," "))}${h.amount!=null?` · ${money(h.amount)}`:""}${h.reference?` · ${esc(h.reference)}`:""}${h.note?` · ${esc(h.note)}`:""}</p>`).join("")}</div></details>`:""}
   </article>`;
 };
 ...
 <section class="class-card-group active-group"><div class="class-card-group-head"><h4>Active dogs</h4><span>${activeCards.length}</span></div>${activeCards.length?`<div class="class-dog-card-list">${activeCards.map(cardHtml).join("")}</div>`:"<p class=\"small\">No active enrolments.</p>"}</section>
 <section class="class-card-group inactive-group"><div class="class-card-group-head"><h4>Inactive / history</h4><span>${inactiveCards.length}</span></div>${inactiveCards.length?`<div class="class-dog-card-list">${inactiveCards.map(cardHtml).join("")}</div>`:"<p class=\"small\">No inactive enrolments.</p>"}</section>
 </div>`;
}
...
function attentionAdminView(){
 const t=state.trainer||{};
 const none=!(t.vaccinationAttention||[]).length&&!(t.cancellationAttention||[]).length&&!(t.classRefundAttention||[]).length&&!(t.rescheduleAttention||[]).length&&!(t.manualPaymentAttention||[]).length&&!(t.notifications||[]).length;
