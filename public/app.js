...
function reportsAdminView(){const r=state.reports;const rows=r?.rows||[];const cols=rows.length?Object.keys(rows[0]):[];return `<section class="screen admin-screen reports-screen"><button class="back-dashboard" onclick="dashboardBack()">← Back to Dashboard</button><div class="admin-head"><div><div class="eyebrow">Amy's workspace</div><h2>Reports</h2><p>Print, save as PDF, or export CSV.</p></div></div><div class="panel report-controls"><div class="form-grid"><label>Report<select id="reportType" onchange="state.reportType=this.value"><option value="daily">Daily Sheet</option><option value="appointments">Appointments</option><option value="payments">Payments</option><option value="clients">Clients</option><option value="vaccinations">Vaccinations</option><option value="newsletter">The Canine Grapevine</option></select></label><label>From / date<input id="reportFrom" type="date" value="${esc(state.reportFrom||nairobiDateKeyClient(0))}"></label><label>To<input id="reportTo" type="date" value="${esc(state.reportTo||nairobiDateKeyClient(0))}"></label></div><div class="actions"><button class="primary" onclick="loadReport()">Generate report</button>${r?`<button class="secondary" onclick="window.print()">Print / Save PDF</button><button class="secondary" onclick="exportReportCsv()">Export CSV</button>`:''}</div></div>${r?`<div class="report-paper"><div class="report-heading"><img src="/brand-logo.png" alt=""><div><h2>The Custom Made Canine</h2><h3>${esc(r.title)}</h3><p>${esc(r.from||'')}${r.to&&r.to!==r.from?` to ${esc(r.to)}`:''}</p></div></div><div class="report-table-wrap"><table class="report-table"><thead><tr>${cols.map(k=>`<th>${esc(k.replaceAll('_',' '))}</th>`).join('')}</tr></thead><tbody>${rows.map(row=>`<tr>${cols.map(k=>`<td>${esc(row[k]??'')}</td>`).join('')}</tr>`).join('')}</tbody></table></div></div>`:''}</section>`}
