...
async function declineProvisional(id){
 if(!confirm("Do not accept this proposed booking/package? The held time(s) will be released."))return;
...
async function addPrivateCalendarByRef(ref){
 ...
 const location=x.location_type==="home"?(x.address||"Client home, Nairobi"):("Amy's Arena, Ridgeways, Nairobi, Kenya");
...
async function addClassCalendarByRef(ref){
 ...
 const events=c.sessions.map((s,i)=>({uid:`${c.id}-${i}-${ref}@custommadecanine`,start:`${s.session_date}T${s.start_time}:00+03:00`,end:`${s.session_date}T${s.end_time}:00+03:00`,title:`${c.title} — Class ${i+1}`,location:"Amy's Arena, Ridgeways, Nairobi, Kenya",description:`Five-class course\nBooking reference: ${ref}`}));
...
function petCard(p,archived=false){
 ...
 return `<article class="dog-card-v2176 ${archived?"pet-archived":""}">
   ...
   <div class="dog-file-grid">
    ...
    <section class="dog-file-panel"><h4>Vaccination record</h4><p class="small">Valid Parvo, DHLP and Rabies proof should show the vaccine sticker/batch label, date, validity/expiry, vet stamp/signature and KVB number.</p><p>${esc(vaccination)}</p><div class="dog-file-actions">${p.vaccination_count?`<button class="secondary compact-button" onclick="viewVaccinations(${p.id})">View</button>`:""}<label class="file-button">${p.vaccination_count?"Replace":"Upload"}<input type="file" accept="image/jpeg,image/png,image/webp" multiple onchange="uploadVaccinations(${p.id},this)"></label>${p.vaccination_count?`<button class="secondary compact-button" onclick="removeVaccinations(${p.id})">Remove</button>`:""}</div></section>
   </div>
...
function trainerAgenda(){
 ...
 for(let i=0;i<times.length-1;i++){
  ...
  else if(cls){kind="class";title=cls.title;detail=`Class · ${cls.enrolled}/${cls.capacity} places · ${cls.location_type==="alternate"?(cls.location_name||"Alternate venue"):"Amy's Arena in Ridgeways"}`}
...
async function saveTrainerDogNotes(id){
 ...
 try{await api(`/api/trainer/pets/${id}/private-notes`,{method:"PUT",body:JSON.stringify({trainerNotes})});if(state.clientRecord?.user?.id)state.clientRecord=await api(`/api/trainer/client/${state.clientRecord.user.id}`);state.editingTrainerNotePetId=null;render()}catch(e){alert(e.message)}
}
...
async function saveTrainerNote(userId,petId,bookingId){
 ...
 alert('Training plan / note saved and shared with the client.');
}
...
function bookingPolicyText(){return `<div class="policy-copy"><p><b>Booking, cancellation & rescheduling policy</b></p><p>Please give Amy as much notice as possible if plans change. Cancellations made less than 36 hours before an appointment may be charged in full. Client rescheduling requests are subject to Amy's approval and are limited to three approved requests per booking/package. Trainer-created provisional appointments are held for 24 hours while awaiting confirmation and payment.</p><p>By confirming, you agree to The Custom Made Canine's booking terms and cancellation policy.</p></div>`}
...
function privateBookingSentence(){if(!state.selectedService||!state.selectedLocation||!state.selectedDate||!state.selectedSlot)return "";const svc=privateServiceLabel(state.selectedService),place=state.selectedLocation==='arena'?arenaClientLabel():'your home',date=displayDate(state.selectedDate,{weekday:'long',day:'numeric',month:'long',year:'numeric'}),time=String(state.selectedSlot.start).slice(11,16);return `Thank you for starting your booking. You have selected One-on-One training — ${svc} at ${place} on ${date} at ${time}. In the next step, you'll be asked to create an account for you and your dog if you haven't already.`}
...
async function confirmPrivate(){if(!state.user)return auth('Your time is selected. Please sign in or create an account to continue to payment.');if(!state.selectedPet)return alert('Please select which dog this training is for.');if(!(state.privateTermsAccepted||document.getElementById('bookingTermsAccepted')?.checked))return alert('Please read and agree to the Booking Terms & Cancellation Policy.');const address=document.getElementById('address')?.value||state.address||'';state.address=address;if(state.selectedLocation==='home'&&!address.trim())return alert('Please add the home address before continuing.');try{const d=await api('/api/bookings/private',{method:'POST',body:JSON.stringify({service:state.selectedService,locationType:state.selectedLocation,address,startAt:state.selectedSlot.start,petId:state.selectedPet,termsAccepted:true})});state.confirm={...d,type:'private',service:state.selectedService,locationType:state.selectedLocation,address,startAt:state.selectedSlot.start,endAt:state.selectedSlot.end,petId:state.selectedPet};go('payment')}catch(e){if(e.paymentPending){state.confirm={...e,type:'private',service:state.selectedService,locationType:state.selectedLocation,address,startAt:state.selectedSlot.start,endAt:state.selectedSlot.end,petId:state.selectedPet,mpesaDemo:false,mpesaMessage:e.message};go('payment')}else alert(e.message)}}
function classDetails(){const c=state.selectedClass;const dog=state.profile?.pets?.find(p=>Number(p.id)===Number(state.selectedPet));const final=state.user&&dog?`<div class="booking-final-summary"><p>You have selected <b>${esc(dog.name)}</b> for <b>${esc(c.title)}</b>, starting on <b>${displayDate(c.start_date,{weekday:'long',day:'numeric',month:'long',year:'numeric'})}</b> at <b>${esc(c.start_time)}</b>. Please click <b>CONFIRM & PAY</b> to reserve the complete course.</p><label class="terms-check"><input id="classTermsAccepted" type="checkbox" onchange="state.classTermsAccepted=this.checked;render()" ${state.classTermsAccepted?'checked':''}> I have read and agree to <button type="button" class="text-button inline-policy" onclick="state.bookingTermsOpen=true;render()">The Custom Made Canine's Booking Terms & Cancellation Policy</button>.</label></div>`:'';return `<div><div class="eyebrow">${c.sessions.length}-session course</div><h2>${esc(c.title)}</h2><p>${esc(c.description)}</p><div class="notice">All ${c.sessions.length} sessions are included. Individual class dates cannot be changed and new clients cannot join after the course starts.</div><div class="class-age-rule">Age range at course start: ${esc(classAgeLabel(c))}</div><div class="list">${c.sessions.map((s,i)=>`<div class="kpi"><span>Class ${i+1}</span><span>${new Date(s.session_date+'T12:00:00').toLocaleDateString('en-KE',{weekday:'short',day:'numeric',month:'short'})} · ${s.start_time}–${s.end_time}</span></div>`).join('')}</div>${state.user?dogPicker('class'):`<div class="notice">Thank you for starting this booking. You have selected ${esc(c.title)}, starting on ${displayDate(c.start_date,{day:'numeric',month:'long',year:'numeric'})} at ${esc(c.start_time)}. In the next step you will create or sign into your account and select your dog.</div>`}${final}<div class="actions"><button class="primary" ${state.user&&(!state.selectedPet||!state.classTermsAccepted)?'disabled':''} onclick="joinClass()">${state.user?'CONFIRM & PAY':'Continue to account'} · ${money(c.price)}</button></div>${bookingTermsModal()}</div>`}
function selectPetForBooking(id){state.selectedPet=Number(id);render()}
async function joinClass(){if(!state.user)return auth('Your place is selected. Please sign in or create an account to continue to payment.');if(!state.selectedPet)return alert('Please select which dog this class is for.');const box=document.getElementById('classTermsAccepted');state.classTermsAccepted=!!box?.checked;if(!state.classTermsAccepted)return alert('Please read and agree to the Booking Terms & Cancellation Policy.');const pet=(state.profile?.pets||[]).find(p=>p.id===state.selectedPet);if(!pet||pet.archived)return alert('Please select an active dog.');const eligibility=dogClassEligibility(pet,state.selectedClass);if(!eligibility.ok)return alert(eligibility.code==='dob'?`Please add ${pet.name}'s date of birth before joining this course.`:`${pet.name} does not meet this course's age range.`);try{const d=await api(`/api/classes/${state.selectedClass.id}/enrol`,{method:'POST',body:JSON.stringify({petId:state.selectedPet,termsAccepted:true})});state.confirm={...d,type:'class',classId:state.selectedClass.id,petId:state.selectedPet};go('payment')}catch(e){alert(e.message)}}

function authView(){const hasBooking=!!(state.selectedService||state.selectedClass);return `<section class="screen"><button class="back" onclick="go(state.selectedService?'private':'classes')">← Back to booking</button><div class="center"><div class="panel auth-panel"><div class="eyebrow">One quick step</div><h2>${hasBooking?'Your booking is saved':'Client Portal'}</h2>${hasBooking?`<div class="notice good saved-notice"><b>✓ Your booking choices are saved.</b><br>Sign in or create your client account to continue.</div>`:''}<p class="auth-message">${esc(state.authMessage||'To continue, choose the option that applies to you.')}</p>${state.authMode==='choice'?`<div class="auth-choice-grid"><button class="auth-choice" onclick="state.authMode='login';render()"><span class="auth-choice-icon">↪</span><strong>I already have an account</strong><span>Sign in to continue.</span></button><button class="auth-choice" onclick="state.authMode='register';render()"><span class="auth-choice-icon">＋</span><strong>I'm a new client</strong><span>Create an account to continue.</span></button></div>`:`<button class="back auth-back" onclick="state.authMode='choice';render()">← Choose another option</button>${state.authMode==='register'?`<label>Name<input id="authName" autocomplete="name"></label><label>WhatsApp number<input id="authWhatsapp" placeholder="07… or 2547…" autocomplete="tel"></label><label>M-Pesa number <span class="small">(if different)</span><input id="authMpesa" placeholder="Leave blank if same as WhatsApp" autocomplete="tel"></label>`:''}<label>Email<input id="authEmail" type="email" autocomplete="email" value="${esc(localStorage.getItem('cmc_last_email')||'')}"></label><label>Password<input id="authPassword" type="password" autocomplete="current-password"></label>${state.authMode==='register'?`<label class="newsletter-optin"><input id="authNewsletter" type="checkbox"> Please tick here if you would like to receive my newsletter, <em>The Canine Grapevine</em>, which I try to send out at least every 2 months, to keep you informed about upcoming classes, events and other news.</label>`:''}<div class="actions"><button class="primary" onclick="submitAuth()">${state.authMode==='register'?'Create my account':'Sign in and continue'}</button></div>${state.authMode==='login'?`<button class="text-button" onclick="showForgotPassword()">Forgot your password?</button>`:''}`}</div></div></section>`}
async function submitAuth(){try{const body={email:document.getElementById('authEmail').value.trim(),password:document.getElementById('authPassword').value};if((state.authMode||'login')==='register'){body.name=document.getElementById('authName').value;body.whatsappPhone=document.getElementById('authWhatsapp').value;body.mpesaPhone=document.getElementById('authMpesa').value;body.newsletterOptIn=!!document.getElementById('authNewsletter').checked}localStorage.setItem('cmc_last_email',body.email);const d=await api(`/api/auth/${(state.authMode||'login')==='register'?'register':'login'}`,{method:'POST',body:JSON.stringify(body)});state.user=d.user;if(state.user.role==='trainer'){state.authReturnToBooking=false;state.menu=false;state.trainer=await api('/api/trainer/summary');await loadTrainerCalendar(new Date());await loadTrainerMonth(new Date());state.serviceAvailability=await api('/api/trainer/service-availability');go('trainer')}else{state.profile=await api('/api/my/profile');if(state.authReturnToBooking){state.authReturnToBooking=false;if(state.selectedClass)go('classes');else if(state.selectedService)go('private');else await portal()}else{clearBookingDraft();await portal()}}}catch(e){alert(e.message)}}
...
function paymentView(){const c=state.confirm||{};const expiry=c.holdExpiresAt?`<p class="small"><b>Held until:</b> ${fmt(c.holdExpiresAt)}</p>`:'';return `<section class="screen"><div class="center"><div class="panel payment-panel" style="width:min(680px,100%)"><div class="eyebrow">Almost there</div><h1 class="mpesa-title">M-Pesa</h1><p class="lead">${esc(c.mpesaMessage||'Approve the payment on your phone.')}</p><div class="notice"><b>Reference:</b> ${esc(c.bookingRef)}<br><b>Amount:</b> ${money(c.amount)}${expiry}</div>${c.mpesaDemo?`<button class="primary" onclick="demoPay()">Confirm trial payment</button>`:`<p class="small">Your booking remains on hold while payment is pending.</p>`}<div class="manual-payment-box"><h3>Automatic payment did not work?</h3><p>Please make your payment to Amy using <b>M-Pesa Send Money</b> to <b>0725 365692</b>. Then enter the confirmation below to finish and confirm your booking.</p><div class="form-grid"><label>M-Pesa confirmation code<input id="manualMpesaCode" autocomplete="off"></label><label>Amount sent (KES)<input id="manualMpesaAmount" type="number" min="1" value="${Number(c.amount||0)}"></label></div><button class="secondary" onclick="submitManualPaymentFromPayment()">Submit payment confirmation</button><p class="small">Amy will verify the payment before the booking is marked confirmed.</p></div><div class="actions"><button class="secondary" onclick="portal()">Return to Client Portal</button></div></div></div></section>`}
async function submitManualPaymentFromPayment(){const c=state.confirm;if(!c?.id)return alert('Open this pending booking from your Client Portal to submit manual payment.');const code=document.getElementById('manualMpesaCode').value.trim(),amount=Number(document.getElementById('manualMpesaAmount').value);try{await api(`/api/my/bookings/${c.id}/manual-payment`,{method:'POST',body:JSON.stringify({confirmationCode:code,amount})});alert('Thank you. Amy has been asked to verify your M-Pesa payment.');await portal()}catch(e){alert(e.message)}}
async function demoPay(){const c=state.confirm;if(c.type==='package'&&c.packageId)await api(`/api/bookings/package/${c.packageId}/demo-pay`,{method:'POST'});else await api(`/api/${c.type==='private'?'bookings':'classes'}/${c.bookingRef}/demo-pay`,{method:'POST'});state.completedBooking={...c,firstAppointment:c.firstAppointment===true};state.selectedService=null;state.selectedLocation=null;state.selectedDate=null;state.slots=[];state.selectedSlot=null;state.selectedClass=null;state.address='';state.privateTermsAccepted=false;state.classTermsAccepted=false;go('confirmation')}
function confirmationView(){const c=state.completedBooking||state.confirm||{},isClass=c.type==='class',title=isClass?(state.selectedClass?.title||'Class course'):c.type==='package'?'Private training package':privateServiceLabel(c.service),dog=state.profile?.pets?.find(p=>p.id===(c.petId||state.selectedPet))?.name;return `<section class="screen"><div class="center"><div style="max-width:700px"><div class="eyebrow">Booking confirmed</div><h1>You're booked.</h1><p class="lead">${esc(title)}${dog?` · ${esc(dog)}`:''}</p><div class="notice good"><b>Payment received.</b><br>Keep your booking reference <b>${esc(c.bookingRef)}</b>.</div>${c.firstAppointment!==false?`<div class="notice vaccination-reminder"><b>Please bring your original vaccination record with you to your first appointment.</b></div>`:""}<div class="actions"><button class="secondary" onclick="portal()">Open Client Portal</button><button class="secondary" onclick="go('home')">Back home</button></div></div></div></section>`}
...
function pendingBookingActions(x){
 ...
 return `<div class="notice">Awaiting payment${x.hold_expires_at?` — slot held until ${fmt(x.hold_expires_at)}`:''}.</div><div class="actions"><button class="primary compact-button" onclick="resumePendingPayment(${x.id})">Resume payment</button><button class="secondary compact-button" onclick="manualPaymentForBooking(${x.id},${Number(x.price||x.package_price||0)})">M-Pesa Send Money</button><button class="danger compact-button" onclick="cancelPendingBooking(${x.id})">Cancel booking</button></div>`;
}

function bookingsView(b){const privateRows=Array.isArray(b?.privateBookings)?b.privateBookings:[],classRows=Array.isArray(b?.classBookings)?b.classBookings:[];
const packageOffers=new Map();for(const x of privateRows){if(x.package_id&&x.status==='provisional'&&x.payment_status==='pending'&&!packageOffers.has(x.package_id))packageOffers.set(x.package_id,x)}
const visiblePrivate=privateRows.filter(x=>!(x.package_id&&x.status==='provisional'&&x.payment_status==='pending'));
const packageCards=[...packageOffers.values()].map(x=>{const sessions=privateRows.filter(y=>Number(y.package_id)===Number(x.package_id)).sort((p,q)=>String(p.start_at).localeCompare(String(q.start_at)));return `<div class="card provisional-card"><h3>${esc(x.package_name||'Private training package')} · ${esc(x.pet_name||'Dog')}</h3><p>${sessions.length} appointment${sessions.length===1?'':'s'} · ${money(x.package_price||0)} total</p><div class="compact-list">${sessions.map(y=>`<div>${fmt(y.start_at)} · ${y.location_type==='home'?'Home visit':arenaClientLabel()}</div>`).join('')}</div>${pendingBookingActions(x)}</div>`}).join('');
return `<div class="list">${packageCards}${visiblePrivate.map(x=>`<div class="card ${['provisional','pending_payment'].includes(x.status)?'provisional-card':''}"><h3>${x.package_name?`${esc(x.package_name)} · `:''}Private training · ${esc(x.pet_name||'Dog not recorded')}</h3><p>${fmt(x.start_at)} · ${x.location_type==='home'?'Home visit':arenaClientLabel()}</p><p>${esc(x.booking_ref)} · ${x.status==='expired'?'Expired':x.payment_status==='pending'?'Awaiting payment':esc(x.payment_status)}${x.status==='cancelled'?' · Cancelled':''}</p>${pendingBookingActions(x)}${x.status==='expired'?`<div class="actions"><button class="secondary compact-button" onclick="startPrivate('portal')">Book again</button></div>`:''}${x.manual_payment_status==='submitted'?`<div class="notice">Manual M-Pesa confirmation sent to Amy for verification.</div>`:''}${x.first_appointment&&['paid','demo_paid'].includes(x.payment_status)?`<div class="notice vaccination-reminder">Please bring your original vaccination record with you to your first appointment.</div>`:''}${['paid','demo_paid'].includes(x.payment_status)&&x.status!=='cancelled'?`<div class="actions"><button class="secondary compact-button" onclick="addPrivateCalendarByRef('${esc(x.booking_ref)}')">＋ Add to calendar</button><button class="secondary compact-button" onclick="clientReschedule(${x.id})">Request reschedule · ${Math.max(0,3-Number(x.reschedule_count||0))} left</button><button class="danger compact-button" onclick="clientCancel(${x.id})">Cancel</button></div>`:''}</div>`).join('')}${classRows.map(x=>`<div class="card"><h3>${esc(x.title)} · ${esc(x.pet_name||'Dog')}</h3><p>${displayDate(x.start_date,{day:'numeric',month:'short',year:'numeric'})}–${displayDate(x.end_date,{day:'numeric',month:'short',year:'numeric'})} · ${esc(x.start_time||'')}–${esc(x.end_time||'')}</p><p>${esc(x.booking_ref)} · ${esc(x.payment_status)}</p>${x.payment_status==='pending'?`<div class="notice">Awaiting payment${x.hold_expires_at?` — place held until ${fmt(x.hold_expires_at)}`:''}.</div>`:''}${x.enrolment_status==='active'&&['paid','demo_paid'].includes(x.payment_status)?`<div class="actions"><button class="secondary compact-button" onclick="addClassCalendarByRef('${esc(x.booking_ref)}')">＋ Add all classes</button><button class="danger compact-button" onclick="clientCancelClass(${x.id})">Cancel class place</button></div>`:''}</div>`).join('')}${!privateRows.length&&!classRows.length?'<div class="center"><p>No bookings yet.</p></div>':''}</div>`}
async function resumePendingPayment(id){try{const d=await api(`/api/my/bookings/${id}/resume-payment`,{method:'POST',body:'{}'});state.confirm={...d,type:d.type||'private'};go('payment')}catch(e){if(e.paymentPending){state.confirm={...e,type:'private'};go('payment')}else alert(e.message)}}
async function cancelPendingBooking(id){if(!confirm('Cancel this unpaid booking and release the held time?'))return;try{await api(`/api/my/bookings/${id}/cancel-pending`,{method:'POST',body:'{}'});await portal()}catch(e){alert(e.message)}}
function manualPaymentForBooking(id,amount){state.manualPaymentBooking={id,amount};const code=prompt('Enter the M-Pesa Send Money confirmation code:','');if(code===null)return;const sent=prompt('Amount sent in KES:',String(amount||''));if(sent===null)return;api(`/api/my/bookings/${id}/manual-payment`,{method:'POST',body:JSON.stringify({confirmationCode:code.trim(),amount:Number(sent)})}).then(()=>portal()).then(()=>alert('Amy has been asked to verify your payment.')).catch(e=>alert(e.message))}
function clientReschedule(id){const b=(state.bookings?.privateBookings||[]).find(x=>x.id===id);if(!b)return;if(Number(b.reschedule_count||0)>=3)return alert('The three client-requested reschedules have been used. Please contact Amy.');state.rescheduleDraft={bookingId:id,booking:b,date:'',slots:[],selected:null};render()}
async function confirmClientReschedule(){const r=state.rescheduleDraft;if(!r?.selected)return;const note=prompt('Optional note for Amy about this request:','');if(note===null)return;try{const d=await api(`/api/my/bookings/${r.bookingId}/reschedule`,{method:'POST',body:JSON.stringify({startAt:r.selected.start,note})});state.rescheduleDraft=null;await portal();alert(`Your reschedule request has been sent to Amy. Your original appointment remains confirmed until Amy approves the change. The requested slot is held for 24 hours.`)}catch(e){alert(e.message)}}

function accountView(){const u=state.profile?.user||state.user||{};return `<section class="screen"><div class="center"><div class="panel account-panel" style="width:min(680px,100%)"><button class="back" onclick="go(state.user?.role==='trainer'?'trainer':'portal')">← Back</button><div class="eyebrow">Account & billing</div><h2>Your details</h2>${state.user?.role==='client'?`<label>Name<input id="accountName" value="${esc(u.name||'')}"></label><div class="form-grid"><label>WhatsApp number<input id="accountWhatsapp" value="${esc(u.whatsapp_phone||u.phone||'')}"></label><label>M-Pesa number <span class="small">(if different)</span><input id="accountMpesa" value="${esc(u.mpesa_phone||u.phone||'')}"></label></div><label>KRA PIN <span class="small">(optional — for eTIMS invoices or receipts)</span><input id="accountKra" value="${esc(u.kra_pin||'')}"></label><label class="newsletter-optin"><input id="accountNewsletter" type="checkbox" ${u.newsletter_opt_in?'checked':''}> Please tick here if you would like to receive my newsletter, <em>The Canine Grapevine</em>, which I try to send out at least every 2 months, to keep you informed about upcoming classes, events and other news.</label><div class="actions"><button class="primary" onclick="saveAccountDetails()">Save account details</button></div><hr>`:''}<h3>Change password</h3><label>Current password<input id="currentPassword" type="password"></label><label>New password<input id="newPassword" type="password"></label><label>Confirm new password<input id="confirmPassword" type="password"></label><div class="actions"><button class="secondary" onclick="changePassword()">Change password</button></div></div></div></section>`}
async function saveAccountDetails(){try{const d=await api('/api/my/profile',{method:'PUT',body:JSON.stringify({name:document.getElementById('accountName').value,whatsappPhone:document.getElementById('accountWhatsapp').value,mpesaPhone:document.getElementById('accountMpesa').value,kraPin:document.getElementById('accountKra').value,newsletterOptIn:document.getElementById('accountNewsletter').checked})});state.profile.user=d.user;state.user={...state.user,...d.user};alert('Account details saved.');render()}catch(e){alert(e.message)}}
...
async function selectClientForBooking(id){
 ...
 }catch(e){alert(e.message||'Could not open this client for booking.')}
}
function clientAdminView(){
 const rows=state.clientAdmin||[],bookingMode=!!state.bookForClientMode;
 const shown=state.archivedClientsOpen?rows.filter(x=>x.client_status==='archived'):rows.filter(x=>x.client_status!=='archived');
 const groups=state.archivedClientsOpen?['archived']:['current','dormant'];
 const rowAction=x=>bookingMode?`selectClientForBooking(${x.id})`:`openClientRecord(${x.id})`;
 ...
 <div class="admin-head"><div><div class="eyebrow">Amy's workspace</div><h2>${bookingMode?'Book for client':state.archivedClientsOpen?'Archived clients':'Clients'}</h2>${bookingMode?'<p class="small">Choose a client to go straight to the booking screen. Client details will not open first.</p>':''}</div>
 <div class="actions">${bookingMode?'':`<button class="secondary compact-button" onclick="state.archivedClientsOpen=!state.archivedClientsOpen;render()">${state.archivedClientsOpen?'← Current & dormant':`Archived clients · ${rows.filter(x=>x.client_status==='archived').length}`}</button>`}<input class="admin-search" placeholder="Search client or dog" oninput="filterClientAdmin(this.value)"></div></div>
 <div class="client-groups client-groups-stacked">${groups.map(g=>`<div class="client-group client-group-wide"><h3>${g[0].toUpperCase()+g.slice(1)} · ${shown.filter(x=>x.client_status===g).length}</h3>${shown.filter(x=>x.client_status===g).map(x=>`<button class="client-overview-card" data-search="${esc(([x.name,x.email,x.whatsapp_phone,x.mpesa_phone,...(x.pets||[]).map(p=>p.name)].join(' ')).toLowerCase())}" onclick="${rowAction(x)}"><span class="client-overview-head"><span><span class="client-overview-name">${esc(x.name)}</span><small>${esc(x.email)}${x.whatsapp_phone?` · WhatsApp ${esc(x.whatsapp_phone)}`:''}</small></span><span class="client-open-arrow">→</span></span><span class="client-dog-table">${(x.pets||[]).filter(p=>!p.archived).map(p=>`<span class="client-dog-table-row"><span>${esc(p.name)}</span><span>${Number(p.class_count||0)}</span><span>${Number(p.private_count||0)}</span></span>`).join('')||'<span class="small">No active dogs.</span>'}</span></button>`).join('')||'<p class="small">None</p>'}</div>`).join('')}</div>
...
function clientRecordModal(){const c=state.clientRecord;if(!c)return '';return `<div class="modal-overlay"><div class="trainer-modal client-record-modal"><button class="close-btn modal-close" onclick="closeClientRecord()">×</button><div class="eyebrow">Client record</div><div class="client-record-title"><div><h2>${esc(c.user.name)}</h2><p>${esc(c.user.email)}</p><p>WhatsApp: ${esc(c.user.whatsapp_phone||c.user.phone||'—')} · M-Pesa: ${esc(c.user.mpesa_phone||c.user.phone||'—')}</p><p>KRA PIN: ${esc(c.user.kra_pin||'Not supplied')} · Canine Grapevine: ${c.user.newsletter_opt_in?'Opted in':'Not subscribed'}</p></div><label class="client-status-mini">Client status<select onchange="setClientStatusFromRecord(${c.user.id},this.value)"><option value="automatic">Automatic (${esc(c.user.client_status)})</option>${['current','dormant','archived'].map(v=>`<option value="${v}" ${v===c.user.status_override?'selected':''}>${v}</option>`).join('')}</select></label></div><h3>Dogs</h3><div class="trainer-dog-records">${(c.pets||[]).map(p=>`<article class="trainer-dog-record"><h4>${esc(p.name)}</h4><p>${esc(p.breed||'Dog')}${p.gender?` · ${p.gender}`:''}${p.date_of_birth?` · DOB ${displayDate(p.date_of_birth,{day:'numeric',month:'short',year:'numeric'})}`:''}</p><label>Amy's private notes<textarea id="trainerNotes-${p.id}" rows="3" ${Number(state.editingTrainerNotePetId)===Number(p.id)?'':'readonly'}>${esc(p.trainer_notes||'')}</textarea></label><div class="actions">${Number(state.editingTrainerNotePetId)===Number(p.id)?`<button class="primary compact-button" onclick="saveTrainerDogNotes(${p.id})">Save note</button><button class="secondary compact-button" onclick="cancelTrainerDogNoteEdit()">Cancel edit</button>`:`<button class="secondary compact-button" onclick="editTrainerDogNote(${p.id})">Edit note</button>`}<button class="secondary compact-button" onclick="openVaccinationReview(${p.id})">Vaccination</button></div></article>`).join('')}</div><h3>Bookings</h3><p>${(c.bookings||[]).length} private booking(s) · ${(c.packages||[]).length} package(s) · ${(c.classes||[]).length} class enrolment(s).</p><div class="actions"><button class="primary" onclick="startTrainerClientBooking(${c.user.id})">＋ Book for this client</button><button class="secondary" onclick="closeClientRecord()">Done</button></div></div></div>`}

function attentionAdminView(){const t=state.trainer||{},none=!(t.vaccinationAttention||[]).length&&!(t.cancellationAttention||[]).length&&!(t.classRefundAttention||[]).length&&!(t.rescheduleAttention||[]).length&&!(t.manualPaymentAttention||[]).length&&!(t.notifications||[]).length;return `<section class="screen admin-screen"><button class="back-dashboard" onclick="dashboardBack()">← Back to Dashboard</button><div class="admin-head"><div><div class="eyebrow">Amy's workspace</div><h2>Needs attention</h2><div class="actions">${(t.pendingReviews||[]).length?`<button class="secondary compact-button" onclick="openTrainerAdmin(\'reviews\')">Pending reviews · ${(t.pendingReviews||[]).length}</button>`:`<button class="secondary compact-button" disabled title="There are no pending reviews">No pending reviews</button>`}</div></div></div><div class="admin-list-large">${(t.notifications||[]).filter(n=>n.kind==='new_account').map(n=>`<div class="card"><b>New client · ${esc(n.client_name||'')}</b><p>${esc(n.message)}</p><div class="actions"><button class="secondary" onclick="openClientRecordFromAttention(${n.user_id})">Open client</button><button class="secondary compact-button" onclick="resolveTrainerNotification(${n.id})">Mark seen</button></div></div>`).join('')}${(t.rescheduleAttention||[]).map(x=>`<div class="card"><b>Reschedule request · ${esc(x.client_name)} · ${esc(x.pet_name||'')}</b><p>Current: ${fmt(x.old_start_at)}<br>Requested: ${fmt(x.proposed_start_at)}<br>Held until ${fmt(x.hold_expires_at)}</p><div class="actions"><button class="primary compact-button" onclick="decideRescheduleRequest(${x.id},'approve')">Approve</button><button class="danger compact-button" onclick="decideRescheduleRequest(${x.id},'decline')">Decline</button></div></div>`).join('')}${(t.manualPaymentAttention||[]).map(x=>`<div class="card"><b>Manual M-Pesa to verify · ${esc(x.client_name)}</b><p>${esc(x.pet_name||'')} · ${esc(x.booking_ref)} · Code ${esc(x.manual_payment_code)} · Reported ${money(x.manual_payment_amount)} · Due ${money(x.price)}</p><div class="actions"><button class="primary compact-button" onclick="verifyManualPayment(${x.id},true)">Verify payment</button><button class="danger compact-button" onclick="verifyManualPayment(${x.id},false)">Reject</button></div></div>`).join('')}${(t.vaccinationAttention||[]).map(x=>`<div class="card"><span>Vaccination · ${esc(x.pet_name)}</span><p>${esc(x.client_name)} · ${Number(x.vaccination_count||0)===0?'No vaccination record uploaded':x.vaccination_status==='rejected'?'Replacement requested':'Record waiting for review'}</p><button class="secondary" onclick="openVaccinationReview(${x.pet_id})">Review record</button></div>`).join('')}${(t.cancellationAttention||[]).map(x=>`<div class="card"><b>Cancellation · ${esc(x.booking_ref)}</b><p>${esc(x.client_name)} · ${esc(x.pet_name||'')}</p><button class="secondary" onclick="openTrainerBooking(${x.id})">Handle refund</button></div>`).join('')}${none?'<p class="small">Nothing needs attention.</p>':''}</div></section>`}
...
async function decideRescheduleRequest(id,decision){const note=prompt(decision==='approve'?'Optional note to client:':'Reason / note to client:','');if(note===null)return;try{await api(`/api/trainer/reschedule-requests/${id}/decision`,{method:'POST',body:JSON.stringify({decision,note})});state.trainer=await api('/api/trainer/summary');await loadTrainerCalendar(new Date());await loadTrainerMonth(new Date());render()}catch(e){alert(e.message)}}
async function verifyManualPayment(id,approve){if(!confirm(approve?'Confirm that this M-Pesa payment has been received?':'Reject this manual payment confirmation?'))return;await api(`/api/trainer/bookings/${id}/manual-payment`,{method:'POST',body:JSON.stringify({approve})});state.trainer=await api('/api/trainer/summary');render()}
...
function reportDisplayColumns(report){
 ...
 const wanted={
  ...
  payments:['start_at','booking_ref','client','dog','source','amount','payment_status','manual_payment_code','manual_payment_amount','refund_amount'],
...
function reportColumnLabel(k){
 const labels={start_at:'Date / time',end_at:'Ends',booking_ref:'Reference',payment_status:'Payment',manual_payment_code:'M-Pesa code',manual_payment_amount:'Manual paid',refund_amount:'Refund',location_type:'Location',newsletter_opt_in:'Grapevine',vaccination_status:'Status',vaccination_verified_at:'Verified'};
