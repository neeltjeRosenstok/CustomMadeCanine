...
function trainerAgenda(){
 ...
 if(!w.enabled||!w.start_time||!w.end_time)return `<div class="trainer-agenda"><div class="empty-agenda"><span>Amy is not working on this date.</span></div></div>`;
 ...
 for(let i=0;i<times.length-1;i++){
  ...
  else if(sblock){kind="block";title=/\blunch\b/i.test(String(sblock.reason||''))?"LUNCH":(sblock.reason||"Amy unavailable")}
  else if(block){kind="block";title=/\blunch\b/i.test(String(block.reason||''))?"LUNCH":(block.reason||"Amy unavailable")}
  else if(rec){kind="block";title=/\blunch\b/i.test(String(rec.reason||''))?"LUNCH":(rec.reason||"Recurring block")}
  const lunch=title==="LUNCH";
  rows.push({st,en,kind,title,detail,id,locationType,lunch});
 }
 ...
 return `<div class="trainer-agenda"><div class="agenda-date"><div><div class="eyebrow">Daily agenda</div><h3>${displayDate(key,{weekday:"long",day:"numeric",month:"long"})}</h3></div><span class="agenda-working-hours">${w.start_time}–${w.end_time}</span></div><div class="agenda-timeline">${merged.map(r=>{const body=`<div class="agenda-time">${r.st}–${r.en}</div><div class="agenda-copy"><span class="agenda-title">${esc(r.title)}</span>${r.detail?`<p>${esc(r.detail)}</p>`:""}</div>`;const cls=`agenda-item ${r.kind}${r.kind==="booking"&&r.locationType?` booking-${r.locationType}`:""}${r.lunch?" lunch-item":""}`;return r.kind==="booking"?`<button class="${cls} clickable-agenda" onclick="openTrainerBooking(${r.id})">${body}</button>`:`<div class="${cls}">${body}</div>`}).join("")}</div></div>`;
}
...
function authView(){
 const hasBooking=!!(state.selectedService||state.selectedClass),register=state.authMode==='register';
 return `<section class="screen"><button class="back" onclick="go(state.selectedService?'private':'classes')">← Back to booking</button><div class="center"><div class="panel auth-panel">
 ...
 ${register?`<label>Name<input id="authName" autocomplete="name"></label><label>WhatsApp number<input id="authWhatsapp" placeholder="07… or 2547…" autocomplete="tel"></label><label>M-Pesa number <span class="small">(if different)</span><input id="authMpesa" placeholder="Leave blank if same as WhatsApp" autocomplete="tel"></label>`:''}
 <label>Email<input id="authEmail" type="email" autocomplete="email" value="${esc(localStorage.getItem('cmc_last_email')||'')}"></label>
 <label>Password<input id="authPassword" type="password" autocapitalize="none" autocorrect="off" spellcheck="false" autocomplete="${register?'new-password':'current-password'}"></label>
 ${register?`<p class="password-rule"><b>Please use 8 characters, including at least one letter and one number. Symbols and spaces are permitted.</b></p><label class="newsletter-optin"><input id="authNewsletter" type="checkbox"> Please tick here if you would like to receive my newsletter, <em>The Canine Grapevine</em>, which I try to send out at least every 2 months, to keep you informed about upcoming classes, events and other news.</label>`:''}
...
async function submitAuth(){
 ...
 try{const body={email:document.getElementById('authEmail').value.trim(),password:document.getElementById('authPassword').value};if((state.authMode||'login')==='register'){if(!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(body.email))throw new Error("Please enter a valid email address.");if(!(body.password.length>=8&&/[A-Za-z]/.test(body.password)&&/\d/.test(body.password)))throw new Error("Please use 8 characters, including at least one letter and one number. Symbols and spaces are permitted.");body.name=document.getElementById('authName').value;body.whatsappPhone=document.getElementById('authWhatsapp').value;body.mpesaPhone=document.getElementById('authMpesa').value;body.newsletterOptIn=!!document.getElementById('authNewsletter').checked}localStorage.setItem('cmc_last_email',body.email);const d=await api(`/api/auth/${(state.authMode||'login')==='register'?'register':'login'}`,{method:'POST',body:JSON.stringify(body)});state.user=d.user;if(state.user.role==='trainer'){
