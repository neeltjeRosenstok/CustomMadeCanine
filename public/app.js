...
function petCard(p,archived=false){
 ...
 return `<article class="dog-card-v2176 ${archived?"pet-archived":""}">
   ...
   <div class="dog-file-grid">
    ...
    <section class="dog-file-panel"><div class="vaccination-profile-title"><h4>Vaccination record</h4><button class="vaccination-info-button" type="button" aria-label="Vaccination record requirements" onclick="state.vaccinationInfoPetId=Number(${p.id});render()">ⓘ</button></div><p>${esc(vaccination)}</p><div class="dog-file-actions">${p.vaccination_count?`<button class="secondary compact-button" onclick="viewVaccinations(${p.id})">View</button>`:""}<label class="file-button">${p.vaccination_count?"Replace":"Upload"}<input type="file" accept="image/jpeg,image/png,image/webp,application/pdf" multiple onchange="uploadVaccinations(${p.id},this)"></label>${p.vaccination_count?`<button class="secondary compact-button" onclick="removeVaccinations(${p.id})">Remove</button>`:""}</div>${Number(state.vaccinationInfoPetId)===Number(p.id)?`<div class="vaccination-profile-info"><button class="close-inline-info" onclick="state.vaccinationInfoPetId=null;render()">×</button><h5>Vaccination record requirements</h5><p>Please upload clear photographs of your dog's vaccination record or passport pages.</p><p>These must show a valid and complete record of Parvo, DHLP and Rabies vaccination showing the vaccine sticker/batch label, date, validity/expiry, as well as your vet's stamp clearly reflecting their KVB number and signature.</p><p>Amy to be able to read the dog's details, vaccine details and dates.</p><p>You can add or replace vaccination information later from the dog's profile.</p></div>`:""}</section>
   </div>
...
function applicationSaveCurrentStep(){
 ...
 if(step===1)applicationSaveFields({name:"appName",email:"appEmail",whatsappPhone:"appWhatsapp",mpesaPhone:"appMpesa",newsletterOptIn:"appNewsletter",location:"appLocation",googleMapsLocation:"appGoogleMaps",introNote:"appIntro"});
...
function applicationStepOneContinue(){
 applicationSaveFields({name:"appName",email:"appEmail",whatsappPhone:"appWhatsapp",mpesaPhone:"appMpesa",newsletterOptIn:"appNewsletter",location:"appLocation",googleMapsLocation:"appGoogleMaps",introNote:"appIntro"});
 const d=state.applicationDraft;
 if(!d.name.trim()||!d.email.trim()||!d.whatsappPhone.trim()||!d.location.trim())return appAlert("Please complete your name, email, WhatsApp number and area.");
...
function openGoogleMapsLocation(value){const v=String(value||"").trim();if(!v)return appAlert("No Google Maps location has been added yet.");window.open(/^https?:\/\//i.test(v)?v:`https://www.google.com/maps?q=${encodeURIComponent(v)}`,"_blank","noopener")}
function useCurrentLocationForApplication(){if(!navigator.geolocation)return appAlert("This device does not provide browser location access. You can paste a Google Maps pin instead.");navigator.geolocation.getCurrentPosition(pos=>{const link=`https://www.google.com/maps?q=${pos.coords.latitude.toFixed(6)},${pos.coords.longitude.toFixed(6)}`;state.applicationDraft=state.applicationDraft||{};state.applicationDraft.googleMapsLocation=link;render()},()=>appAlert("Location permission was not granted. You can paste a Google Maps pin instead."),{enableHighAccuracy:true,timeout:12000,maximumAge:60000})}
...
function accountView(){const u=state.profile?.user||state.user||{};return `<section class="screen"><div class="center"><div class="panel account-panel" style="width:min(680px,100%)"><button class="back" onclick="go(state.user?.role==='trainer'?'trainer':'portal')">← Back</button><div class="eyebrow">Account & billing</div><h2>Your details</h2>${state.user?.role==='client'?`<div class="credit-balance-card"><span>Credit available</span><strong>${money(state.profile?.creditBalance||0)}</strong>${(state.profile?.creditHistory||[]).length?`<small>Recent credit activity is kept in your account history.</small>`:""}</div>`:""}${state.user?.role==='client'?`<label>Name<input id="accountName" value="${esc(u.name||'')}"></label><div class="form-grid"><label>WhatsApp number<input id="accountWhatsapp" value="${esc(u.whatsapp_phone||u.phone||'')}"></label><label>M-Pesa number <span class="small">(if different)</span><input id="accountMpesa" value="${esc(u.mpesa_phone||u.phone||'')}"></label></div><div class="form-grid"><label>Area<input id="accountArea" value="${esc(u.location||'')}" placeholder="Ridgeways, Loresho, Karen"></label><label>Google Maps location<input id="accountMaps" value="${esc(u.google_maps_location||'')}" placeholder="Paste a Google Maps pin/link"></label></div><div class="actions compact-map-actions">${u.google_maps_location?`<button class="secondary compact-button" onclick="openGoogleMapsLocation(document.getElementById('accountMaps')?.value)">Open map</button>`:""}</div><label>KRA PIN <span class="small">(optional — for eTIMS invoices or receipts)</span><input id="accountKra" value="${esc(u.kra_pin||'')}"></label><label class="newsletter-optin"><input id="accountNewsletter" type="checkbox" ${u.newsletter_opt_in?'checked':''}> Please tick here if you would like to receive my newsletter, <em>The Canine Grapevine</em>, which I try to send out at least every 2 months, to keep you informed about upcoming classes, events and other news.</label><div class="actions"><button class="primary" onclick="saveAccountDetails()">Save account details</button></div><hr>`:''}<h3>Change password</h3><label>Current password<input id="currentPassword" type="password" autocapitalize="none" autocorrect="off" spellcheck="false"></label><label>New password<input id="newPassword" type="password" autocapitalize="none" autocorrect="off" spellcheck="false"></label><label>Confirm new password<input id="confirmPassword" type="password" autocapitalize="none" autocorrect="off" spellcheck="false"></label><div class="actions"><button class="secondary" onclick="changePassword()">Change password</button></div></div></div></section>`}
async function saveAccountDetails(){try{const d=await api('/api/my/profile',{method:'PUT',body:JSON.stringify({name:document.getElementById('accountName').value,whatsappPhone:document.getElementById('accountWhatsapp').value,mpesaPhone:document.getElementById('accountMpesa').value,kraPin:document.getElementById('accountKra').value,location:document.getElementById('accountArea').value,googleMapsLocation:document.getElementById('accountMaps').value,newsletterOptIn:document.getElementById('accountNewsletter').checked})});state.profile.user=d.user;state.user={...state.user,...d.user};appAlert('Account details saved.');render()}catch(e){appAlert(e.message)}}
...
function clientAdminView(){
 ...
 const groups=state.archivedClientsOpen?['archived']:['applicant','current','dormant'];
 ...
 <div class="client-groups client-groups-stacked">${groups.map(g=>`<div class="client-group client-group-wide"><h3>${g==="applicant"?"New Client Applications":g[0].toUpperCase()+g.slice(1)} · ${shown.filter(x=>x.client_status===g).length}</h3>${shown.filter(x=>x.client_status===g).map(x=>`<button class="client-overview-card" data-search="${esc(([x.name,x.email,x.whatsapp_phone,x.mpesa_phone,...(x.pets||[]).map(p=>p.name)].join(' ')).toLowerCase())}" onclick="${rowAction(x)}"><span class="client-overview-head"><span><span class="client-overview-name">${esc(x.name)}</span><small>${esc(x.email)}${x.whatsapp_phone?` · WhatsApp ${esc(x.whatsapp_phone)}`:''}</small>${x.client_status==="applicant"?`<small class="application-stage-label">${esc(x.application_stage||"Application in progress")}</small>`:""}</span><span class="client-open-arrow">→</span></span><span class="client-dog-table">${(x.pets||[]).filter(p=>!p.archived).map(p=>`<span class="client-dog-table-row client-dog-name-only"><span>${esc(p.name)}</span></span>`).join('')||'<span class="small">No active dogs.</span>'}</span></button>`).join('')||'<p class="small">None</p>'}</div>`).join('')}</div>
...
function applicationPaymentStatusLabel(d){if(!d)return "No application payment submitted";if(d.manual_payment_status==="submitted")return "UNVERIFIED";if(d.manual_payment_status==="verified")return "Verified";if(d.manual_payment_status==="rejected")return "Rejected";return d.manual_payment_status||"Not submitted"}
async function approveClientApplication(id){if(!await appConfirm("Approve this New Client Application? Booking and class access will become available."))return;try{await api(`/api/trainer/clients/${id}/approve`,{method:"POST",body:"{}"});state.clientRecord=await api(`/api/trainer/client/${id}`);state.clientAdmin=await api("/api/trainer/clients");state.trainer=await api("/api/trainer/summary");render()}catch(e){appAlert(e.message)}}
function clientRecordModal(){const c=state.clientRecord;if(!c)return '';const u=c.user||{},dep=c.applicationDeposit,applicant=u.raw_client_status!=="current";return `<div class="modal-overlay"><div class="trainer-modal client-record-modal"><button class="close-btn modal-close" onclick="closeClientRecord()">×</button><div class="eyebrow">${applicant?"New Client Application":"Client record"}</div><div class="client-record-title"><div><h2>${esc(u.name)}</h2><p>${esc(u.email)}</p><p>WhatsApp: ${esc(u.whatsapp_phone||u.phone||'—')} · M-Pesa: ${esc(u.mpesa_phone||u.phone||'—')}</p><p>Area: ${esc(u.location||'Not supplied')}${u.google_maps_location?` · <button class="text-button" onclick='openGoogleMapsLocation(${JSON.stringify(u.google_maps_location)})'>Open Google Maps pin</button>`:""}</p><p>Canine Grapevine: ${u.newsletter_opt_in?'Opted in':'Not subscribed'} · Profile created: ${esc(formatReportDate(u.created_at||''))}</p></div>${applicant?`<div class="application-stage-card"><span>Application stage</span><strong>${esc(u.application_stage||"In progress")}</strong><small>Deposit: ${esc(applicationPaymentStatusLabel(dep))}${dep?.manual_payment_code?` · ${esc(dep.manual_payment_code)}`:""}</small></div>`:`<label class="client-status-mini">Client status<select onchange="setClientStatusFromRecord(${u.id},this.value)"><option value="automatic">Automatic (${esc(u.client_status)})</option>${['current','dormant','archived'].map(v=>`<option value="${v}" ${v===u.status_override?'selected':''}>${v}</option>`).join('')}</select></label>`}</div>${u.client_intro_note?`<section class="client-record-section"><h3>Client introduction</h3><p>${esc(u.client_intro_note)}</p></section>`:""}<section class="client-record-section"><h3>Household</h3><div class="client-data-grid"><span><b>Dogs</b>${esc(u.household_dogs??'—')}</span><span><b>Adults</b>${esc(u.household_adults??'—')}</span><span><b>Children 0–8</b>${esc(u.children_0_8??'—')}</span><span><b>Children 9–13</b>${esc(u.children_9_13??'—')}</span><span><b>Children 14+</b>${esc(u.children_14_plus??'—')}</span></div>${u.household_changes?`<p><b>Household changes:</b> ${esc(u.household_changes)}</p>`:""}${u.household_note?`<p><b>Additional note:</b> ${esc(u.household_note)}</p>`:""}</section><div class="credit-balance-card trainer-credit"><span>Account credit</span><strong>${money(c.creditBalance||0)}</strong></div><h3>Dogs</h3><div class="trainer-dog-records">${(c.pets||[]).map(p=>`<article class="trainer-dog-record"><h4>${esc(p.name)}</h4><p>${esc(p.breed||'Dog')}${p.gender?` · ${p.gender}`:''}${p.date_of_birth?` · DOB ${displayDate(p.date_of_birth,{day:'numeric',month:'short',year:'numeric'})}`:''} · Vaccination ${esc(p.vaccination_status||"not provided")}</p>${p.behavior_notes?`<p><b>Behaviour:</b> ${esc(p.behavior_notes)}</p>`:""}${p.medical_procedures?`<p><b>Medical:</b> ${esc(p.medical_procedures)}</p>`:""}<label>Amy's private notes<textarea id="trainerNotes-${p.id}" rows="3" ${Number(state.editingTrainerNotePetId)===Number(p.id)?'':'readonly'}>${esc(p.trainer_notes||'')}</textarea></label><div class="actions">${Number(state.editingTrainerNotePetId)===Number(p.id)?`<button class="primary compact-button" onclick="saveTrainerDogNotes(${p.id})">Save note</button><button class="secondary compact-button" onclick="cancelTrainerDogNoteEdit()">Cancel edit</button>`:`<button class="secondary compact-button" onclick="editTrainerDogNote(${p.id})">Edit note</button>`}<button class="secondary compact-button" onclick="openVaccinationReview(${p.id})">Vaccination</button></div></article>`).join('')}</div><h3>Resources shared</h3>${(c.resources||[]).length?`<div class="client-shared-resources">${(c.resources||[]).map(r=>`<div class="client-shared-resource"><strong>${esc(r.title)}</strong><span>${r.shared_at?esc(formatReportDate(r.shared_at)):""} · ${esc(r.category||"General")}</span></div>`).join("")}</div>`:'<p class="small">No training resources shared yet.</p>'}<h3>Bookings</h3><p>${(c.bookings||[]).length} private booking(s) · ${(c.packages||[]).length} package(s) · ${(c.classes||[]).length} class enrolment(s).</p><div class="actions">${applicant&&dep?.manual_payment_status==="verified"?`<button class="primary" onclick="approveClientApplication(${u.id})">Approve client</button>`:""}<button class="secondary" onclick="openClientReport()">Client report / Print</button>${!applicant?`<button class="primary" onclick="startTrainerClientBooking(${u.id})">＋ Book for this client</button>`:""}<button class="secondary" onclick="closeClientRecord()">Done</button></div></div></div>${clientReportModal()}`}


function clientReportDefaultSections(){const c=state.clientRecord||{},x={header:true,client:true,household:true,history:false,trainingNotes:true,payments:true,resources:true};for(const p of c.pets||[])x[`dog-${p.id}`]=true;return x}
function openClientReport(){state.clientReportRange="all";state.clientReportSections=clientReportDefaultSections();state.clientReport={from:"",to:""};render()}
function closeClientReport(){state.clientReport=null;render()}
function clientReportInRange(v){const k=String(v||"").slice(0,10),r=state.clientReport||{};return !k||(!r.from||k>=r.from)&&(!r.to||k<=r.to)}
function setClientReportRange(kind){const today=new Date(`${nairobiDateKeyClient(0)}T12:00:00`),to=dateKey(today),fromDate=new Date(today);if(kind==="all"){state.clientReport={from:"",to:""};state.clientReportRange="all";return render()}if(kind==="week")fromDate.setDate(fromDate.getDate()-6);else fromDate.setMonth(fromDate.getMonth()-1);state.clientReport={from:dateKey(fromDate),to};state.clientReportRange=kind;render()}
function syncClientReportDates(){state.clientReport={from:document.getElementById("clientReportFrom")?.value||"",to:document.getElementById("clientReportTo")?.value||""};state.clientReportRange="custom";render()}
function toggleClientReportSection(key,checked){state.clientReportSections=state.clientReportSections||clientReportDefaultSections();state.clientReportSections[key]=!!checked;render()}
function clientReportModal(){const c=state.clientRecord;if(!state.clientReport||!c)return "";const u=c.user||{},sel=state.clientReportSections||clientReportDefaultSections(),check=(k,l)=>`<label class="check-row"><input type="checkbox" ${sel[k]?"checked":""} onchange='toggleClientReportSection(${JSON.stringify(k)},this.checked)'> ${esc(l)}</label>`,payments=(c.payments||[]).filter(x=>clientReportInRange(x.date)),notes=(c.trainingNotes||[]).filter(x=>clientReportInRange(x.created_at)),history=(c.activity||[]).filter(x=>clientReportInRange(x.created_at)),resources=(c.resources||[]).filter(x=>clientReportInRange(x.shared_at));return `<div class="modal-overlay client-report-overlay"><div class="trainer-modal client-report-builder"><button class="close-btn modal-close no-print" onclick="closeClientReport()">×</button><div class="no-print"><div class="eyebrow">Client report</div><h2>Choose what to print</h2><div class="client-report-range"><button class="secondary compact-button" onclick="setClientReportRange('week')">Last 7 days</button><button class="secondary compact-button" onclick="setClientReportRange('month')">Last month</button><button class="secondary compact-button" onclick="setClientReportRange('all')">All history</button><label>From<input id="clientReportFrom" type="date" value="${esc(state.clientReport.from||"")}" onchange="syncClientReportDates()"></label><label>To<input id="clientReportTo" type="date" value="${esc(state.clientReport.to||"")}" onchange="syncClientReportDates()"></label></div><div class="client-report-checkboxes">${check("header","Business / report header")}${check("client","Client information")}${check("household","Household")}${(c.pets||[]).map(p=>check(`dog-${p.id}`,`Dog: ${p.name}`)).join("")}${check("history","History")}${check("trainingNotes","Training notes")}${check("payments","Payments")}${check("resources","Resources shared")}</div><div class="actions"><button class="primary" onclick="window.print()">Print / Save PDF</button><button class="secondary" onclick="closeClientReport()">Close</button></div></div><article class="client-report-paper">${sel.header?`<header class="client-report-header"><img src="/brand-logo.png" alt=""><div><h1>Client Profile: ${esc(u.name||"")}</h1><strong>The Custom Made Canine</strong><div>Amy L. Rapp · 0733 255406</div></div></header>`:""}${sel.client?`<section><h2>Client information</h2><div class="report-detail-grid"><div><b>Profile Created</b><span>${esc(formatReportDate(u.created_at||""))}</span></div><div><b>Email</b><span>${esc(u.email||"")}</span></div><div><b>Area</b><span>${esc(u.location||"—")}</span></div><div><b>WhatsApp</b><span>${esc(u.whatsapp_phone||u.phone||"—")}</span></div><div><b>M-Pesa</b><span>${esc(u.mpesa_phone||u.phone||"—")}</span></div><div><b>Newsletter</b><span>${u.newsletter_opt_in?"Yes":"No"}</span></div><div class="report-wide"><b>Google Maps pin</b><span>${esc(u.google_maps_location||"—")}</span></div>${u.client_intro_note?`<div class="report-wide"><b>Client Introduction</b><span>${esc(u.client_intro_note)}</span></div>`:""}</div></section>`:""}${sel.household?`<section><h2>Household</h2><div class="report-detail-grid"><div><b>Number of dogs</b><span>${esc(u.household_dogs??"—")}</span></div><div><b>Adults</b><span>${esc(u.household_adults??"—")}</span></div><div><b>Children 0–8</b><span>${esc(u.children_0_8??"—")}</span></div><div><b>Children 9–13</b><span>${esc(u.children_9_13??"—")}</span></div><div><b>Children 14+</b><span>${esc(u.children_14_plus??"—")}</span></div>${u.household_changes?`<div class="report-wide"><b>Major household changes</b><span>${esc(u.household_changes)}</span></div>`:""}${u.household_note?`<div class="report-wide"><b>Additional notes</b><span>${esc(u.household_note)}</span></div>`:""}</div></section>`:""}${(c.pets||[]).filter(p=>sel[`dog-${p.id}`]).map(p=>`<section><h2>Dog Profile: ${esc(p.name)}</h2><div class="report-detail-grid"><div><b>Date of Birth</b><span>${p.date_of_birth?esc(formatReportDate(p.date_of_birth)):"—"}</span></div><div><b>Breed</b><span>${esc(p.breed||"—")}</span></div><div><b>Sex</b><span>${p.gender==="male"?"Male":p.gender==="female"?"Female":"—"}</span></div><div><b>Vaccination verified</b><span>${p.vaccination_status==="verified"?"Yes":"No · "+esc(p.vaccination_status||"not provided")}</span></div>${p.behavior_notes?`<div class="report-wide"><b>Behaviour</b><span>${esc(p.behavior_notes)}</span></div>`:""}${p.medical_procedures?`<div class="report-wide"><b>Medical history</b><span>${esc(p.medical_procedures)}</span></div>`:""}${p.trainer_notes?`<div class="report-wide"><b>Amy's private dog notes</b><span>${esc(p.trainer_notes)}</span></div>`:""}</div></section>`).join("")}${sel.history?`<section><h2>History</h2>${history.length?history.map(x=>`<div class="report-line"><time>${esc(formatReportDate(x.created_at))}</time><span>${esc(x.details||x.action||"")}</span></div>`).join(""):'<p>No history in this date range.</p>'}</section>`:""}${sel.trainingNotes?`<section><h2>Training notes</h2>${notes.length?notes.map(x=>`<div class="report-line"><time>${esc(formatReportDate(x.created_at))}</time><span>${x.pet_name?`<b>${esc(x.pet_name)}:</b> `:""}${esc(x.note)}</span></div>`).join(""):'<p>No training notes in this date range.</p>'}</section>`:""}${sel.payments?`<section><h2>Payments</h2>${payments.length?payments.map(x=>`<div class="report-line"><time>${esc(formatReportDate(x.date))}</time><span>${esc(x.confirmation_code||x.reference||"—")} · ${money(x.amount||0)} · ${esc(x.status||"")}</span></div>`).join(""):'<p>No payments in this date range.</p>'}<div class="report-balance"><b>Account balance:</b> ${money(c.creditBalance||0)}</div></section>`:""}${sel.resources?`<section><h2>Resources shared</h2>${resources.length?resources.map(x=>`<div class="report-line"><time>${esc(formatReportDate(x.shared_at||""))}</time><span>${esc(x.title)}</span></div>`).join(""):'<p>No resources shared in this date range.</p>'}</section>`:""}</article></div>`}
function attentionAdminView(){
 ...
 ${(t.applicationDepositAttention||[]).map(x=>`<div class="card"><b>Application deposit to verify · ${esc(x.client_name)}</b><p>M-Pesa Ref ${esc(x.manual_payment_code)} · ${money(x.amount||1000)}</p><div class="actions"><button class="primary compact-button" onclick="verifyApplicationDeposit(${x.id},true)">Verify payment</button><button class="danger compact-button" onclick="verifyApplicationDeposit(${x.id},false)">Reject</button></div></div>`).join('')}
 ${newClients.map(n=>`<div class="card"><b>New client · ${esc(n.client_name||'')}</b><p>${esc(n.message)}</p><div class="actions"><button class="secondary" onclick="openClientRecordFromAttention(${n.user_id})">Open application</button><button class="secondary compact-button" onclick="resolveTrainerNotification(${n.id})">Mark seen</button></div></div>`).join('')}
