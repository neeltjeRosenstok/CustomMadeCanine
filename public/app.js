...
async function decideRefund(id,decision){
 ...
 if(credit){if(!await appConfirm(`${full?"Full":"Partial"} client credit of ${money(amount)} will be added to the client's account. No refund payment will be made.`,"Add client credit"))return;try{const d=await api(`/api/trainer/bookings/${id}/refund`,{method:"POST",body:JSON.stringify({decision,amount})});state.trainer=await api("/api/trainer/summary");state.trainerBooking=null;render();await appAlert(`${full?"Full":"Partial"} client credit recorded: ${money(d.amount||amount)}.`)}catch(e){appAlert(e.message)}return}
 const confirmationCode=await appPrompt("Enter the M-Pesa refund/transaction confirmation code:","",full?"Full refund":"Partial refund");if(confirmationCode===null)return;
 try{await api(`/api/trainer/bookings/${id}/refund`,{method:"POST",body:JSON.stringify({decision,amount,confirmationCode})});state.trainer=await api("/api/trainer/summary");state.trainerBooking=null;render();await appAlert(`${full?"Full":"Partial"} refund recorded: ${money(amount)} · M-Pesa ${confirmationCode}`)}catch(e){appAlert(e.message)}
}
...
function bookingIsInactive(x,type){const today=nairobiDateKeyClient(0);if(type==="class")return ["cancelled_by_client","rejected","expired"].includes(x.enrolment_status)||String(x.end_date||"")<today;return ["cancelled","expired"].includes(x.status)||String(x.start_at||"").slice(0,10)<today}
function bookingNeedsAttention(x,type){if(type==="class")return x.payment_status==="pending"||x.payment_status==="refund_pending";return ["provisional","pending_payment"].includes(x.status)||x.payment_status==="pending"||x.payment_status==="refund_pending"||x.manual_payment_status==="submitted"}
function bookingsView(b){
 ...
 for(const p of packages){const sessions=privateRows.filter(x=>Number(x.package_id)===Number(p.id)).sort((x,y)=>String(x.start_at).localeCompare(String(y.start_at)));if(!sessions.length)continue;const inactive=sessions.every(x=>bookingIsInactive(x,"private"))||["cancelled","expired"].includes(p.status),awaiting=sessions.some(x=>bookingNeedsAttention(x,"private"))||p.payment_status==="pending",declined=p.status==="cancelled"&&sessions.every(x=>x.status==="cancelled");if((declined||awaiting)&&(!inactive||showAll||awaiting)){grouped.add(Number(p.id));cards.push({priority:awaiting?0:inactive?2:1,date:sessions[0]?.start_at||p.created_at||"",html:`<div class="card ${inactive?"inactive-booking":""} ${awaiting?"provisional-card":""}"><h3>${awaiting?"Action required · ":""}${esc(p.name||"Private training package")} · ${esc(p.pet_name||sessions[0]?.pet_name||"Dog")}</h3><p>${sessions.length} appointments · ${money(p.package_price||0)} total</p><details><summary>Show session dates</summary><div class="compact-list">${sessions.map(y=>`<div>${fmt(y.start_at)} · ${y.location_type==="home"?"Home visit":arenaClientLabel()}</div>`).join("")}</div></details>${declined?`<div class="notice">Package declined. All held appointment times were released.</div>`:pendingBookingActions(sessions[0])}</div>`})}}
 for(const x of privateRows){if(x.package_id&&grouped.has(Number(x.package_id)))continue;const inactive=bookingIsInactive(x,"private"),awaiting=bookingNeedsAttention(x,"private");if(inactive&&!showAll&&!awaiting)continue;cards.push({priority:awaiting?0:inactive?2:1,date:x.start_at||"",html:`<div class="card ${inactive?"inactive-booking":""} ${['provisional','pending_payment'].includes(x.status)?'provisional-card':''}"><h3>${awaiting?"Action required · ":""}${x.package_name?`${esc(x.package_name)} · `:""}Private training · ${esc(x.pet_name||'Dog')}</h3><p>${fmt(x.start_at)} · ${x.location_type==='home'?'Home visit':arenaClientLabel()}</p><p>${esc(x.booking_ref)} · ${x.status==='expired'?'Expired':x.payment_status==='pending'?'Awaiting payment':esc(x.payment_status)}${x.status==='cancelled'?' · Cancelled':''}</p>${pendingBookingActions(x)}${x.manual_payment_status==='submitted'?`<div class="notice">Awaiting Amy payment verification.</div>`:""}${["refund_partial","refunded"].includes(x.payment_status)?`<div class="notice good">${x.payment_status==="refund_partial"?"Partial refund":"Refund"}: ${money(x.refund_amount||0)}${x.refund_confirmation_code?` · M-Pesa ${esc(x.refund_confirmation_code)}`:""}</div>`:""}${["credit_partial","credited"].includes(x.payment_status)?`<div class="notice good">${x.payment_status==="credit_partial"?"Partial client credit":"Client credit"}: ${money(x.credit_amount||0)} added to your account.</div>`:""}${bookingRescheduleNote(b,x.id)}${['paid','demo_paid'].includes(x.payment_status)&&x.status!=='cancelled'?`<div class="actions"><button class="secondary compact-button" onclick="addPrivateCalendarByRef('${esc(x.booking_ref)}')">＋ Add to calendar</button><button class="secondary compact-button" onclick="clientReschedule(${x.id})">Request reschedule · ${Math.max(0,3-Number(x.reschedule_count||0))} left</button><button class="danger compact-button" onclick="clientCancel(${x.id})">Cancel</button></div>`:''}</div>`})}
 for(const x of classRows){const inactive=bookingIsInactive(x,"class"),awaiting=bookingNeedsAttention(x,"class");if(inactive&&!showAll&&!awaiting)continue;cards.push({priority:awaiting?0:inactive?2:1,date:x.start_date||"",html:`<div class="card ${inactive?"inactive-booking":""}"><h3>${awaiting?"Action required · ":""}${esc(x.title)} · ${esc(x.pet_name||'Dog')}</h3><p>${displayDate(x.start_date,{day:'numeric',month:'short',year:'numeric'})}–${displayDate(x.end_date,{day:'numeric',month:'short',year:'numeric'})}</p><p>${esc(x.booking_ref)} · ${esc(x.payment_status)}</p>${x.payment_status==='pending'?`<div class="notice">Awaiting payment${x.hold_expires_at?` — place held until ${fmt(x.hold_expires_at)}`:''}.</div>`:''}${["refund_partial","refunded"].includes(x.payment_status)?`<div class="notice good">${x.payment_status==="refund_partial"?"Partial refund":"Refund"}: ${money(x.refund_amount||0)}</div>`:""}${x.enrolment_status==='active'&&['paid','demo_paid'].includes(x.payment_status)?`<div class="actions"><button class="secondary compact-button" onclick="addClassCalendarByRef('${esc(x.booking_ref)}')">＋ Add all classes</button><button class="danger compact-button" onclick="clientCancelClass(${x.id})">Cancel class place</button></div>`:''}</div>`})}
