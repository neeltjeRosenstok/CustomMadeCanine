...
function paymentView(){
 ...
 const paybill=`<div class="manual-payment-box paybill-payment-box"><h3>Pay with M-Pesa PayBill</h3><p>Pay manually, then enter the 10-character M-Pesa confirmation code below. Amy will confirm the payment.</p><div class="paybill-quick-grid"><div class="paybill-qr"><img src="/mpesa-paybill-qr.png" alt="M-Pesa PayBill QR code for The Custom Made Canine"><small>Scan the QR code with a compatible M-Pesa/Safaricom scanner to fill the payment details.</small></div><div class="paybill-details"><div><span>PayBill</span><strong>542542</strong></div><div><span>Account</span><strong>727777</strong></div><div><span>Business</span><strong>The Custom Made Canine</strong><small>Amy L Rapp</small></div><div><span>Amount due</span><strong>${money(amount)}</strong></div><a class="button secondary paybill-app-link" href="https://oneapp.safaricom.co.ke/" target="_blank" rel="noopener">Open Safaricom One App</a><small>On supported devices this may open Safaricom's app/web experience. Choose <b>Pay Bill</b> and use the details above.</small></div></div><details class="paybill-instructions" open><summary>Step-by-step M-Pesa PayBill instructions</summary><ol class="manual-payment-steps"><li>Open the <b>M-Pesa / Safaricom One</b> app and choose <b>Transact</b>, then <b>Pay Bill</b>.</li><li>Enter PayBill number <b>542542</b>.</li><li>Enter account number <b>727777</b>.</li><li>Enter <b>${money(amount)}</b>.</li><li>Continue and check the payment details carefully. The account is for <b>The Custom Made Canine (Amy L Rapp)</b>.</li><li>Confirm the payment and enter your M-Pesa PIN.</li><li>Wait for the success screen or M-Pesa confirmation SMS.</li><li>Find the <b>10-character M-Pesa transaction code</b> in that confirmation and enter it below.</li><li>Submit the code. The booking will show <b>Awaiting confirmation</b> until Amy confirms receipt.</li></ol></details><label>M-Pesa confirmation code<input id="manualMpesaCode" maxlength="10" value="${esc(state.manualMpesaDraft||'')}" autocomplete="off" autocapitalize="characters" placeholder="e.g. ABC123DEFG" oninput="this.value=this.value.replace(/[^a-z0-9]/gi,'').toUpperCase().slice(0,10);state.manualMpesaDraft=this.value"></label><p class="small">Exactly 10 letters/numbers. Spaces and punctuation are removed.</p><button class="primary manual-submit-button" onclick="submitManualPaymentFromPayment()">Submit payment confirmation</button><p class="small"><b>Awaiting confirmation by Amy.</b> Your booking has been recorded after you submit the code.</p></div>`;
