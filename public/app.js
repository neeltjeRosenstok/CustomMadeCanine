...
function about(){
 return `<section class="screen meet-amy-screen"><button class="back" onclick="go('home')">← Home</button><div class="center"><div class="meet-amy-content">
  ...
  <div class="amy-bio"><p>Amy is a qualified companion dog trainer specializing in Force Free Behaviour Modification and Socialization. She is an experienced, resourceful, and professional trainer who has been dedicated to improving the lives of dogs and their owners in Kenya since 2006.</p>
  ...
  <p>Amy’s philosophy focuses on positive, humane methods. Instead of just training the dog, she teaches owners how to communicate clearly with their pets to ensure lasting good behaviour.</p></div>
...
function bookingsHub(b){const rr=(b?.rescheduleRequests||[]).filter(r=>['approved','declined'].includes(r.status)&&r.trainer_note);return `<div class="bookings-panel"><h3>My bookings</h3>${rr.map(r=>`<div class="notice">Reschedule ${esc(r.status)} · ${esc(r.booking_ref)}<br>Amy's note: ${esc(r.trainer_note)}</div>`).join('')}<div class="actions client-booking-actions"><button class="secondary" onclick="startPrivate('portal')">Book private training</button><button class="secondary" onclick="openPortalClasses()">View classes</button></div>${bookingsView(b)}</div>`}
...
function bookingsView(b){const privateRows=b?.privateBookings||[],classRows=b?.classBookings||[],packages=b?.packages||[],pendingIds=new Set(packages.filter(p=>p.status==='provisional'&&p.payment_status==='pending').map(p=>Number(p.id))),pkgCards=packages.filter(p=>p.status==='provisional'&&p.payment_status==='pending').map(p=>{const sessions=privateRows.filter(x=>Number(x.package_id)===Number(p.id)),first=sessions[0],manual=p.manual_payment_status==='submitted';return `<div class="card provisional-card"><h3>Action required · ${esc(p.name)}</h3><p>${esc(p.pet_name||'Dog')} · ${sessions.length} sessions · ${money(p.package_price)} total</p><div class="package-session-list">${sessions.map(x=>`<div>${fmt(x.start_at)} · ${x.location_type==='home'?'Home visit':"Amy's Arena in Ridgeways"}</div>`).join('')}</div>${manual?`<div class="notice good">Awaiting Amy payment verification. Please WhatsApp Amy to make sure she sees this.</div>`:`<div class="actions"><button class="primary compact-button" onclick="resumePackagePayment(${p.id})">Confirm & Pay</button><button class="secondary compact-button" onclick="manualPaymentForPackage(${p.id},${Number(p.package_price||0)})">M-Pesa Send Money</button><button class="danger compact-button" onclick="cancelPendingBooking(${first?.id||0})">Cancel package</button></div>`}</div>`}).join(''),visible=privateRows.filter(x=>!pendingIds.has(Number(x.package_id)));return `<div class="list">${pkgCards}${visible.map(x=>`<div class="card"><h3>Private training · ${esc(x.pet_name||'Dog')}</h3><p>${fmt(x.start_at)} · ${x.location_type==='home'?'Home visit':"Amy's Arena in Ridgeways"}</p><p>${esc(x.booking_ref)} · ${x.payment_status==='pending'?'Awaiting payment':esc(x.payment_status)}${x.status==='cancelled'?' · Cancelled':''}</p>${x.status==='provisional'?`<div class="actions"><button class="primary compact-button" onclick="acceptProvisional(${x.id})">Confirm & Pay</button><button class="secondary compact-button" onclick="declineProvisional(${x.id})">Decline</button></div>`:''}${x.payment_status==='pending'&&x.status!=='provisional'&&x.status!=='expired'?pendingBookingActions(x):''}${['paid','demo_paid'].includes(x.payment_status)&&x.status!=='cancelled'?`<div class="actions"><button class="secondary compact-button" onclick="addPrivateCalendarByRef('${esc(x.booking_ref)}')">＋ Add to calendar</button><button class="secondary compact-button" onclick="clientReschedule(${x.id})">Reschedule</button><button class="danger compact-button" onclick="clientCancel(${x.id})">Cancel</button></div>`:''}</div>`).join('')}${classRows.map(x=>`<div class="card"><h3>${esc(x.title)} · ${esc(x.pet_name||'Dog')}</h3><p>${displayDate(x.start_date,{day:'numeric',month:'short',year:'numeric'})}–${displayDate(x.end_date,{day:'numeric',month:'short',year:'numeric'})}</p><p>${esc(x.booking_ref)} · ${x.enrolment_status==='rejected'||x.enrolment_status==='cancelled_by_client'?'Cancelled':x.manual_payment_status==='submitted'?'Awaiting Amy payment verification':x.payment_status==='pending'?'Awaiting payment':esc(x.payment_status)}</p>${(x.enrolment_status==='rejected'||x.enrolment_status==='cancelled_by_client')?`<div class="notice">${x.rejected_reason?`Amy's note: ${esc(x.rejected_reason)}`:x.payment_status==='cancelled'?'Your class place has been cancelled and released. No payment was taken.':'Your class place has been cancelled.'}</div>`:''}${x.enrolment_status==='active'&&x.payment_status==='pending'?(x.manual_payment_status==='submitted'?`<div class="notice good">Payment confirmation submitted. Please WhatsApp Amy to make sure she sees this.</div>`:`<div class="actions"><button class="primary compact-button" onclick="resumeClassPayment(${x.id},'${esc(x.booking_ref)}',${Number(x.price||0)})">Resume payment</button><button class="secondary compact-button" onclick="manualPaymentForClass(${x.id},${Number(x.price||0)})">M-Pesa Send Money</button><button class="danger compact-button" onclick="clientCancelClass(${x.id})">Cancel</button></div>`):''}${x.enrolment_status==='active'&&['paid','demo_paid'].includes(x.payment_status)?`<div class="actions"><button class="secondary compact-button" onclick="addClassCalendarByRef('${esc(x.booking_ref)}')">＋ Add all classes</button><button class="danger compact-button" onclick="clientCancelClass(${x.id})">Cancel class place</button></div>`:''}</div>`).join('')}</div>`}
...
function monthCalendarView(){const selected=parseDateKey(state.trainerMonthDate)||new Date(),first=monthStart(selected),last=monthEnd(selected),cal=state.trainerMonthCalendar||{bookings:[],classSessions:[],blocks:[],scheduleBlocks:[]},lead=(first.getDay()+6)%7,days=last.getDate(),cells=[];for(let i=0;i<lead;i++)cells.push(`<span class="month-empty"></span>`);const part=(blocks,target,am)=>blocks.some(b=>b.target===target&&(b.all_day||(am?(!b.start_time||b.start_time<'13:00'):(!b.end_time||b.end_time>'13:00'))));for(let day=1;day<=days;day++){const d=new Date(first.getFullYear(),first.getMonth(),day,12),key=dateKey(d),dbs=cal.bookings.filter(x=>String(x.start_at).slice(0,10)===key),arena=dbs.filter(x=>x.location_type==='arena').length,home=dbs.filter(x=>x.location_type==='home').length,hasClass=cal.classSessions.some(x=>x.session_date===key),sb=(cal.scheduleBlocks||[]).filter(b=>key>=b.start_date&&key<=b.end_date),classes=[];if(sb.some(b=>b.target==='amy'&&b.all_day))classes.push('full-unavailable');else{if(part(sb,'arena',true))classes.push('availability-am-arena');if(part(sb,'arena',false))classes.push('availability-pm-arena');if(part(sb,'home',true))classes.push('availability-am-home');if(part(sb,'home',false))classes.push('availability-pm-home')}cells.push(`<button class="month-day ${classes.join(' ')} ${key===state.trainerSelectedDate?'selected':''}" onclick="selectMonthDate('${key}')"><span>${day}</span><span class="month-dots">${arena?`<i class="dot arena-booking-dot">${arena}</i>`:''}${home?`<i class="dot home-booking-dot">${home}</i>`:''}${hasClass?'<i class="dot class-dot"></i>':''}</span></button>`)}return `<div class="month-calendar"><div class="calendar-legend"><span><i class="legend-swatch arena-block"></i>Arena unavailable</span><span><i class="legend-swatch home-block"></i>Home visits unavailable</span><span><i class="legend-swatch unavailable"></i>Amy unavailable</span><span><i class="dot arena-booking-dot"></i>Arena booking</span><span><i class="dot home-booking-dot"></i>Home visit</span><span><i class="dot class-dot"></i>Class</span></div><div class="calendar-head"><button class="secondary compact-button" onclick="moveTrainerMonth(-1)">←</button><span>${first.toLocaleDateString('en-KE',{month:'long',year:'numeric'})}</span><button class="secondary compact-button" onclick="moveTrainerMonth(1)">→</button></div><div class="month-weekdays">${['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map(x=>`<span>${x}</span>`).join('')}</div><div class="month-grid">${cells.join('')}</div></div>`}
...
function trainerView(){
 ...
 return `<section class="screen trainer-screen">
  ...
  <div class="dashboard-taskbar dashboard-six"><button class="task-button task-book" onclick="openBookForClientPicker()"><span>Book for client</span><span>＋</span></button><button class="task-button task-resources" onclick="openResourceLibrary()"><span>Training resources</span><span>→</span></button><button class="task-button task-attention" onclick="openTrainerAdmin('attention')"><span>Needs attention</span><b>${attentionCount+pendingCount}</b></button><button class="task-button task-scheduling" onclick="openScheduling()"><span>Scheduling</span><span>→</span></button><button class="task-button task-clients" onclick="openTrainerAdmin('clients')"><span>Clients</span><b>${clientCount||'→'}</b></button><button class="task-button task-history" onclick="openTrainerAdmin('activity')"><span>Activity history</span><span>→</span></button></div>
...
async function trainerClientBookingCheckTimes(){
 ...
 try{
  ...
  let msg="There are no available appointment times on this date.";
  try{
...
function attentionAdminView(){const t=state.trainer||{},none=!(t.manualPaymentAttention||[]).length&&!(t.classManualPaymentAttention||[]).length&&!(t.cancellationAttention||[]).length&&!(t.classRefundAttention||[]).length&&!(t.rescheduleAttention||[]).length&&!(t.classPaymentAttention||[]).length&&!(t.vaccinationAttention||[]).length&&!(t.notifications||[]).length&&!(t.pendingReviews||[]).length;return `<section class="screen admin-screen"><button class="back-dashboard" onclick="dashboardBack()">← Back to Dashboard</button><div class="admin-head"><div><div class="eyebrow">Amy's workspace</div><h2>Needs attention</h2></div><button class="secondary compact-button" onclick="openTrainerAdmin('reviews')">Reviews · ${(t.pendingReviews||[]).length}</button></div><div class="admin-list-large">${(t.manualPaymentAttention||[]).map(x=>`<div class="card"><b>Manual M-Pesa to verify · ${esc(x.client_name)}</b><p>${esc(x.pet_name||'')} · ${esc(x.booking_ref)} · ${esc(x.manual_payment_code)} · ${money(x.manual_payment_amount)}</p><button class="primary compact-button" onclick="verifyManualPayment(${x.id},true)">Verify payment</button></div>`).join('')}${(t.classManualPaymentAttention||[]).map(x=>`<div class="card"><b>Class M-Pesa to verify · ${esc(x.client_name)}</b><p>${esc(x.pet_name||'')} · ${esc(x.class_title)} · ${esc(x.manual_payment_code)} · ${money(x.manual_payment_amount)}</p><button class="primary compact-button" onclick="verifyClassManualPayment(${x.id},true)">Verify payment</button></div>`).join('')}${(t.cancellationAttention||[]).map(x=>`<div class="card"><b>Cancellation / refund · ${esc(x.booking_ref)}</b><p>${esc(x.client_name)} · ${esc(x.pet_name||'')}</p><button class="secondary" onclick="openTrainerBookingById(${x.id})">Handle refund</button></div>`).join('')}${(t.classRefundAttention||[]).map(x=>`<div class="card"><b>Class cancellation / refund · ${esc(x.booking_ref)}</b><p>${esc(x.client_name)} · ${esc(x.pet_name||'')}</p><button class="secondary" onclick="decideClassRefund(${x.id})">Handle refund</button></div>`).join('')}${(t.rescheduleAttention||[]).map(x=>`<div class="card"><b>Reschedule request · ${esc(x.client_name)} · ${esc(x.pet_name||'')}</b><p>Current: ${fmt(x.old_start_at)}<br>Requested: ${fmt(x.proposed_start_at)}${x.client_note?`<br>Client note: ${esc(x.client_note)}`:''}</p><button class="primary compact-button" onclick="decideRescheduleRequest(${x.id},'approve')">Approve</button> <button class="danger compact-button" onclick="decideRescheduleRequest(${x.id},'decline')">Decline</button></div>`).join('')}${(t.classPaymentAttention||[]).map(x=>`<div class="card"><b>Class payment incomplete · ${esc(x.client_name)}</b><p>${esc(x.pet_name||'')} · ${esc(x.class_title)} · held until ${fmt(x.hold_expires_at)}</p></div>`).join('')}${(t.vaccinationAttention||[]).map(x=>`<div class="card"><span>Vaccination · ${esc(x.pet_name)}</span><p>${esc(x.client_name)}</p><button class="secondary" onclick="openVaccinationReview(${x.pet_id})">Review record</button></div>`).join('')}${(t.notifications||[]).filter(n=>n.kind==='new_account').map(n=>`<div class="card"><b>New client · ${esc(n.client_name||'')}</b><p>${esc(n.message)}</p><button class="secondary compact-button" onclick="resolveTrainerNotification(${n.id})">Mark seen</button></div>`).join('')}${none?'<p class="small">Nothing needs attention.</p>':''}</div></section>`}
async function verifyClassManualPayment(id,approve){if(!confirm(approve?'Confirm that this class payment has been received?':'Reject this class payment confirmation?'))return;await api(`/api/trainer/class-enrolments/${id}/manual-payment`,{method:'POST',body:JSON.stringify({approve})});state.trainer=await api('/api/trainer/summary');render()}
async function openTrainerBookingById(id){try{state.trainerBooking=await api(`/api/trainer/bookings/${id}`);state.trainerClient=await api(`/api/trainer/client/${state.trainerBooking.user_id}`);render()}catch(e){alert(e.message)}}
...
function formatReportDate(value){
 const str=String(value||''); const m=str.match(/^(\d{4})-(\d{2})-(\d{2})(?:[T\s](\d{2}):(\d{2}))?/); if(!m)return str;
 const months=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']; const d=`${Number(m[3])} ${months[Number(m[2])-1]} ${m[1]}`; return m[4]?`${d}, ${m[4]}:${m[5]}`:d;
}
