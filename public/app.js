...
function shell(content){
  const logged=!!state.user,first=state.user?.first_name||String(state.user?.name||"").trim().split(/\s+/)[0]||"Client";
  return `<div class="app-shell">
    <header class="topbar">
      ...
      <button class="header-login-btn" aria-haspopup="menu" aria-expanded="${state.menu?'true':'false'}" onclick="${logged?'toggleMenu()':"auth('Please sign in to access your Client Portal.')"}">${logged?`Welcome, ${esc(first)}`:'LOGIN'}</button>
    </header>
    ...
    ${appDialogView()}
    ${state.menu&&logged?`<nav class="menu account-menu" aria-label="Account menu">${state.user?.role==='trainer'?`<button onclick="go('trainer')">Amy's Workspace</button>`:`<button onclick="portal()">Client Portal</button>`}<button onclick="go('account')">Account & security</button><button onclick="logout()">Log out</button></nav>`:""}
  </div>`;
}
function toggleMenu(){state.menu=!state.menu;render()}
function go(v){if(v==='classes'&&(!state.user||state.user.role!=='client'||String(state.user.client_status||'current')!=='current')){if(!state.user)return auth('Please sign in to view and book classes.');state.portalTab='bookings';state.view='portal';state.menu=false;return render()}state.view=v;state.menu=false;render();if(v==='payment'){requestAnimationFrame(()=>window.scrollTo({top:0,left:0,behavior:'auto'}));}}
...
function startHomepageSignup(){
 ...
 state.applicationDraft=state.applicationDraft||{name:"",firstName:"",lastName:"",email:"",whatsappPhone:"",mpesaPhone:"",newsletterOptIn:false,location:"",googleMapsLocation:"",introNote:"",dogName:"",dogBreed:"",dogGender:"",dogDob:"",householdDogs:"",householdAdults:"",children0to8:"",children9to13:"",children14plus:"",householdChanges:"",householdNote:"",password:"",confirmPassword:""};
 go("application");
}
...
async function refreshClientOnFocus(){
 ...
 try{
   state.profile=await api("/api/my/profile");state.user={...state.user,...(state.profile?.user||{})};
   state.bookings=await api("/api/my/bookings");
...
function portalView(){
 const p=state.profile,b=state.bookings,r=state.resources,tab=state.portalTab||"dogs",portalUser=p?.user||state.user||{};
 const body=tab==="media"?resourceList(r):tab==="bookings"?bookingsHub(b):tab==="messages"?clientTrainingNotesView():petsView(p.pets);
 const applicant=String(portalUser.client_status||"current")!=="current";
 return `<section class="screen client-dashboard"><div class="eyebrow">Client Portal</div><h2>Welcome, ${esc(portalUser.first_name||String(portalUser.name||"Client").split(" ")[0])}</h2>
 ${String(portalUser.client_status||"current")==="rejected"?`<div class="notice bad rejection-client-banner"><b>Dear ${esc(portalUser.first_name||String(portalUser.name||"Client").split(" ")[0])},</b><br>Amy has reviewed your application and decided not to accept you as a client.${portalUser.rejection_note?`<br><br><b>Reason:</b> ${esc(portalUser.rejection_note)}`:""}<br><br>Please respect this decision.${Number(state.profile?.applicationDeposit?.refund_amount||0)>0?` Your KES 700 will be refunded shortly.`:""}</div>`:applicant?`<div class="notice application-status-banner"><b>New Client Application</b><br>Your account is open for profile and dog information. Booking is locked until Amy approves your application.</div>`:""}
 <div class="portal-menu portal-menu-primary portal-menu-four"><button class="${tab==="dogs"?"active":""}" onclick="openPortalTab('dogs')"><span>Dogs</span><small>Profiles & vaccination</small></button><button class="${tab==="bookings"?"active":""}" onclick="openPortalTab('bookings')"><span>Bookings</span><small>Bookings & new training</small></button><button class="${tab==="messages"?"active":""}" onclick="openPortalTab('messages')"><span>Messages & notes</span><small>From Amy</small></button><button class="${tab==="media"?"active":""}" onclick="openPortalTab('media')"><span>Resources</span><small>Training material</small></button></div>
...
function clientTrainingNotesView(){
 const all=state.trainingNotes||[],cutoff=Date.now()-14*24*60*60*1000; const recent=all.filter(x=>{const d=new Date(String(x.created_at||x.shared_at||x.booking_start_at||''));return Number.isNaN(d.getTime())||d.getTime()>=cutoff}); const rows=state.messageHistoryExpanded?all:recent,olderCount=Math.max(0,all.length-recent.length); return `<div class="client-messages"><div class="section-title-actions"><div><h3>Messages & Training Notes</h3><p class="small">Newest messages are shown first.</p></div><button class="primary compact-button" onclick="replyToClientMessage('general',0)">Message</button></div>${rows.length?rows.map(x=>`<article class="client-message-entry"><div class="client-message-meta"><b>${esc(x.sender||'Amy')} · ${esc(formatNairobiActivityTime(x.created_at||x.shared_at||x.booking_start_at))}${x.pet_name?` · ${esc(x.pet_name)}`:''}${x.booking_service?` · Private training`:x.item_type==='resource'?` · Shared ${String(x.resource_type||'resource').toUpperCase()} — ${esc(x.resource_title||'Training resource')}`:''}</b></div><div class="client-message-card">${esc(x.note||x.body||'')}</div></article>`).join(''):'<div class="notice">Amy has not shared any messages or training notes yet.</div>'}${olderCount&&!state.messageHistoryExpanded?`<div class="actions"><button class="secondary compact-button" onclick="state.messageHistoryExpanded=true;render()">Load more · ${olderCount}</button></div>`:state.messageHistoryExpanded&&olderCount?`<div class="actions"><button class="secondary compact-button" onclick="state.messageHistoryExpanded=false;render()">Show last 2 weeks</button></div>`:''}</div>`;
}
async function replyToClientMessage(type,id){const body=await appPrompt('Reply to Amy:','');if(body===null||!String(body).trim())return;try{await api('/api/my/messages',{method:'POST',body:JSON.stringify({replyToType:type,replyToId:id,body:String(body).trim()})});state.trainingNotes=await api('/api/my/training-notes');render();}catch(e){appAlert(e.message)}}
...
function trainerView(){
 ...
 const attentionCount=['vaccinationAttention','cancellationAttention','classRefundAttention','rescheduleAttention','manualPaymentAttention','classManualPaymentAttention','applicationDepositAttention','newBookingAttention','pendingReviews'].reduce((n,k)=>n+(Array.isArray(t[k])?t[k].length:0),0);
...
async function openClientRecord(id){state.clientRecord=await api(`/api/trainer/client/${id}`);document.body.classList.add('modal-open');render()}
function closeClientRecord(){state.clientRecord=null;document.body.classList.remove('modal-open');render()}
...
function vaccinationReviewModal(){const d=state.vaccinationReview;if(!d)return '';const files=d.files||[];return `<div class="modal-overlay"><div class="trainer-modal vaccination-review-modal"><button class="close-btn modal-close" aria-label="Close vaccination record" onclick="closeVaccinationReview()">×</button><div class="eyebrow">Vaccination record</div><h2>${esc(d.name||'Dog')}</h2>${files.length?`<p class="small">Look through the uploaded passport page(s) before choosing Verify.</p><div class="vaccination-pages">${files.map((f,i)=>String(f.mime_type||'').toLowerCase()==='application/pdf'?`<figure class="vaccination-pdf"><div class="pdf-file-card"><strong>${esc(f.original_name||`Vaccination PDF ${i+1}`)}</strong><a class="secondary compact-button" href="${esc(f.url)}" target="_blank" rel="noopener">Open PDF ↗</a></div><figcaption>Document ${i+1}</figcaption></figure>`:`<figure><a href="${esc(f.url)}" target="_blank" rel="noopener"><img src="${esc(f.url)}" alt="Vaccination passport page ${i+1}"></a><figcaption>Page ${i+1} · ${esc(f.original_name||'uploaded image')}</figcaption></figure>`).join('')}</div>`:`<div class="notice">No image or document has been uploaded. Only use Verify if you have physically seen the vaccination record.</div>`}<div class="actions">${d.vaccination_status==='verified'?`<button class="secondary" onclick="setVaccinationStatus(${d.id},'pending')">Undo verification</button>`:`<button class="primary" onclick="setVaccinationStatus(${d.id},'verified')">Verify record</button><button class="danger" onclick="setVaccinationStatus(${d.id},'rejected')">Reject / request replacement</button>`}<button class="secondary" onclick="closeVaccinationReview()">Close</button></div></div></div>`}
...
function filterClientAdmin(q){q=String(q||'').trim().toLowerCase();const needsFull=q&&(state.clientStatusFilter!=='all'||state.clientActivityFilter!=='all');state.clientSearch=q;if(needsFull){state.clientStatusFilter='all';state.clientActivityFilter='all';render();requestAnimationFrame(()=>{const el=document.querySelector('.client-inline-search');if(el){el.focus();el.setSelectionRange(el.value.length,el.value.length)}});return}document.querySelectorAll('.client-overview-card').forEach(el=>{el.style.display=!q||String(el.dataset.search||'').includes(q)?'':'none'})}
...
async function openTrainerBooking(id){
 ...
 const fromNew=(state.trainer?.newBookingAttention||[]).find(x=>Number(x.booking_id)===Number(id));
 const raw=fromAttention||fromCalendar||fromNew;if(!raw)return appAlert("This appointment could not be opened.");
 const b=(fromAttention||fromNew)?{...fromCalendar,...raw,id:Number(raw.id||raw.booking_id),client:raw.client_name||fromCalendar?.client,user_id:raw.user_id||fromCalendar?.user_id,pet_id:raw.pet_id||fromCalendar?.pet_id}:raw;
...
function applicationDepositCopy(){
 return `<div class="application-deposit-copy"><h3>You are starting a New Client Application</h3><p>Amy asks that you pay <b>KES 1,000</b> to start a client account as this initial phase takes a lot of her time. Of this <b>KES 1,000</b>, only <b>KES 300</b> will be non-refundable (administration fee).</p><p>The remaining <b>KES 700</b> is held pending Amy’s decision:</p><ul><li>if your application is approved, <b>KES 700</b> is added to your account as a credit which can be spent on her services</li><li>if your application is not approved, <b>KES 700</b> is refunded</li></ul><p>Paying the application deposit does not guarantee approval or a booking.</p><p class="application-vaccination-reminder">To make your sign-up quick and easy, please have your dog's <b>vaccination record handy</b>. You can upload it as a document or a photo during the process.</p><p class="application-duration"><b>The sign up takes roughly 5 minutes to complete.</b></p></div>`;
}
...
function applicationSaveCurrentStep(){
 ...
 if(step===1){applicationSaveFields({firstName:"appFirstName",lastName:"appLastName",email:"appEmail",whatsappPhone:"appWhatsapp",mpesaPhone:"appMpesa",newsletterOptIn:"appNewsletter",location:"appLocation",googleMapsLocation:"appGoogleMapsLocation",introNote:"appIntro"});const d=state.applicationDraft||{};d.name=[d.firstName,d.lastName].filter(Boolean).join(" ").trim();state.applicationDraft=d;}
...
async function useCurrentApplicationLocation(btn){
 if(!navigator.geolocation)return appAlert("Location access is not available on this device. You can paste a Google Maps link instead.");
 if(btn){btn.disabled=true;btn.textContent="Finding location…"}
 navigator.geolocation.getCurrentPosition(pos=>{const value=`https://www.google.com/maps?q=${pos.coords.latitude.toFixed(6)},${pos.coords.longitude.toFixed(6)}`;const input=document.getElementById("appGoogleMapsLocation");if(input)input.value=value;state.applicationDraft=state.applicationDraft||{};state.applicationDraft.googleMapsLocation=value;if(btn){btn.disabled=false;btn.textContent="Use my current location"}},err=>{if(btn){btn.disabled=false;btn.textContent="Use my current location"}appAlert(err.code===1?"Location permission was not granted. You can paste a Google Maps link instead.":"Your current location could not be read. You can paste a Google Maps link instead.")},{enableHighAccuracy:true,timeout:12000,maximumAge:60000});
}
async function applicationStepOneContinue(){
 applicationSaveFields({firstName:"appFirstName",lastName:"appLastName",email:"appEmail",whatsappPhone:"appWhatsapp",mpesaPhone:"appMpesa",newsletterOptIn:"appNewsletter",location:"appLocation",googleMapsLocation:"appGoogleMapsLocation",introNote:"appIntro"});
 const d=state.applicationDraft;d.name=[d.firstName,d.lastName].filter(Boolean).join(" ").trim();
 if(!String(d.firstName||"").trim()||!String(d.lastName||"").trim()||!d.email.trim()||!d.whatsappPhone.trim()||!d.location.trim())return appAlert("Please complete your first name, last name, email, WhatsApp number and Area.");
...
async function submitNewClientApplication(){
 ...
 try{
  const body={name:d.name,firstName:d.firstName,lastName:d.lastName,email:d.email,whatsappPhone:d.whatsappPhone,mpesaPhone:d.mpesaPhone,password:d.password,newsletterOptIn:!!d.newsletterOptIn,applicationSignup:true,location:d.location,googleMapsLocation:d.googleMapsLocation,introNote:d.introNote,householdDogs:d.householdDogs,householdAdults:d.householdAdults,children0to8:d.children0to8,children9to13:d.children9to13,children14plus:d.children14plus,householdChanges:d.householdChanges,householdNote:d.householdNote};
...
function applicationView(){
 ...
 return `<section class="screen application-screen">${step<5?`<button class="back" tabindex="-1" onclick="applicationBack()">← ${step===1?"Home":"Back"}</button>`:""}<div class="center"><div class="application-shell">${step===1?applicationDepositCopy():""}${progress}${body}</div></div>${state.vaccinationInfoOpen?vaccinationRequirements():""}${state.parklandsReferralOpen?parklandsReferralModal():""}</section>`;
}

function authView(){
 const hasBooking=!!(state.selectedService||state.selectedClass),register=state.authMode==='register',draft=state.authDraft||{};
 return `<section class="screen"><button class="back" onclick="state.selectedService=null;state.selectedClass=null;go('home')">← Back to home</button><div class="center"><div class="panel auth-panel">
 <div class="eyebrow">PLEASE LOG IN</div><h2>Client Portal</h2>
 ${hasBooking&&!String(state.authMessage||'').startsWith('Your password has been reset')?`<div class="notice good saved-notice"><b>✓ Your booking choices are saved.</b><br>They will stay here while you log in or sign up.</div>`:''}
 ...
 ${state.authMode==='choice'?`<div class="auth-choice-grid">
   ...
   <button class="auth-choice" onclick="startHomepageSignup()"><strong>Sign up</strong><span>I'm new here</span></button>
...
function accountView(){const u=state.profile?.user||state.user||{};return `<section class="screen"><div class="center"><div class="panel account-panel" style="width:min(680px,100%)"><button class="back" onclick="go(state.user?.role==='trainer'?'trainer':'portal')">← Back</button><div class="eyebrow">Account & billing</div><h2>Your details</h2>${state.user?.role==='client'?`<div class="credit-balance-card"><span>Credit available</span><strong>${money(state.profile?.creditBalance||0)}</strong>${(state.profile?.creditHistory||[]).length?`<small>Recent credit activity is kept in your account history.</small>`:""}</div>`:""}${state.user?.role==='client'?`<div class="form-grid"><label>First name<input id="accountFirstName" value="${esc(u.first_name||'')}"></label><label>Last name<input id="accountLastName" value="${esc(u.last_name||'')}"></label></div><div class="form-grid"><label>WhatsApp number<input id="accountWhatsapp" value="${esc(u.whatsapp_phone||u.phone||'')}" autocomplete="tel" inputmode="tel" oninput="accountWhatsappInput(this)"><small id="accountWhatsappWarning" class="field-warning" hidden></small></label><label>M-Pesa number <span class="small">(if different)</span><input id="accountMpesa" value="${esc(u.mpesa_phone||u.phone||'')}" autocomplete="tel" inputmode="tel" oninput="accountMpesaInput(this)" onblur="accountMpesaInput(this)"><small id="accountMpesaWarning" class="field-warning" hidden></small></label></div><label>Area<input id="accountLocation" value="${esc(u.location||'')}" placeholder="Ridgeways, Loresho, Karen"></label><label>Google Maps location<input id="accountGoogleMaps" value="${esc(u.google_maps_location||'')}" placeholder="Paste a Google Maps pin/link"></label><label>KRA PIN <span class="small">(optional — for eTIMS invoices or receipts)</span><input id="accountKra" value="${esc(u.kra_pin||'')}"></label><label class="newsletter-optin"><input id="accountNewsletter" type="checkbox" ${u.newsletter_opt_in?'checked':''}> Please tick here if you would like to receive my newsletter, <em>The Canine Grapevine</em>, which I try to send out at least every 2 months, to keep you informed about upcoming classes, events and other news.</label><div class="actions"><button class="primary" onclick="saveAccountDetails()">Save account details</button></div><hr>`:''}<h3>Change password</h3><label>Current password<input id="currentPassword" type="password" autocapitalize="none" autocorrect="off" spellcheck="false"></label><label>New password<input id="newPassword" type="password" autocapitalize="none" autocorrect="off" spellcheck="false"></label><label>Confirm new password<input id="confirmPassword" type="password" autocapitalize="none" autocorrect="off" spellcheck="false"></label><div class="actions"><button class="secondary" onclick="changePassword()">Change password</button></div></div></div></section>`}
function accountWhatsappInput(input){const raw=String(input?.value||''),cleaned=raw.replace(/[^0-9+ ]/g,'').replace(/(?!^)\+/g,'');if(input&&input.value!==cleaned)input.value=cleaned;const warning=document.getElementById('accountWhatsappWarning'),digits=cleaned.replace(/\D/g,''),compact=cleaned.replace(/\s+/g,''),kenyanPrefix=/^(?:\+254|0[17])/.test(compact),kenyan=kenyanPrefix?formatKenyanMpesa(cleaned):'',ok=!!cleaned.trim()&&/^\+?[0-9 ]+$/.test(cleaned)&&digits.length>=6&&digits.length<=18&&(!kenyanPrefix||!!kenyan);if(kenyan&&input)input.value=kenyan;if(warning){warning.textContent=ok?'':kenyanPrefix?'Enter the complete Kenyan number, for example 0722 123456 or +254 700 123456.':'Use a complete international WhatsApp number with numbers, spaces and a leading +.';warning.hidden=ok}return ok}
function accountMpesaInput(input){const raw=String(input?.value||'');const cleaned=raw.replace(/[^0-9+ ]/g,'').replace(/(?!^)\+/g,'');if(input&&input.value!==cleaned)input.value=cleaned;const warning=document.getElementById('accountMpesaWarning');if(!cleaned.trim()){if(warning){warning.textContent='';warning.hidden=true}return true}const formatted=formatKenyanMpesa(cleaned),compact=cleaned.replace(/\s+/g,'');const plausible=/^(?:0[17]\d{0,8}|\+254[17]\d{0,8})$/.test(compact);const msg=plausible&&!formatted?'Complete the Kenyan M-Pesa number, e.g. 0722 123456 or +254 700 123456.':!formatted?'Use a Kenyan M-Pesa number beginning 07, 01, +254 7 or +254 1.':'';if(formatted&&input)input.value=formatted;if(warning){warning.textContent=msg;warning.hidden=!msg}return !!formatted}
async function saveAccountDetails(){const wa=document.getElementById('accountWhatsapp'),mp=document.getElementById('accountMpesa');if(!accountWhatsappInput(wa))return appAlert('Please enter a valid WhatsApp number.');if(!accountMpesaInput(mp))return appAlert('Please enter a valid Kenyan M-Pesa number.');try{const d=await api('/api/my/profile',{method:'PUT',body:JSON.stringify({firstName:document.getElementById('accountFirstName').value,lastName:document.getElementById('accountLastName').value,whatsappPhone:wa.value,mpesaPhone:mp.value,location:document.getElementById('accountLocation').value,googleMapsLocation:document.getElementById('accountGoogleMaps').value,kraPin:document.getElementById('accountKra').value,newsletterOptIn:document.getElementById('accountNewsletter').checked})});state.profile.user=d.user;state.user={...state.user,...d.user};appAlert('Account details saved.');render()}catch(e){appAlert(e.message)}}
...
function clientAdminView(){
 ...
 const filter=bookingMode?'current':(state.clientStatusFilter||'current'),activityFilter=bookingMode?'all':(state.clientActivityFilter||'all');
 const q=String(state.clientSearch||'').trim().toLowerCase();
 const searched=q?rows.filter(x=>[x.name,x.email,x.whatsapp_phone,x.mpesa_phone,...(x.pets||[]).map(p=>p.name)].join(' ').toLowerCase().includes(q)):null;
 const filtered=searched||rows.filter(x=>(filter==='all'||x.client_status===filter)&&(activityFilter==='all'||x.activity_status===activityFilter));
 ...
 const statusOrder=['current','unverified','incomplete','rejected','all'];
 return `<section class="screen admin-screen"><button class="back-dashboard" onclick="state.bookForClientMode=false;dashboardBack()">← Back to Dashboard</button><div class="admin-head"><div><div class="eyebrow">Amy's workspace</div><h2>${bookingMode?'Book for client':'Clients'}</h2>${bookingMode?'<p class="small">Only verified Current clients are shown.</p>':''}</div></div>${bookingMode?'':`<div class="client-filter-toolbar"><div class="client-status-filters"><button class="${filter==='current'?'primary':'secondary'} compact-button" onclick="state.clientSearch='';state.clientStatusFilter='current';render()">Current · ${rows.filter(x=>x.client_status==='current').length}</button><input class="admin-search client-inline-search" value="${esc(state.clientSearch||'')}" placeholder="Search client or dog" oninput="filterClientAdmin(this.value)"><button class="${filter==='unverified'?'primary':'secondary'} compact-button" onclick="state.clientSearch='';state.clientStatusFilter='unverified';render()">Unverified · ${rows.filter(x=>x.client_status==='unverified').length}</button><button class="${filter==='incomplete'?'primary':'secondary'} compact-button" onclick="state.clientSearch='';state.clientStatusFilter='incomplete';render()">Incomplete · ${rows.filter(x=>x.client_status==='incomplete').length}</button><button class="${filter==='rejected'?'primary':'secondary'} compact-button" onclick="state.clientSearch='';state.clientStatusFilter='rejected';render()">Rejected · ${rows.filter(x=>x.client_status==='rejected').length}</button><button class="${filter==='all'?'primary':'secondary'} compact-button" onclick="state.clientSearch='';state.clientStatusFilter='all';state.clientActivityFilter='all';render()">All · ${rows.length}</button></div><div class="client-activity-filters">${['dormant','archived'].map(f=>`<button class="${activityFilter===f?'primary':'secondary'} compact-button" onclick="state.clientActivityFilter='${f}';render()">${f[0].toUpperCase()+f.slice(1)} · ${rows.filter(x=>x.activity_status===f).length}</button>`).join('')}</div></div>`}<div class="client-groups client-groups-stacked"><div class="client-group client-group-wide"><h3>${operationalClientStatusLabel(filter)} · ${shown.length}</h3>${shown.map(x=>`<button class="client-overview-card status-${esc(x.client_status)}" data-search="${esc(([x.name,x.email,x.whatsapp_phone,x.mpesa_phone,...(x.pets||[]).map(p=>p.name)].join(' ')).toLowerCase())}" onclick="${rowAction(x)}"><span class="client-overview-head"><span><span class="client-overview-name">${esc(x.name)}</span><small>${esc(x.email)}${x.whatsapp_phone?` · WhatsApp ${esc(x.whatsapp_phone)}`:''}</small></span><span class="client-status-badge status-${esc(x.client_status)}">${esc(operationalClientStatusLabel(x.client_status))}</span><span class="client-open-arrow">→</span></span><span class="client-status-detail">${esc(clientStatusDetail(x))}</span><span class="client-dog-table">${(x.pets||[]).filter(p=>!p.archived).map(p=>`<span class="client-dog-table-row client-dog-name-only"><span>${esc(p.name)}</span></span>`).join('')||'<span class="small">No dog saved yet.</span>'}</span></button>`).join('')||'<p class="small">None</p>'}</div></div>${bookingMode?'':clientRecordModal()}${trainerClientBookingModal()}${rejectionDecisionModal()}</section>`;
}
async function openIncompleteClientRecord(id){try{state.clientRecord=await api(`/api/trainer/application-drafts/${id}`);document.body.classList.add('modal-open');render()}catch(e){appAlert(e.message)}}
...
function clientRecordModal(){const c=state.clientRecord;if(!c)return '';
 ...
 const appointmentsForPet=(pid)=>(c.bookings||[]).filter(b=>Number(b.pet_id)===Number(pid));
 const generalNotes=c.privateNotes||[],messages=c.messages||[];
 return `<div class="modal-overlay"><div class="trainer-modal client-record-modal"><button class="close-btn modal-close client-record-close" aria-label="Close client record" onclick="closeClientRecord()">×</button><div class="eyebrow">${isDraft?'Incomplete application':'Client record'}</div><div class="client-record-top"><div class="client-record-title compact-client-contact"><h2>${esc(u.name||'Applicant')}</h2><p>${esc(u.email||'')}</p><p>WhatsApp: ${esc(u.whatsapp_phone||u.phone||'—')}</p><p>M-Pesa: ${esc(u.mpesa_phone||u.phone||'—')}</p><p><b>Area:</b> ${esc(u.location||'Not supplied')}</p>${u.google_maps_location?`<p><b>Google Maps:</b> <a href="${esc(u.google_maps_location)}" target="_blank" rel="noopener">Open location ↗</a></p>`:''}<p>Canine Grapevine: ${u.newsletter_opt_in?'Opted in':'Not subscribed'}</p></div><div class="client-payment-status-box"><span class="client-status-badge status-${esc(status)}">${esc(operationalClientStatusLabel(status))}</span><strong>Application payment</strong><p>${isDraft?'Not yet submitted':esc(applicationPaymentSummary(c))}</p></div></div>
 <div class="client-record-folds"><details><summary>Introduction</summary><div class="fold-content"><div class="application-introduction-box"><b class="application-box-heading">Introduction / training needs</b><p>${esc(u.client_intro_note||c.draft?.introNote||'Not supplied')}</p></div>${!isDraft?`<button class="secondary compact-button" onclick="editClientProfileFromRecord(${u.id})">Edit client profile</button><div class="general-private-notes"><h4>Private notes · Amy only</h4><div class="private-note-entry"><textarea id="newPrivateClientNote" rows="2" placeholder="Record a private note..."></textarea><button class="primary compact-button" onclick="savePrivateClientNote(${u.id})">Save note</button></div>${generalNotes.map(n=>`<div class="private-note-card"><time>${esc(formatNairobiActivityTime(n.created_at))}</time><p>${esc(n.note)}</p></div>`).join('')||'<p class="small">No private notes yet.</p>'}${(c.pets||[]).filter(p=>p.trainer_notes).map(p=>`<div class="private-note-card legacy-note"><time>Earlier note · ${esc(p.name)}</time><p>${esc(p.trainer_notes)}</p></div>`).join('')}</div>`:''}</div></details>
 <details><summary>Dogs</summary><div class="fold-content trainer-dog-records">${(c.pets||[]).map(p=>`<details class="trainer-dog-record"><summary>${esc(p.name)}</summary><div><p>${esc(p.breed||c.draft?.dogBreed||'Dog')}${p.gender?` · ${esc(p.gender)}`:''}${p.date_of_birth?` · DOB ${displayDate(p.date_of_birth,{day:'numeric',month:'short',year:'numeric'})}`:''}</p>${!isDraft?`<p>Vaccination: ${p.vaccination_count?`Uploaded · ${p.vaccination_status==='verified'?'Verified':'Pending verification'}`:esc(p.vaccination_status||'not provided')}</p><button class="secondary compact-button" onclick="openVaccinationReview(${p.id})">Vaccination</button><h4>Appointments</h4><div class="compact-list">${appointmentsForPet(p.id).map(b=>`<div class="package-session-row"><span>${fmt(b.start_at)} · ${esc(b.booking_ref||'')}</span><button class="secondary compact-button" onclick="openTrainerBookingFromClientRecord(${b.id},${u.id})">Open</button></div>`).join('')||'<p class="small">No private appointments.</p>'}</div>`:'<p class="small">Dog details saved in the unfinished application.</p>'}</div></details>`).join('')||'<p class="small">No dog details saved yet.</p>'}</div></details>
 ${!isDraft?`<details><summary>Messages</summary><div class="fold-content message-admin-stream">${messages.map(m=>`<article class="client-message-entry"><div class="client-message-meta"><b>${esc(m.sender||'Amy')} · ${esc(formatNairobiActivityTime(m.created_at))}${m.pet_name?` · ${esc(m.pet_name)}`:''}</b></div><div class="client-message-card">${esc(m.body||'')}</div></article>`).join('')||'<p class="small">No messages yet.</p>'}<div class="actions"><button class="primary compact-button" onclick="sendTrainerClientMessage(${u.id})">Message</button></div></div></details><details><summary>Resources shared</summary><div class="fold-content">${(c.resources||[]).map(r=>`<div class="client-shared-resource"><strong>${esc(r.title)}</strong><span>${esc(r.category||'General')} · ${esc(String(r.type||'').toUpperCase())}${r.shared_at?` · ${esc(formatNairobiActivityTime(r.shared_at))}`:''}</span>${r.note?`<small>${esc(r.note)}</small>`:''}</div>`).join('')||'<p class="small">No training resources shared yet.</p>'}</div></details>`:''}</div>
 ${isRejected&&u.rejection_note?`<div class="notice bad"><b>Rejection reason</b><br>${esc(u.rejection_note)}</div>`:''}<div class="actions client-decision-actions">${isDraft?`<button class="danger" onclick="deleteIncompleteApplication(${u.id})">Delete application</button>`:''}${isUnverified&&c.applicationDeposit?.manual_payment_status==='submitted'?`<button class="primary" onclick="approveNewClient(${c.applicationDeposit.id})">Approve payment + KES 700 credit</button>`:''}${isUnverified?`<button class="danger" onclick="openRejectNewClient(${u.id},${c.applicationDeposit?.manual_payment_status==='submitted'?'true':'false'})">${c.applicationDeposit?.manual_payment_status==='submitted'?'Refund & Reject':'Reject application'}</button>`:''}${status==='current'?`<button class="primary" onclick="startTrainerClientBooking(${u.id})">＋ Book for this client</button>`:''}${isRejected?`<button class="danger" onclick="deleteRejectedClient(${u.id})">Delete rejected client</button>`:''}<button class="secondary" onclick="closeClientRecord()">Done</button></div></div></div>`}
async function editClientProfileFromRecord(id){const c=state.clientRecord,u=c?.user;if(!u)return;const ask=async(label,value)=>await appPrompt(label,String(value??''));const first=await ask('First name:',u.first_name);if(first===null)return;const last=await ask('Last name:',u.last_name);if(last===null)return;const area=await ask('Area:',u.location);if(area===null)return;const maps=await ask('Google Maps location:',u.google_maps_location);if(maps===null)return;const intro=await ask('Introduction / training needs:',u.client_intro_note);if(intro===null)return;const adults=await ask('Adults in household:',u.household_adults);if(adults===null)return;const dogs=await ask('Dogs in household:',u.household_dogs);if(dogs===null)return;const c08=await ask('Children aged 0–8:',u.children_0_8);if(c08===null)return;const c913=await ask('Children aged 9–13:',u.children_9_13);if(c913===null)return;const c14=await ask('Children aged 14+:',u.children_14_plus);if(c14===null)return;const changes=await ask('Recent / planned household changes:',u.household_changes);if(changes===null)return;const householdNote=await ask('Additional household note:',u.household_note);if(householdNote===null)return;const kra=await ask('KRA PIN (optional):',u.kra_pin);if(kra===null)return;try{await api(`/api/trainer/clients/${id}/profile`,{method:'PUT',body:JSON.stringify({firstName:first,lastName:last,location:area,googleMapsLocation:maps,clientIntroNote:intro,householdAdults:adults,householdDogs:dogs,children0to8:c08,children9to13:c913,children14plus:c14,householdChanges:changes,householdNote,kraPin:kra})});state.clientRecord=await api(`/api/trainer/client/${id}`);state.clientAdmin=await api('/api/trainer/clients');render()}catch(e){appAlert(e.message)}}
async function savePrivateClientNote(id){const el=document.getElementById('newPrivateClientNote'),note=el?.value.trim();if(!note)return appAlert('Enter a private note.');try{await api(`/api/trainer/clients/${id}/private-notes`,{method:'POST',body:JSON.stringify({note})});state.clientRecord=await api(`/api/trainer/client/${id}`);render()}catch(e){appAlert(e.message)}}
async function sendTrainerClientMessage(id){const body=await appPrompt('Message to client:','');if(body===null||!String(body).trim())return;try{await api(`/api/trainer/clients/${id}/messages`,{method:'POST',body:JSON.stringify({body:String(body).trim()})});state.clientRecord=await api(`/api/trainer/client/${id}`);render()}catch(e){appAlert(e.message)}}
async function openTrainerBookingFromClientRecord(id,userId){const c=state.clientRecord,b=(c?.bookings||[]).find(x=>Number(x.id)===Number(id));if(!b)return;state.trainerBooking={...b,client:c.user.name,client_phone:c.user.whatsapp_phone,user_id:Number(userId)};state.trainerClient=c;render()}
async function deleteIncompleteApplication(id){if(!await appConfirm('Delete this incomplete application? The unfinished record will be removed and the email can be used to start again.'))return;try{await api(`/api/trainer/application-drafts/${id}`,{method:'DELETE'});state.clientRecord=null;document.body.classList.remove('modal-open');state.clientAdmin=await api('/api/trainer/clients');render()}catch(e){appAlert(e.message)}}
...
function openRejectNewClient(id,hasPayment){state.rejectionDecision={id:Number(id),hasPayment:!!hasPayment,refundMethod:hasPayment?'mpesa':'',refundReference:'',rejectionNote:'',error:''};render()}
function rejectionDecisionModal(){const m=state.rejectionDecision;if(!m)return '';return `<div class="modal-overlay"><div class="trainer-modal rejection-decision-modal"><button class="close-btn modal-close" onclick="state.rejectionDecision=null;render()">×</button><div class="eyebrow">New Client Application</div><h2>Reject application</h2><label>Optional reason for client<textarea id="rejectionNote" rows="3" oninput="state.rejectionDecision.rejectionNote=this.value">${esc(m.rejectionNote||'')}</textarea></label>${m.hasPayment?`<label>Refund method<select id="rejectionRefundMethod" onchange="state.rejectionDecision.refundMethod=this.value;render()"><option value="mpesa" ${m.refundMethod==='mpesa'?'selected':''}>M-Pesa</option><option value="cash" ${m.refundMethod==='cash'?'selected':''}>Cash</option></select></label>${m.refundMethod==='mpesa'?`<label>M-Pesa refund/reference code<input id="rejectionRefundReference" maxlength="10" autocomplete="off" autocapitalize="characters" autocorrect="off" spellcheck="false" value="${esc(m.refundReference||'')}" oninput="this.value=this.value.replace(/[^a-z0-9]/gi,'').toUpperCase().slice(0,10);state.rejectionDecision.refundReference=this.value"><small>Exactly 10 letters/numbers. Stored in uppercase.</small></label>`:''}<div class="notice">KES 700 refundable portion</div>`:''}${m.error?`<div class="notice bad">${esc(m.error)}</div>`:''}<div class="actions"><button class="secondary" onclick="state.rejectionDecision=null;render()">Cancel</button><button class="danger" onclick="submitRejectNewClient()">Reject application${m.hasPayment?' & record refund':''}</button></div></div></div>`}
async function submitRejectNewClient(){const m=state.rejectionDecision;if(!m)return;m.rejectionNote=document.getElementById('rejectionNote')?.value??m.rejectionNote;m.refundMethod=m.hasPayment?(document.getElementById('rejectionRefundMethod')?.value||m.refundMethod):'';m.refundReference=m.refundMethod==='mpesa'?(document.getElementById('rejectionRefundReference')?.value||m.refundReference):'';if(m.hasPayment&&m.refundMethod==='mpesa'&&!/^[A-Z0-9]{10}$/.test(m.refundReference)){m.error='Enter the full 10-character M-Pesa refund/reference code.';render();return}if(!await appConfirm('Reject this application? The client will remain restricted until Amy later deletes the rejected record.'))return;try{await api(`/api/trainer/clients/${m.id}/reject-application`,{method:'POST',body:JSON.stringify({refundMethod:m.refundMethod,refundReference:m.refundReference,rejectionNote:m.rejectionNote})});state.rejectionDecision=null;state.clientRecord=await api(`/api/trainer/client/${m.id}`);state.clientAdmin=await api('/api/trainer/clients');state.trainer=await api('/api/trainer/summary');render()}catch(e){m.error=e.message;render()}}
async function deleteRejectedClient(id){if(!await appConfirm('Delete this rejected client from the active system? A historical archive record will be kept and the email can be used for a fresh future application.'))return;try{await api(`/api/trainer/clients/${id}/rejected`,{method:'DELETE'});state.clientRecord=null;document.body.classList.remove('modal-open');state.clientAdmin=await api('/api/trainer/clients');render()}catch(e){appAlert(e.message)}}


function attentionAdminView(){
 const t=state.trainer||{},filter=state.attentionFilter||'all',q=String(state.attentionSearch||'').trim().toLowerCase();
 const items=[];
 const add=(category,priority,search,html,expiry='')=>items.push({category,priority,search:String(search||'').toLowerCase(),html,expiry:String(expiry||'')});
 (t.rescheduleAttention||[]).forEach(x=>add('new',0,`${x.client_name} ${x.pet_name||''} ${x.booking_ref||''}`,`<div class="card attention-card"><b>Booking change · ${esc(x.client_name)} · ${esc(x.pet_name||'')}</b><p>Current: ${fmt(x.old_start_at)}<br>Requested: ${fmt(x.proposed_start_at)}${x.client_note?`<br>Client note: ${esc(x.client_note)}`:''}<br><b>Held until ${fmt(x.hold_expires_at)}</b></p><div class="actions"><button class="primary compact-button" onclick="decideRescheduleRequest(${x.id},'approve')">Approve</button><button class="danger compact-button" onclick="decideRescheduleRequest(${x.id},'decline')">Decline</button></div></div>`,x.hold_expires_at));
 (t.newBookingAttention||[]).forEach(x=>add('new',1,`${x.client_name} ${x.pet_name||''} ${x.booking_ref||''}`,`<div class="card attention-card"><b>New booking · ${esc(x.client_name)}</b><p>${esc(x.pet_name||'')} · ${esc(x.booking_ref||'')} · ${fmt(x.start_at)}</p><div class="actions"><button class="secondary compact-button" onclick="openTrainerBooking(${x.booking_id})">Open booking</button><button class="primary compact-button" onclick="resolveTrainerNotification(${x.notification_id})">Mark seen</button></div></div>`)); (t.newClassBookingAttention||[]).forEach(x=>add('new',1,`${x.client_name} ${x.pet_name||''} ${x.title||''} ${x.booking_ref||''}`,`<div class="card attention-card"><b>New class booking · ${esc(x.client_name)}</b><p>${esc(x.pet_name||'')} · ${esc(x.title||'Class')} · ${esc(x.booking_ref||'')} · starts ${displayDate(x.start_date,{day:'numeric',month:'short',year:'numeric'})}</p><div class="actions"><button class="secondary compact-button" onclick="openClassFromAgenda(${x.class_id})">Open class</button><button class="primary compact-button" onclick="resolveTrainerNotification(${x.notification_id})">Mark seen</button></div></div>`));
 (t.messagesAttention||[]).forEach(x=>add('messages',0,`${x.client_name} ${x.body}`,`<div class="card attention-card"><b>Client message · ${esc(x.client_name)}</b><p>${esc(String(x.body||'').slice(0,220))}</p><div class="actions"><button class="secondary compact-button" onclick="openClientRecordFromAttention(${x.user_id})">Open client</button><button class="primary compact-button" onclick="markClientMessageSeen(${x.id})">Mark seen</button></div></div>`));

 (t.cancellationAttention||[]).forEach(x=>add('refunds',0,`${x.client_name} ${x.pet_name||''} ${x.booking_ref}`,`<div class="card attention-card"><b>Refund pending · ${esc(x.booking_ref)}</b><p>${esc(x.client_name)} · ${esc(x.pet_name||'')}</p><button class="secondary" onclick="openTrainerBooking(${x.id})">Handle refund</button></div>`));
 (t.classRefundAttention||[]).forEach(x=>add('refunds',0,`${x.client_name} ${x.pet_name||''} ${x.title}`,`<div class="card attention-card"><b>Class refund pending · ${esc(x.title)}</b><p>${esc(x.client_name)} · ${esc(x.pet_name||'')} · Refund decision required</p><button class="secondary" onclick="openClassRefundFromAttention(${x.class_id},${x.id})">Handle refund</button></div>`));
 (t.manualPaymentAttention||[]).forEach(x=>add('payments',x.hold_expires_at?0:1,`${x.client_name} ${x.pet_name||''} ${x.booking_ref} ${x.manual_payment_code}`,`<div class="card attention-card"><b>Manual M-Pesa to verify · ${esc(x.client_name)}</b><p>${esc(x.pet_name||'')} · ${esc(x.booking_ref)} · M-Pesa Ref ${esc(x.manual_payment_code)} · M-Pesa ${money(x.manual_payment_amount)}${Number(x.credit_applied||0)>0?` · Credit used ${money(x.credit_applied)}`:''} · Total ${money(x.price)}</p><div class="actions"><button class="primary compact-button" onclick="verifyManualPayment(${x.id},true)">Verify payment</button><button class="danger compact-button" onclick="verifyManualPayment(${x.id},false)">Reject</button></div></div>`,x.hold_expires_at));
 (t.classManualPaymentAttention||[]).forEach(x=>add('payments',x.hold_expires_at?0:1,`${x.client_name} ${x.pet_name||''} ${x.title} ${x.booking_ref} ${x.manual_payment_code}`,`<div class="card attention-card"><b>Class M-Pesa to verify · ${esc(x.client_name)}</b><p>${esc(x.pet_name||'')} · ${esc(x.title||'Class')} · ${esc(x.booking_ref)} · M-Pesa Ref ${esc(x.manual_payment_code)} · M-Pesa ${money(x.manual_payment_amount)}${Number(x.credit_applied||0)>0?` · Credit used ${money(x.credit_applied)}`:''} · Total ${money(x.price)}</p><div class="actions"><button class="primary compact-button" onclick="verifyClassManualPayment(${x.id},true)">Verify payment</button><button class="danger compact-button" onclick="verifyClassManualPayment(${x.id},false)">Reject</button></div></div>`,x.hold_expires_at));
 (t.applicationDepositAttention||[]).forEach(x=>add('payments',0,`${x.client_name} ${x.manual_payment_code}`,`<div class="card attention-card"><b>New client payment waiting · ${esc(x.client_name)}</b><p>M-Pesa Ref ${esc(x.manual_payment_code)} · ${money(x.amount||1000)} · Unverified</p><button class="primary compact-button" onclick="openClientRecordFromAttention(${x.user_id})">Open client & decide</button></div>`));
 (t.vaccinationAttention||[]).forEach(x=>add('vaccinations',1,`${x.client_name} ${x.pet_name}`,`<div class="card attention-card"><b>Vaccination · ${esc(x.pet_name)}</b><p>${esc(x.client_name)} · ${Number(x.vaccination_count||0)===0?'No vaccination record uploaded':x.vaccination_status==='rejected'?'Replacement requested':'Record waiting for review'}</p><button class="secondary" onclick="openVaccinationReview(${x.pet_id})">Review record</button></div>`));
 (t.pendingReviews||[]).forEach(x=>add('reviews',1,`${x.name||''} ${x.text||''}`,`<div class="card attention-card"><b>Review waiting · ${esc(x.name||'Client')}</b><p>${esc(String(x.text||'').slice(0,180))}</p><button class="secondary" onclick="openTrainerAdmin('reviews')">Review</button></div>`));
 const map={payments:'payments',new:'new',refunds:'refunds',vaccinations:'vaccinations',reviews:'reviews',messages:'messages',all:'all'},filtered=items.filter(x=>(filter==='all'||x.category===map[filter])&&(!q||x.search.includes(q)));
 filtered.sort((a,b)=>a.priority-b.priority||((a.expiry&&b.expiry)?a.expiry.localeCompare(b.expiry):a.expiry?-1:b.expiry?1:0));
 const filters=[['all','All'],['payments','Unverified payments'],['new','New bookings'],['refunds','Refunds Pending'],['vaccinations','Vaccination Records'],['reviews','Reviews'],['messages','Messages']];
 return `<section class="screen admin-screen"><button class="back-dashboard" onclick="dashboardBack()">← Back to Dashboard</button><div class="admin-head"><div><div class="eyebrow">Amy's workspace</div><h2>Needs attention</h2><p class="small">Urgent expiring items are shown first.</p></div><input class="admin-search attention-search" value="${esc(state.attentionSearch||'')}" placeholder="Search client, dog, booking or M-Pesa ref" oninput="attentionSearchInput(this)"></div><div class="attention-filters">${filters.map(([k,label])=>`<button class="${filter===k?'primary':'secondary'} compact-button" onclick="state.attentionFilter='${k}';render()">${label} · ${k==='all'?items.length:items.filter(x=>x.category===k).length}</button>`).join('')}</div><div class="admin-list-large">${filtered.map(x=>x.html).join('')||'<p class="small">Nothing in this view needs attention.</p>'}</div></section>`;
}


function attentionSearchInput(input){state.attentionSearch=input.value;clearTimeout(window.__cmcAttentionSearchTimer);window.__cmcAttentionSearchTimer=setTimeout(()=>{render();requestAnimationFrame(()=>{const el=document.querySelector('.attention-search');if(el){el.focus();const n=el.value.length;try{el.setSelectionRange(n,n)}catch(_e){}}})},180)}
...
async function openClientRecordFromAttention(id){state.clientAdmin=await api('/api/trainer/clients');state.clientRecord=await api(`/api/trainer/client/${id}`);document.body.classList.add('modal-open');state.trainerAdminPage='clients';state.view='trainerAdmin';render()}
async function markClientMessageSeen(id){await api(`/api/trainer/messages/${id}/seen`,{method:'POST',body:'{}'});state.trainer=await api('/api/trainer/summary');render()}
...
function reportColumnLabel(k){
 const labels={amount:'Payment',deleted_at:'Deleted',rejected_at:'Rejected',start_at:'Date / time',received_at:'Received date / time',end_at:'Ends',booking_ref:'Reference',payment_status:'Payment',status:'Status',mpesa_ref:'M-Pesa Ref',refund_credit:'Refund / credit',manual_payment_code:'M-Pesa Ref',manual_payment_amount:'Manual paid',refund_amount:'Refund',location_type:'Location',newsletter_opt_in:'Grapevine',vaccination_status:'Status',vaccination_verified_at:'Verified'};
...
function reportCellValue(k,v){
 ...
 if(k==='newsletter_opt_in')return Number(v)?'Yes':'No';if(k==='status'&&String(v||'')==='credit_paid')return 'Credit used';
...
function rejectedArchiveCards(rows){return `<div class="rejected-archive-cards">${rows.map(row=>`<article class="rejected-archive-card"><div class="archive-card-head"><div><h3>${esc(row.name||'Rejected client')}</h3><p>${esc(row.email||'')}${row.whatsapp?` · ${esc(row.whatsapp)}`:''}</p></div><span class="client-status-badge status-rejected">Rejected</span></div><div class="archive-card-grid"><div><strong>Area</strong><span>${esc(row.area||'—')}</span></div>${row.google_maps_location?`<div><strong>Google Maps</strong><span><a href="${esc(row.google_maps_location)}" target="_blank" rel="noopener">Open location ↗</a></span></div>`:''}<div class="archive-wide"><strong>Training needs</strong><span>${esc(row.training_needs||'—')}</span></div><div class="archive-wide"><strong>Dogs</strong><span>${esc(row.dogs||'—')}</span></div>${row.rejection_reason?`<div class="archive-wide"><strong>Reason for rejection</strong><span>${esc(row.rejection_reason)}</span></div>`:''}<div><strong>Application payment</strong><span>${esc(row.payment||'—')}</span></div><div><strong>Refund</strong><span>${esc(row.refund||'—')}</span></div><div><strong>Rejected</strong><span>${esc(formatReportDate(String(row.rejected_at||'').slice(0,10)))}</span></div><div><strong>Deleted / archived</strong><span>${esc(formatReportDate(String(row.deleted_at||'').slice(0,10)))}</span></div></div></article>`).join('')||'<p class="small">No rejected clients in this date range.</p>'}</div>`}
function reportsAdminView(){const r=state.reports;const rows=r?.rows||[];const cols=reportDisplayColumns(r);return `<section class="screen admin-screen reports-screen"><button class="back-dashboard" onclick="dashboardBack()">← Back to Dashboard</button><div class="admin-head"><div><div class="eyebrow">Amy's workspace</div><h2>Reports</h2></div><button class="primary" onclick="openSingleClientReport()">Single Client Report</button></div><div class="panel report-controls"><div class="form-grid"><label>Report<select id="reportType" onchange="state.reportType=this.value"><option value="daily" ${state.reportType==="daily"?"selected":""}>Daily Sheet</option><option value="appointments" ${state.reportType==="appointments"?"selected":""}>Appointments</option><option value="payments" ${state.reportType==="payments"?"selected":""}>Payments</option><option value="clients" ${state.reportType==="clients"?"selected":""}>Clients</option><option value="vaccinations" ${state.reportType==="vaccinations"?"selected":""}>Vaccinations</option><option value="newsletter" ${state.reportType==="newsletter"?"selected":""}>The Canine Grapevine</option><option value="rejected_archive" ${state.reportType==="rejected_archive"?"selected":""}>Rejected Client Archive</option></select></label><label>From / date<input id="reportFrom" type="date" value="${esc(state.reportFrom||nairobiDateKeyClient(0))}"></label><label>To<input id="reportTo" type="date" value="${esc(state.reportTo||nairobiDateKeyClient(0))}"></label></div><div class="actions"><button class="primary" onclick="loadReport()">Generate report</button>${r?`<button class="secondary" onclick="window.print()">Print / Save PDF</button><button class="secondary" onclick="exportReportCsv()">Export CSV</button>`:''}</div></div>${r?`<div class="report-paper"><div class="report-heading"><img src="/brand-logo.png" alt=""><div><h2>The Custom Made Canine</h2><h3>${esc(r.title)}</h3><p>${esc(formatReportDate(r.from||''))}${r.to&&r.to!==r.from?` to ${esc(formatReportDate(r.to))}`:''}</p></div></div>${state.reportType==="rejected_archive"?rejectedArchiveCards(rows):`<div class="report-table-wrap"><table class="report-table"><thead><tr>${cols.map(k=>`<th>${esc(reportColumnLabel(k))}</th>`).join('')}</tr></thead><tbody>${rows.map(row=>`<tr>${cols.map(k=>`<td>${esc(reportCellValue(k,row[k]))}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`}</div>`:''}</section>`}
...
function singleClientDateDefaults(){
 const to=nairobiDateKeyClient(0);let from='';
 if(state.singleClientRange==='last_week')from=nairobiDateKeyClient(-7);else if(state.singleClientRange==='last_month')from=nairobiDateKeyClient(-30);else from=state.singleClientFrom||nairobiDateKeyClient(-30);
 state.singleClientFrom=from;state.singleClientTo=state.singleClientTo||to;
}
async function openSingleClientReport(){state.trainerAdminPage='singleClientReport';state.view='trainerAdmin';state.singleClientReport=null;state.singleClientClients=(await api('/api/trainer/clients')).filter(x=>x.record_type==='client');state.singleClientId=state.singleClientId||state.singleClientClients[0]?.id||null;state.singleClientRange='last_month';state.singleClientTo=nairobiDateKeyClient(0);singleClientDateDefaults();render()}
function setSingleClientRange(v){state.singleClientRange=v;state.singleClientTo=nairobiDateKeyClient(0);if(v==='last_week')state.singleClientFrom=nairobiDateKeyClient(-7);else if(v==='last_month')state.singleClientFrom=nairobiDateKeyClient(-30);render()}
function scrCheck(k,label,on=true){const checked=k.startsWith('dog-')?(state.singleClientDogs[k]??on):(state.singleClientSelections[k]??on);return `<label class="check-row"><input type="checkbox" id="scr-${k}" ${checked?'checked':''}> ${esc(label)}</label>`}
function singleClientReportView(){
 const clients=state.singleClientClients||[],r=state.singleClientReport;singleClientDateDefaults();const selectedClient=clients.find(c=>Number(c.id)===Number(state.singleClientId))||clients[0];
 return `<section class="screen admin-screen single-client-report-screen"><button class="back-dashboard report-builder-only" onclick="openTrainerAdmin('reports')">← Back to Reports</button><div class="admin-head report-builder-only"><div><div class="eyebrow">Amy's workspace</div><h2>Single Client Report</h2><p>Choose the client, date range and sections. These selections only filter what is included and do not form part of the report.</p></div></div>
 <div class="panel single-client-builder report-builder-only"><div class="form-grid"><label>Client<select id="scrClient" onchange="state.singleClientId=Number(this.value);state.singleClientReport=null;state.singleClientDogs={};render()">${clients.map(c=>`<option value="${c.id}" ${Number(c.id)===Number(state.singleClientId)?'selected':''}>${esc(c.name)}</option>`).join('')}</select></label><label>Date range<select id="scrRange" onchange="setSingleClientRange(this.value)"><option value="last_week" ${state.singleClientRange==='last_week'?'selected':''}>Last week</option><option value="last_month" ${state.singleClientRange==='last_month'?'selected':''}>Last month</option><option value="custom" ${state.singleClientRange==='custom'?'selected':''}>Custom date range</option></select></label><label>From<input id="scrFrom" type="date" value="${esc(state.singleClientFrom)}" ${state.singleClientRange==='custom'?'':'disabled'}></label><label>To<input id="scrTo" type="date" value="${esc(state.singleClientTo)}" ${state.singleClientRange==='custom'?'':'disabled'}></label></div>
 <div class="single-client-section-picker"><div><h4>Static sections</h4>${scrCheck('header','Client header')}${scrCheck('client','Client information')}${scrCheck('household','Household')}</div><div><h4>Dogs</h4>${(selectedClient?.pets||[]).length?(selectedClient.pets||[]).filter(p=>!p.archived).map(p=>scrCheck(`dog-${p.id}`,p.name)).join(''):'<p class="small">No active dogs.</p>'}</div><div><h4>Dated sections</h4>${scrCheck('history','History')}${scrCheck('notes','Training Notes')}${scrCheck('payments','Payments')}${scrCheck('resources','Resources Shared')}</div></div>
 <div class="actions"><button class="primary" onclick="generateSingleClientReport()">Generate report</button>${r?`<button class="secondary" onclick="window.print()">Print / Save PDF</button>`:''}</div></div>
 ${r?renderSingleClientReport(r):'<div class="panel"><p>Choose a client and generate the report.</p></div>'}</section>`;
}
async function generateSingleClientReport(){const id=Number(document.getElementById('scrClient')?.value||state.singleClientId);if(!id)return appAlert('Choose a client.');state.singleClientId=id;['header','client','household','history','notes','payments','resources'].forEach(k=>state.singleClientSelections[k]=!!document.getElementById(`scr-${k}`)?.checked);document.querySelectorAll('[id^=scr-dog-]').forEach(el=>state.singleClientDogs[el.id.replace('scr-','')]=!!el.checked);state.singleClientRange=document.getElementById('scrRange')?.value||state.singleClientRange;if(state.singleClientRange==='custom'){state.singleClientFrom=document.getElementById('scrFrom')?.value||'';state.singleClientTo=document.getElementById('scrTo')?.value||'';}else singleClientDateDefaults();if(!state.singleClientFrom||!state.singleClientTo)return appAlert('Choose a complete date range.');try{state.singleClientReport=await api(`/api/trainer/single-client-report/${id}?from=${encodeURIComponent(state.singleClientFrom)}&to=${encodeURIComponent(state.singleClientTo)}`);render()}catch(e){appAlert(e.message)}}
function scrSelected(id,def=true){return id.startsWith('dog-')?(state.singleClientDogs[id]??def):(state.singleClientSelections[id]??def)}
function scrLine(label,value){return value!==null&&value!==undefined&&String(value).trim()!==''?`<div class="single-report-line"><strong>${esc(label)}</strong><span>${esc(String(value))}</span></div>`:''}
function renderSingleClientReport(r){const u=r.user||{},sections=[];
 if(scrSelected('header'))sections.push(`<header class="single-report-header"><img src="/brand-logo.png" alt=""><div><h2>${esc(u.name||'Client')}</h2><p>Single Client Report</p></div></header>`);
 if(scrSelected('client'))sections.push(`<section class="single-report-section"><h3>Client information</h3>${scrLine('Name',u.name)}${scrLine('Email',u.email)}${scrLine('WhatsApp',u.whatsapp_phone||u.phone)}${scrLine('M-Pesa',u.mpesa_phone||u.phone)}${scrLine('Area',u.location)}${scrLine('Google Maps location',u.google_maps_location)}${scrLine('Training needs / introduction',u.client_intro_note)}${scrLine('Canine Grapevine',Number(u.newsletter_opt_in)?'Yes':'No')}${scrLine('KRA PIN',u.kra_pin)}</section>`);
 if(scrSelected('household'))sections.push(`<section class="single-report-section"><h3>Household</h3>${scrLine('Adults',u.household_adults)}${scrLine('Dogs',u.household_dogs)}${scrLine('Children 0–8',u.children_0_8)}${scrLine('Children 9–13',u.children_9_13)}${scrLine('Children 14+',u.children_14_plus)}${scrLine('Changes expected',u.household_changes)}${scrLine('Household note',u.household_note)}</section>`);
 (r.pets||[]).forEach(p=>{if(scrSelected(`dog-${p.id}`,true))sections.push(`<section class="single-report-section"><h3>${esc(p.name||'Dog')}</h3>${scrLine('Breed',p.breed)}${scrLine('Sex',p.gender)}${scrLine('Date of birth',p.date_of_birth?formatReportDate(p.date_of_birth):'')}${scrLine('Neutered / spayed',Number(p.neutered_spayed)?'Yes':'No')}${scrLine('Behaviour',p.behavior_notes)}${scrLine('Medical procedures',p.medical_procedures)}${scrLine('Vaccination',p.vaccination_status)}</section>`)});
 if(scrSelected('history'))sections.push(`<section class="single-report-section"><h3>History</h3>${(r.history||[]).map(x=>`<div class="single-report-dated"><time>${esc(formatNairobiActivityTime(x.created_at))}</time><span><b>${esc(String(x.action||'').replaceAll('_',' '))}</b>${x.pet_name?` · ${esc(x.pet_name)}`:''}${x.details?`<br>${esc(x.details)}`:''}</span></div>`).join('')||'<p>No history in this date range.</p>'}</section>`);
 if(scrSelected('notes'))sections.push(`<section class="single-report-section"><h3>Training Notes</h3>${(r.trainingNotes||[]).map(x=>`<div class="single-report-dated"><time>${esc(formatNairobiActivityTime(x.created_at))}</time><span>${x.pet_name?`<b>${esc(x.pet_name)}</b><br>`:''}${esc(x.note)}</span></div>`).join('')||'<p>No training notes in this date range.</p>'}</section>`);
 if(scrSelected('payments'))sections.push(`<section class="single-report-section"><h3>Payments</h3><div class="single-report-balance"><b>Account balance</b><span>${money(r.creditBalance||0)}</span></div>${(r.payments||[]).map(x=>`<div class="single-report-payment"><span>${esc(formatReportDate(String(x.date||'').slice(0,10)))}</span><span>${esc(x.reference||'—')}</span><span>${money(x.amount||0)}</span><span>${esc(x.status||'')}</span></div>`).join('')||'<p>No payments in this date range.</p>'}</section>`);
 if(scrSelected('resources'))sections.push(`<section class="single-report-section"><h3>Resources Shared</h3>${(r.resources||[]).map(x=>`<div class="single-report-resource"><span>${esc(x.title)}</span><time>${esc(formatReportDate(String(x.shared_at||'').slice(0,10)))}</time></div>`).join('')||'<p>No resources shared in this date range.</p>'}</section>`);
 return `<article class="report-paper single-client-report-output">${sections.join('')}</article>`}
...
(function () {
  ...
  function scan(){ document.querySelectorAll('input[type="password" autocapitalize="none" autocorrect="off" spellcheck="false" data-gramm="false" data-gramm_editor="false"]').forEach(enhance); }
