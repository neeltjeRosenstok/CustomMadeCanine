...
function clientFinancialSummary(b){
 const priv=Array.isArray(b?.privateBookings)?b.privateBookings:[],cls=Array.isArray(b?.classBookings)?b.classBookings:[],events=[];
 for(const x of priv){const d=displayDate(String(x.start_at||"").slice(0,10),{day:"numeric",month:"long",year:"numeric"});if(["refund_partial","refunded"].includes(x.payment_status)&&Number(x.refund_amount||0)>0)events.push({at:x.refund_recorded_at||x.start_at,text:`${x.payment_status==="refund_partial"?"Partial refund":"Refund"} ${money(x.refund_amount)} for cancelled training on ${d}${x.client_visible_note?` — ${esc(x.client_visible_note)}`:""}`});if(["credit_partial","credited"].includes(x.payment_status)&&Number(x.credit_amount||0)>0)events.push({at:x.refund_recorded_at||x.start_at,text:`${x.payment_status==="credit_partial"?"Partial credit":"Credit"} ${money(x.credit_amount)} for cancelled training on ${d}${x.client_visible_note?` — ${esc(x.client_visible_note)}`:""}`})}
 for(const x of cls){if(["refund_partial","refunded"].includes(x.payment_status)&&Number(x.refund_amount||0)>0)events.push({at:x.refund_recorded_at||x.created_at,text:`${x.payment_status==="refund_partial"?"Partial refund":"Refund"} ${money(x.refund_amount)} for cancelled class ${esc(x.title||"")}`});if(["credit_partial","credited"].includes(x.payment_status)&&Number(x.credit_amount||0)>0)events.push({at:x.refund_recorded_at||x.created_at,text:`${x.payment_status==="credit_partial"?"Partial credit":"Credit"} ${money(x.credit_amount)} for cancelled class ${esc(x.title||"")}`})}
 events.sort((x,y)=>String(y.at||"").localeCompare(String(x.at||"")));return events[0]?`<div class="notice account-update"><b>Latest account update</b><br>${events[0].text}</div>`:"";
}
...
async function decideRefund(id,decision){
 ...
 let confirmationCode=state.refundDraftCode||"";while(true){const entered=await appPrompt("Enter the full 10-character M-Pesa refund confirmation reference:",confirmationCode,full?"Full refund":"Partial refund");if(entered===null)return;confirmationCode=String(entered).replace(/\s+/g,"").toUpperCase();state.refundDraftCode=confirmationCode;if(/^[A-Z0-9]{10}$/.test(confirmationCode))break;await appAlert("Enter the full 10-character M-Pesa refund confirmation reference.")}
 try{await api(`/api/trainer/bookings/${id}/refund`,{method:"POST",body:JSON.stringify({decision,amount,confirmationCode})});state.refundDraftCode="";state.trainer=await api("/api/trainer/summary");state.trainerBooking=null;render();await appAlert(`${full?"Full":"Partial"} refund recorded: ${money(amount)} · M-Pesa ${confirmationCode}`)}catch(e){appAlert(e.message)}
}
...
function paymentView(){
 ...
 const automaticAction=c.paymentStartRequired?`<button class="primary payment-main-action" onclick="startPaymentFromPayment()">Continue with M-Pesa</button>`:c.mpesaDemo?`<button class="primary payment-main-action" onclick="demoPay()">Complete trial payment</button>`:`<div class="notice good"><b>STK Push sent.</b><br>Approve the M-Pesa prompt on your phone. If it does not arrive, you can retry or use the manual fallback below.</div><button class="secondary payment-main-action" onclick="startPaymentFromPayment()">Send STK Push again</button>`;
 const manual=c.type==='class'?`<div class="manual-payment-box"><h3>Manual payment if STK Push fails</h3><p class="small">Class manual-payment recovery is still being completed. Please contact Amy if the automatic payment cannot be completed.</p></div>`:`<div class="manual-payment-box"><h3>Manual payment if STK Push fails</h3><p>Use this only if the automatic M-Pesa prompt does not work.</p><ol class="manual-payment-steps"><li>Send <b>${money(amount)}</b> by M-Pesa Send Money to <b>0725 365692</b>.</li><li>Wait for the M-Pesa SMS confirmation.</li><li>Enter the full 10-character M-Pesa reference below.</li></ol><div class="manual-payment-details"><span>Amount due</span><strong>${money(amount)}</strong></div><label>M-Pesa confirmation reference<input id="manualMpesaCode" maxlength="10" value="${esc(state.manualMpesaDraft||'')}" autocomplete="off" autocapitalize="characters" placeholder="e.g. ABC123DEFG" oninput="this.value=this.value.replace(/\\s+/g,'').toUpperCase()"></label><button class="secondary manual-submit-button" onclick="submitManualPaymentFromPayment()">Submit payment confirmation</button><p class="small">Amy will verify the payment before the booking is marked confirmed.</p></div>`;
 return `<section class="screen payment-screen"><div class="center"><div class="panel payment-panel" style="width:min(680px,100%)"><div class="eyebrow">Almost there</div><h1 class="mpesa-title">M-Pesa</h1><p class="lead">${esc(c.mpesaMessage||'Complete your payment to confirm the booking.')}</p><div class="notice"><b>Reference:</b> ${esc(c.bookingRef)}<br><b>Amount to pay:</b> ${money(amount)}${Number(c.creditApplied||0)>0?`<br><b>Credit already applied:</b> ${money(c.creditApplied)}`:""}${expiry}</div>${creditBox}<div class="automatic-payment-box"><h3>Automatic M-Pesa payment</h3>${automaticAction}</div>${manual}<div class="actions payment-exit-actions"><button class="secondary" onclick="state.portalTab='bookings';portal()">Return to Client Portal</button></div></div></div></section>`;
}

async function startPaymentFromPayment(){
 const c=state.confirm;if(!c?.id)return appAlert("Open the pending booking from your Client Portal to continue payment.");
 try{
...
async function submitManualPaymentFromPayment(){
 ...
 try{await api(`/api/my/bookings/${c.id}/manual-payment`,{method:'POST',body:JSON.stringify({confirmationCode:code,amount})});state.manualMpesaDraft="";await appAlert('Thank you. Your M-Pesa reference has been submitted to Amy for verification. Your booking is being held while she checks the payment. Please WhatsApp Amy to make sure she sees this.');state.portalTab='bookings';await portal()}catch(e){if(input)input.value=state.manualMpesaDraft||code;appAlert(e.message)}
}
...
function reportColumnLabel(k){
 const labels={start_at:'Date / time',received_at:'Received date / time',end_at:'Ends',booking_ref:'Reference',payment_status:'Payment',status:'Status',mpesa_ref:'M-Pesa Ref',refund_credit:'Refund / credit',manual_payment_code:'M-Pesa Ref',manual_payment_amount:'Manual paid',refund_amount:'Refund',location_type:'Location',newsletter_opt_in:'Grapevine',vaccination_status:'Status',vaccination_verified_at:'Verified'};
...
function reportCellValue(k,v){
 if((k==='start_at'||k==='received_at'||k==='end_at'||k==='vaccination_verified_at')&&v)return formatReportDate(v);
