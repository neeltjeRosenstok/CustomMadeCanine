...
function clientFinancialSummary(b){
 ...
 const recent=(value)=>{if(!value)return false;const raw=String(value).trim().replace(" ","T");const ms=Date.parse(raw.endsWith("Z")?raw:raw+"Z");return Number.isFinite(ms)&&Date.now()-ms<=30*24*60*60*1000};
 for(const x of priv){const d=displayDate(String(x.start_at||"").slice(0,10),{day:"numeric",month:"long",year:"numeric"});if(["refund_partial","refunded"].includes(x.payment_status)&&Number(x.refund_amount||0)>0&&recent(x.refund_recorded_at))events.push({at:x.refund_recorded_at,text:`${x.payment_status==="refund_partial"?"Partial refund":"Refund"} ${money(x.refund_amount)} for cancelled training on ${d}${x.client_visible_note?` — ${esc(x.client_visible_note)}`:""}`});if(["credit_partial","credited"].includes(x.payment_status)&&Number(x.credit_amount||0)>0&&recent(x.refund_recorded_at))events.push({at:x.refund_recorded_at,text:`${x.payment_status==="credit_partial"?"Partial credit":"Credit"} ${money(x.credit_amount)} for cancelled training on ${d}${x.client_visible_note?` — ${esc(x.client_visible_note)}`:""}`})}
 for(const x of cls){if(["refund_partial","refunded"].includes(x.payment_status)&&Number(x.refund_amount||0)>0&&recent(x.refund_recorded_at))events.push({at:x.refund_recorded_at,text:`${x.payment_status==="refund_partial"?"Partial refund":"Refund"} ${money(x.refund_amount)} for cancelled class ${esc(x.title||"")}`});if(["credit_partial","credited"].includes(x.payment_status)&&Number(x.credit_amount||0)>0&&recent(x.refund_recorded_at))events.push({at:x.refund_recorded_at,text:`${x.payment_status==="credit_partial"?"Partial credit":"Credit"} ${money(x.credit_amount)} for cancelled class ${esc(x.title||"")}`})}
 events.sort((x,y)=>String(y.at||"").localeCompare(String(x.at||"")));return events[0]?`<div class="notice account-update"><b>Latest account update</b><br>${events[0].text}<div class="small">This notice stays here for 30 days; the transaction remains in your booking history afterwards.</div></div>`:"";
}
...
function monthCalendarView(){
 ...
 return `<div class="month-calendar"><div class="calendar-head"><button class="secondary compact-button" onclick="moveTrainerMonth(-1)">←</button><span>${first.toLocaleDateString("en-KE",{month:"long",year:"numeric"})}</span><button class="secondary compact-button" onclick="moveTrainerMonth(1)">→</button></div><div class="month-calendar-legend"><span><i class="legend-swatch open"></i>Available</span><span><i class="legend-swatch arena"></i>Arena unavailable</span><span><i class="legend-swatch home"></i>Home visits unavailable</span><span><i class="legend-swatch amy"></i>Amy unavailable</span><span><i class="month-booking-marker arena"></i>Arena booking</span><span><i class="month-booking-marker home"></i>Home visit</span><span><i class="month-booking-marker class"></i>Class</span></div><div class="month-weekdays">${["Mon","Tue","Wed","Thu","Fri","Sat","Sun"].map((x,i)=>`<span class="${i>=5?"weekend-heading":""}">${x}</span>`).join("")}</div><div class="month-grid">${cells.join("")}</div></div>`;
}
...
async function assignResourceFromAppointment(){
 ...
 try{const rows=await api('/api/trainer/resources');if(!rows.length)return appAlert('There are no training resources in the library yet.');const choice=await appPrompt(rows.slice(0,30).map((r,i)=>`${i+1}. ${r.title} — ${String(r.type||'').toUpperCase()}`).join('\n')+'\n\nChoose resource number:','');if(choice===null)return;const n=Number(choice);if(!n||!rows[n-1])return appAlert('Choose a valid resource number.');const r=rows[n-1];const note=await appPrompt(`Optional note for ${b.client||'the client'}:`, '') ;if(note===null)return;await api(`/api/trainer/resources/${r.id}/access`,{method:'POST',body:JSON.stringify({userId:b.user_id,note})});await appAlert(`${r.title} has been shared with ${b.client||'the client'}.`)}catch(e){appAlert(e.message)}
}
...
function classModalView(){
 ...
 return `<div class="modal-overlay"><div class="trainer-modal schedule-modal"><button class="close-btn modal-close" aria-label="Close" onclick="state.scheduleModal=null;render()">×</button><div class="eyebrow">Course planner</div><h2>${m.editId?"Edit class course":"Create a class course"}</h2><p class="small">${m.editId?"Future session dates can be changed. Sessions that have already taken place remain part of the course history and cannot be moved.":"Choose the first date; weekly or fortnightly courses follow that weekday automatically. Saturdays are not assumed."}</p><div class="form-grid"><label>Course name<input id="classTitle" value="${esc(m.title||'')}" oninput="state.scheduleModal.title=this.value" required></label><label>Number of classes<input id="classCount" type="number" min="1" max="20" value="${n}" ${m.hasPastSessions?'disabled':''} onchange="state.scheduleModal.count=Number(this.value);render()"></label></div>${m.editId?`<input type="hidden" id="classStartDateBtn" data-value="${esc(m.startDate||'')}"><input type="hidden" id="classRecurrence" value="custom">`:`<div class="form-grid class-date-row"><label class="class-date-field">First date<button type="button" id="classStartDateBtn" class="picker-button" data-value="${esc(m.startDate||'')}" onclick="pickDateButton(this)">${m.startDate?`📅 ${new Date(`${m.startDate}T12:00:00`).toLocaleDateString('en-KE',{weekday:'short',day:'numeric',month:'short',year:'numeric'})}`:'📅 Select date'}</button></label><label>Repeats<select id="classRecurrence" onchange="state.scheduleModal.recurrence=this.value;render()"><option value="weekly" ${recurrence==='weekly'?'selected':''}>Every week</option><option value="biweekly" ${recurrence==='biweekly'?'selected':''}>Every 2 weeks</option><option value="custom" ${custom?'selected':''}>Custom dates</option></select></label></div>`}${!custom?`<div class="notice"><b>Weekday:</b> ${esc(weekdayLabel)}. The first date determines the weekday automatically.</div>`:`<div class="custom-date-list"><h3>Class dates</h3>${Array.from({length:n},(_,i)=>{const value=m.customDates?.[i]||'',past=!!m.editId&&value<today;return `<label>Class ${i+1}${past?' · completed':''}<input class="custom-class-date" type="date" inputmode="none" ${past?'disabled':''} onkeydown="event.preventDefault()" onbeforeinput="event.preventDefault()" onclick="${past?'':'this.showPicker&&this.showPicker()'}" value="${esc(value)}" onchange="state.scheduleModal.customDates[${i}]=this.value"></label>`}).join('')}</div>`}<div class="form-grid"><label>Start time<button type="button" id="classStartTimeBtn" class="picker-button" data-value="${esc(m.startTime||'09:00')}" ${m.hasPastSessions?'disabled':''} onclick="pickTimeButton(this)">${esc(m.startTime||'09:00')}</button></label><label>End time<button type="button" id="classEndTimeBtn" class="picker-button" data-value="${esc(m.endTime||'10:00')}" ${m.hasPastSessions?'disabled':''} onclick="pickTimeButton(this)">${esc(m.endTime||'10:00')}</button></label></div>${m.hasPastSessions?'<div class="notice">Completed session dates and the course time are locked so the historical record cannot be rewritten.</div>':''}<div class="form-grid"><label>Price (KES)<input id="classPrice" type="number" min="0" value="${esc(m.price||'7500')}" onchange="state.scheduleModal.price=this.value"></label><label>Places<input id="classCapacity" type="number" min="1" max="100" value="${esc(m.capacity||'12')}" onchange="state.scheduleModal.capacity=this.value"></label></div><div class="form-grid"><label>Minimum age (months)<input id="classMinAge" type="number" min="0" step="1" placeholder="3" value="${esc(m.minAgeMonths||'')}" oninput="state.scheduleModal.minAgeMonths=this.value"></label><label>Maximum age (months)<input id="classMaxAge" type="number" min="0" step="1" placeholder="12" value="${esc(m.maxAgeMonths||'')}" oninput="state.scheduleModal.maxAgeMonths=this.value"></label></div><p class="small">Age is checked from each dog’s date of birth on the course start date.</p><div class="form-grid"><label>Class location<select id="classLocationType" onchange="document.getElementById('alternateClassLocation').hidden=this.value!=='alternate'"><option value="arena" ${m.locationType!=="alternate"?"selected":""}>Amy's arena</option><option value="alternate" ${m.locationType==="alternate"?"selected":""}>Alternate location</option></select></label><label id="alternateClassLocation" ${m.locationType==="alternate"?"":"hidden"}>Alternate location<input id="classLocationName" placeholder="Venue / address" value="${esc(m.locationName||"")}"></label></div><div class="notice">If any proposed future class conflicts with an existing booking, class or blocked time, the course will not be saved until the conflict is resolved.</div><div class="actions"><button class="secondary" onclick="state.scheduleModal=null;render()">Cancel</button><button class="primary" onclick="submitClassModal()">${m.editId?"Save course changes":"Create course"}</button></div></div></div>`;
}
...
function bookingPolicyText(){
 return `<div class="policy-copy terms-copy"><p><b>Booking, cancellation & rescheduling policy</b></p><ol class="policy-list"><li><b>If your plans change, please give Amy as much notice as possible.</b></li><li><b>Cancellations:</b> cancellations made less than 36 hours before an appointment may be charged in full.</li><li><b>Rescheduling:</b> client rescheduling requests are subject to Amy's approval. A maximum of three approved client-requested reschedules is allowed per booking or package.</li><li><b>Trainer-created provisional bookings:</b> appointments and packages created by Amy are held for 24 hours while awaiting the client's confirmation and payment.</li></ol><p>By confirming a booking, you agree to The Custom Made Canine's Booking Terms & Cancellation Policy.</p></div>`;
}
