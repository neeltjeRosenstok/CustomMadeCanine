...
async function addClass(){
 state.scheduleModal={mode:'class',title:'',count:5,startDate:'',startTime:'',endTime:'',price:'7500',capacity:'12',recurrence:'weekly',customDates:[],minAgeMonths:'',maxAgeMonths:'',locationType:'arena',locationName:'',classAvailability:null,classAvailabilityMessage:'Choose the first date to see available times.'};render();
}
...
async function submitClassModal(){
 ...
 const endTime=state.scheduleModal?.endTime||document.getElementById('classEndTimeBtn')?.dataset.value||'';
...
async function loadClassAvailability(){
 ...
 try{
   m.classAvailabilityMessage="Checking availability…";render();
   ...
   if(m.startTime&&m.endTime&&!classTimeFitsAvailableBlock(m.startTime,m.endTime,m.classAvailability)){m.startTime="";m.endTime=""}
   render();
...
function classStartOptions(){
 const m=state.scheduleModal;if(!m)return [];
 const out=[];for(const b of m.classAvailability||[]){for(let ms=wallClockMsClient(`2000-01-01T${b.start}:00`);ms<wallClockMsClient(`2000-01-01T${b.end}:00`);ms+=30*60000)out.push(new Date(ms).toISOString().slice(11,16))}
 return [...new Set(out)];
}
function classEndOptions(){
 const m=state.scheduleModal;if(!m?.startTime)return [];
 const block=(m.classAvailability||[]).find(b=>m.startTime>=b.start&&m.startTime<b.end);if(!block)return [];
 const out=[];for(let ms=wallClockMsClient(`2000-01-01T${m.startTime}:00`)+30*60000;ms<=wallClockMsClient(`2000-01-01T${block.end}:00`);ms+=30*60000)out.push(new Date(ms).toISOString().slice(11,16));
 return out;
}
function chooseClassStart(time){const m=state.scheduleModal;if(!m)return;m.startTime=time;if(!m.endTime||m.endTime<=time||!classTimeFitsAvailableBlock(time,m.endTime,m.classAvailability))m.endTime="";render()}
function chooseClassEnd(time){const m=state.scheduleModal;if(!m)return;m.endTime=time;render()}
function classDateChanged(value){const m=state.scheduleModal;if(!m)return;m.startDate=value;if(m.recurrence==="custom"&&m.customDates?.length)m.customDates[0]=value;m.startTime="";m.endTime="";loadClassAvailability()}
function classLocationChanged(value){const m=state.scheduleModal;if(!m)return;m.locationType=value;m.startTime="";m.endTime="";loadClassAvailability()}
function classModalView(){
 ...
 const starts=classStartOptions(),ends=classEndOptions();
 return `<div class="modal-overlay"><div class="trainer-modal schedule-modal"><button class="close-btn modal-close" aria-label="Close" onclick="state.scheduleModal=null;render()">×</button><div class="eyebrow">Course planner</div><h2>${m.editId?"Edit class course":"Create a class course"}</h2><p class="small">${m.editId?"Future session dates can be changed. Sessions that have already taken place remain part of the course history and cannot be moved.":"Choose the first date, then choose the class time from genuinely available half-hour blocks."}</p>
 ...
 ${m.hasPastSessions?`<div class="notice">Completed session dates and the course time are locked so the historical record cannot be rewritten.</div><div class="class-time-summary"><strong>${esc(m.startTime)}–${esc(m.endTime)}</strong></div>`:`<section class="class-availability-picker"><h3>Available time</h3>${m.classAvailabilityMessage?`<p class="small">${esc(m.classAvailabilityMessage)}</p>`:""}${m.classAvailability===null?`<p class="small">Choose the first date above.</p>`:(m.classAvailability||[]).length?`<div class="availability-window-list">${m.classAvailability.map(b=>`<span>${esc(b.start)}–${esc(b.end)}</span>`).join("")}</div><div class="class-time-choice"><div><span class="choice-label">Start</span><div class="time-blocks">${starts.map(t=>`<button type="button" class="time-choice ${m.startTime===t?"selected":""}" onclick="chooseClassStart('${t}')">${t}</button>`).join("")}</div></div><div><span class="choice-label">End</span>${m.startTime?`<div class="time-blocks">${ends.map(t=>`<button type="button" class="time-choice ${m.endTime===t?"selected":""}" onclick="chooseClassEnd('${t}')">${t}</button>`).join("")}</div>`:'<p class="small">Choose the start time first.</p>'}</div></div>`:`<div class="notice">No available class time on this date.</div>`}</section>`}
 <input type="hidden" id="classStartTimeBtn" data-value="${esc(m.startTime||'')}"><input type="hidden" id="classEndTimeBtn" data-value="${esc(m.endTime||'')}">
 ...
 <div class="notice">The first date only shows currently available half-hour blocks. Every future course date is checked again when you save, so an existing booking, class or blocked period on any session date will still prevent a conflict.</div>
