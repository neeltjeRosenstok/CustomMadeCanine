...
function clientTrainingNotesView(){
 const rows=Array.isArray(state.trainingNotes)?state.trainingNotes:[];
 const month=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sept','Oct','Nov','Dec'];const parts=x=>{const raw=String(x||'').replace(' ','T')+(String(x||'').includes('Z')?'':'Z'),d=new Date(raw);if(Number.isNaN(d.getTime()))return null;const z=new Date(d.toLocaleString('en-US',{timeZone:'Africa/Nairobi'}));return {date:`${String(z.getDate()).padStart(2,'0')} ${month[z.getMonth()]}`,time:`${String(z.getHours()).padStart(2,'0')}:${String(z.getMinutes()).padStart(2,'0')}`}};const fmtStamp=x=>{const z=parts(x.created_at);return z?`${z.date} at ${z.time}`:esc(x.created_at||'')};
 return `<div class="client-messages"><h3>Messages & Training Notes</h3><p class="small">Newest messages are shown first.</p>${rows.length?rows.map(x=>{const service=x.booking_service?'Private training':'Training note',sd=x.booking_start_at?new Date(String(x.booking_start_at).slice(0,10)+'T12:00:00'):null,scheduled=sd&&!Number.isNaN(sd.getTime())?`${String(sd.getDate()).padStart(2,'0')} ${month[sd.getMonth()]}`:'';return `<article class="client-message-entry"><div class="client-message-meta"><b>Amy · ${fmtStamp(x)}${x.pet_name?` · ${esc(x.pet_name)}`:''}${service?` · ${esc(service)}`:''}${scheduled?` scheduled for ${esc(scheduled)}`:''}</b></div><div class="client-message-card">${esc(x.note)}</div></article>`}).join(''):'<div class="notice">Amy has not shared any messages or training notes yet.</div>'}</div>`;
}
...
function clientAdminView(){
 ...
 const filter=bookingMode?'current':(state.clientStatusFilter||'all'),activityFilter=bookingMode?'all':(state.clientActivityFilter||'all');
 ...
 ${bookingMode?'':`<div class="client-status-filters">${filters.map(f=>`<button class="${filter===f?'primary':'secondary'} compact-button" onclick="state.clientStatusFilter='${f}';render()">${operationalClientStatusLabel(f)} · ${f==='all'?rows.length:rows.filter(x=>x.client_status===f).length}</button>`).join('')}</div><div class="client-activity-filters"><span class="small"><b>Activity:</b></span>${['all','dormant','archived'].map(f=>`<button class="${activityFilter===f?'primary':'secondary'} compact-button" onclick="state.clientActivityFilter='${f}';render()">${f==='all'?'All activity':f[0].toUpperCase()+f.slice(1)} · ${f==='all'?rows.filter(x=>x.record_type==='client').length:rows.filter(x=>x.activity_status===f).length}</button>`).join('')}</div>`}
 ...
 ${bookingMode?'':clientRecordModal()}${trainerClientBookingModal()}${rejectionDecisionModal()}</section>`;
}
...
function clientRecordModal(){const c=state.clientRecord;if(!c)return '';
 ...
 const householdRows=[u.household_adults!=null?`<div><strong>Adults</strong><span>${esc(u.household_adults)}</span></div>`:'',u.household_dogs!=null?`<div><strong>Dogs in household</strong><span>${esc(u.household_dogs)}</span></div>`:'',u.children_0_8!=null?`<div><strong>Children 0–8</strong><span>${esc(u.children_0_8)}</span></div>`:'',u.children_9_13!=null?`<div><strong>Children 9–13</strong><span>${esc(u.children_9_13)}</span></div>`:'',u.children_14_plus!=null?`<div><strong>Children 14+</strong><span>${esc(u.children_14_plus)}</span></div>`:'',u.household_changes?`<div><strong>Recent / planned changes</strong><span>${esc(u.household_changes)}</span></div>`:'',u.household_note?`<div><strong>Additional note</strong><span>${esc(u.household_note)}</span></div>`:''].filter(Boolean).join('');
 return `<div class="modal-overlay"><div class="trainer-modal client-record-modal"><button class="close-btn modal-close" onclick="closeClientRecord()">×</button><div class="eyebrow">${isDraft?'Incomplete application':'Client record'}</div><div class="client-record-top"><div class="client-record-title"><h2>${esc(u.name||'Applicant')}</h2><p>${esc(u.email||'')}</p><p>WhatsApp: ${esc(u.whatsapp_phone||u.phone||'—')} · M-Pesa: ${esc(u.mpesa_phone||u.phone||'—')}</p><p><b>Area:</b> ${esc(u.location||'Not supplied')}</p>${u.google_maps_location?`<p><b>Google Maps:</b> <a href="${esc(u.google_maps_location)}" target="_blank" rel="noopener">Open location ↗</a></p>`:''}<p>Canine Grapevine: ${u.newsletter_opt_in?'Opted in':'Not subscribed'}</p></div><div class="client-payment-status-box"><span class="client-status-badge status-${esc(status)}">${esc(operationalClientStatusLabel(status))}</span><strong>Application payment</strong><p>${isDraft?'Not yet submitted':esc(applicationPaymentSummary(c))}</p></div></div><div class="application-review-grid"><div class="application-introduction-box"><b class="application-box-heading">Introduction / training needs</b><p>${esc(u.client_intro_note||c.draft?.introNote||'Not supplied')}</p></div><div class="application-household-box"><b class="application-box-heading">Household</b><div class="household-detail-lines">${householdRows||'<span>Not supplied</span>'}</div></div></div>${isRejected&&u.rejection_note?`<div class="notice bad"><b>Rejection reason</b><br>${esc(u.rejection_note)}</div>`:''}${!isDraft?`<div class="credit-balance-card trainer-credit"><span>Account credit</span><strong>${money(c.creditBalance||0)}</strong>${(c.creditHistory||[]).length?`<small>Latest: ${esc(c.creditHistory[0].note||"Credit adjustment")} · ${money(c.creditHistory[0].amount_delta||0)}</small>`:""}</div>`:''}<h3>Dogs</h3><div class="trainer-dog-records">${(c.pets||[]).map(p=>`<article class="trainer-dog-record"><h4>${esc(p.name)}</h4><p>${esc(p.breed||c.draft?.dogBreed||'Dog')}${p.gender?` · ${esc(p.gender)}`:''}${p.date_of_birth?` · DOB ${displayDate(p.date_of_birth,{day:'numeric',month:'short',year:'numeric'})}`:''}</p>${!isDraft?`<p>Vaccination: ${p.vaccination_count?`Uploaded · ${p.vaccination_status==='verified'?'Verified':'Pending verification'}`:esc(p.vaccination_status||'not provided')}</p><label>Amy's private notes<textarea id="trainerNotes-${p.id}" rows="3" ${Number(state.editingTrainerNotePetId)===Number(p.id)?'':'readonly'}>${esc(p.trainer_notes||'')}</textarea></label><div class="actions">${Number(state.editingTrainerNotePetId)===Number(p.id)?`<button class="primary compact-button" onclick="saveTrainerDogNotes(${p.id})">Save note</button><button class="secondary compact-button" onclick="cancelTrainerDogNoteEdit()">Cancel edit</button>`:`<button class="secondary compact-button" onclick="editTrainerDogNote(${p.id})">Edit note</button>`}<button class="secondary compact-button" onclick="openVaccinationReview(${p.id})">Vaccination</button></div>`:'<p class="small">Dog details saved in the unfinished application. Uploaded files are only attached once the application is submitted.</p>'}</article>`).join('')||'<p class="small">No dog details saved yet.</p>'}</div>${!isDraft?`<h3>Resources shared</h3>${(c.resources||[]).length?`<div class="client-shared-resources">${(c.resources||[]).map(r=>`<div class="client-shared-resource"><strong>${esc(r.title)}</strong><span>${esc(r.category||"General")} · ${esc(String(r.type||"").toUpperCase())}</span>${r.note?`<small>${esc(r.note)}</small>`:""}</div>`).join("")}</div>`:'<p class="small">No training resources shared yet.</p>'}`:''}<div class="actions client-decision-actions">${isDraft?`<button class="danger" onclick="deleteIncompleteApplication(${u.id})">Delete application</button>`:''}${isUnverified&&c.applicationDeposit?.manual_payment_status==='submitted'?`<button class="primary" onclick="approveNewClient(${c.applicationDeposit.id})">Approve payment + KES 700 credit</button>`:''}${isUnverified?`<button class="danger" onclick="openRejectNewClient(${u.id},${c.applicationDeposit?.manual_payment_status==='submitted'?'true':'false'})">${c.applicationDeposit?.manual_payment_status==='submitted'?'Refund & Reject':'Reject application'}</button>`:''}${status==='current'?`<button class="primary" onclick="startTrainerClientBooking(${u.id})">＋ Book for this client</button>`:''}${isRejected?`<button class="danger" onclick="deleteRejectedClient(${u.id})">Delete rejected client</button>`:''}<button class="secondary" onclick="closeClientRecord()">Done</button></div></div></div>`}
...
function attentionAdminView(){
 ...
 const map={payments:'payments',new:'new',refunds:'refunds',vaccinations:'vaccinations',reviews:'reviews',all:'all'},filtered=items.filter(x=>(filter==='all'||x.category===map[filter])&&(!q||x.search.includes(q)));
 filtered.sort((a,b)=>a.priority-b.priority||((a.expiry&&b.expiry)?a.expiry.localeCompare(b.expiry):a.expiry?-1:b.expiry?1:0));
 const filters=[['all','All'],['payments','Unverified payments'],['new','New bookings'],['refunds','Refunds Pending'],['vaccinations','Vaccination Records'],['reviews','Reviews']];
