...
function clientTrainingNotesView(){
 const rows=state.trainingNotes||[],month=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
 return `<div class="client-messages"><h3>Messages & Training Notes</h3><p class="small">Newest messages are shown first.</p>${rows.length?rows.map(x=>{const service=x.booking_service?'Private training':x.item_type==='resource'?'Training resource':x.item_type==='reply'?'Reply':'Training note',sd=x.booking_start_at?new Date(String(x.booking_start_at).slice(0,10)+'T12:00:00'):null,scheduled=sd&&!Number.isNaN(sd.getTime())?`${String(sd.getDate()).padStart(2,'0')} ${month[sd.getMonth()]}`:'';const head=x.item_type==='resource'?`Amy · ${esc(formatNairobiActivityTime(x.created_at||x.shared_at||x.booking_start_at))} · Shared ${String(x.resource_type||'resource').toUpperCase()} — ${esc(x.resource_title||'Training resource')}`:`${esc(x.sender||'Amy')} · ${esc(formatNairobiActivityTime(x.created_at||x.shared_at||x.booking_start_at))}${x.pet_name?` · ${esc(x.pet_name)}`:''} · ${esc(service)}${scheduled?` scheduled for ${esc(scheduled)}`:''}`;return `<article class="client-message-entry"><div class="client-message-meta"><b>${head}</b></div><div class="client-message-card">${esc(x.note||'')}${x.item_type==='resource'&&x.resource_title?`<div class="actions"><button class="secondary compact-button" onclick="state.portalTab='media';render()">Open shared resources</button></div>`:''}</div>${x.item_type!=='reply'?`<button class="secondary compact-button message-reply-btn" onclick="replyToClientMessage('${esc(x.item_type||'note')}',${Number(x.id)||0})">Reply</button>`:''}</article>`}).join(''):'<div class="notice">Amy has not shared any messages or training notes yet.</div>'}</div>`;
}
...
function accountWhatsappInput(input){const raw=String(input?.value||'');const cleaned=raw.replace(/[^0-9+ ]/g,'').replace(/(?!^)\+/g,'');if(input&&input.value!==cleaned)input.value=cleaned;const warning=document.getElementById('accountWhatsappWarning'),digits=cleaned.replace(/\D/g,'');const ok=!!cleaned.trim()&&/^\+?[0-9 ]+$/.test(cleaned)&&digits.length>=6&&digits.length<=18;const kenyan=ok?formatKenyanMpesa(cleaned):'';if(kenyan&&input)input.value=kenyan;if(warning){warning.textContent=ok?'':'Use numbers, spaces and a leading + only.';warning.hidden=ok}return ok}
