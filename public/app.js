...
async function clientCancelClass(id){
 if(!await appConfirm("Cancel this class place? Amy will be notified and will decide any refund."))return;
 const note=await appPrompt("Anything you would like Amy to know about the cancellation? (optional)","");if(note===null)return;
 try{const d=await api(`/api/my/class-enrolments/${id}/cancel`,{method:"POST",body:JSON.stringify({note:note.trim()})});state.bookings=await api("/api/my/bookings");render();appAlert(d.refundPending?"Cancellation sent to Amy. A refund decision is now pending.":"Your class place has been cancelled and Amy has been notified.")}catch(e){appAlert(e.message)}
}
...
async function loadRescheduleSlots(){
 ...
 const date=document.getElementById("rescheduleDate")?.value||r.date;if(!date)return appAlert("Choose a date.");r.date=date;
 try{r.slots=await api(`/api/availability?date=${encodeURIComponent(date)}&locationType=${encodeURIComponent(r.booking.location_type)}&address=${encodeURIComponent(r.booking.address||"")}&service=${encodeURIComponent(r.booking.service)}`);r.selected=null;render()}catch(e){appAlert(e.message)}
}
...
async function clientCancel(id){if(!await appConfirm('Cancel this confirmed booking? Amy will be notified and will decide the refund or alternative arrangement.'))return;try{await api(`/api/my/bookings/${id}/cancel`,{method:'POST',body:JSON.stringify({reason:'Cancelled by client'})});await portal();appAlert('Cancellation sent to Amy. Your refund decision is pending.')}catch(e){appAlert(e.message)}}
async function addPrivateCalendarByRef(ref){
 const x=state.bookings?.privateBookings?.find(b=>b.booking_ref===ref);
 if(!x)return appAlert("Booking details could not be found.");
...
async function addClassCalendarByRef(ref){
 ...
 if(!x)return appAlert("Class booking details could not be found.");
 const c=state.classes.find(c=>c.id===x.class_id);
 if(!c)return appAlert("The class schedule could not be found. Please refresh the Client Portal.");
...
async function submitResourceUpload(ev){ev.preventDefault();const fd=new FormData();const title=document.getElementById('resourceTitle').value.trim(),category=document.getElementById('resourceCategory').value.trim()||'General',description=document.getElementById('resourceDescription').value.trim(),type=document.getElementById('resourceType').value,file=document.getElementById('resourceFile').files[0],url=document.getElementById('resourceUrl').value.trim();if(!file&&!url)return appAlert('Please choose a file or enter an external link.');fd.append('title',title);fd.append('category',category);fd.append('description',description);if(type!=='auto')fd.append('type',type);if(url)fd.append('url',url);if(file)fd.append('file',file);try{await api('/api/trainer/resources',{method:'POST',body:fd});state.resourceUploadOpen=false;state.resourceLibrary=await api('/api/trainer/resources');render();appAlert('Resource saved to the library.')}catch(e){appAlert(e.message)}}
...
async function manageResourceAccess(id){
 ...
 if(!rows.length)return appAlert('This resource is not assigned yet.');
 const text=rows.map((r,i)=>(`${i+1}. ${r.user_name||''}${r.pet_name?' · Dog: '+r.pet_name:''}${r.class_title?' · Class: '+r.class_title:''}`)).join('\n');
 const n=Number(await appPrompt(text+'\n\nEnter number to unassign:'));
 if(!n||!rows[n-1])return;
 await api('/api/trainer/resources/access/'+rows[n-1].id,{method:'DELETE'});
 appAlert('Resource unassigned.');
}
async function archiveResource(id){if(!await appConfirm('Remove this resource from the active library?'))return;await api(`/api/trainer/resources/${id}`,{method:'DELETE'});state.resourceLibrary=await api('/api/trainer/resources');render()}
...
async function submitClassModal(){
 ...
 if(!title||!startDate)return appAlert('Please complete the course name and first date.');
 try{
...
async function openTrainerBooking(id){
 ...
 const raw=fromCalendar||fromAttention;if(!raw)return appAlert("This appointment could not be opened.");
...
async function decideRefund(id,decision){
  if(decision==="none"){
    if(!await appConfirm("Record that no refund will be made for this cancelled booking?"))return;
    try{await api(`/api/trainer/bookings/${id}/refund`,{method:"POST",body:JSON.stringify({decision:"none"})});state.trainer=await api("/api/trainer/summary");render();appAlert("No-refund decision recorded.");}catch(e){appAlert(e.message)}
    return;
  }
  ...
  const amountText=await appPrompt(`${decision==="full"?"Full":"Partial"} refund amount in KES:`,String(decision==="full"?fullAmount:""));
  if(amountText===null)return;
  const amount=Number(amountText);
  const confirmationCode=await appPrompt("Enter the M-Pesa refund/transaction confirmation code:");
  ...
  try{
    ...
    render();
    appAlert(`${decision==="full"?"Full":"Partial"} refund recorded: KES ${amount.toLocaleString()} · M-Pesa ${confirmationCode}`);
  }catch(e){appAlert(e.message)}
}
...
async function submitManualPaymentFromPayment(){
 const c=state.confirm;if(!c?.id)return appAlert('Open this pending booking from your Client Portal to submit manual payment.');
 const code=(document.getElementById('manualMpesaCode')?.value||'').trim().toUpperCase();
 const amount=Number(document.getElementById('manualMpesaAmount')?.value);
 if(code.length<6)return appAlert('Please enter the M-Pesa confirmation code.');
 if(!Number.isFinite(amount)||amount<=0)return appAlert('Please enter the amount sent in KES.');
 try{
   await api(`/api/my/bookings/${c.id}/manual-payment`,{method:'POST',body:JSON.stringify({confirmationCode:code,amount})});
   await appAlert('Thank you. Your M-Pesa confirmation has been submitted to Amy for verification. Your booking is being held while she checks the payment. Please WhatsApp Amy to make sure she sees this.');
   await portal();
 }catch(e){appAlert(e.message)}
}
...
function pendingBookingActions(x){
 ...
 if(x.manual_payment_status==='submitted')return `<div class="notice">Awaiting Amy payment verification. Your booking remains held while Amy checks the payment.</div>`;
...
async function manualPaymentForBooking(id,amount){state.manualPaymentBooking={id,amount};const code=await appPrompt('Enter the M-Pesa Send Money confirmation code:','');if(code===null)return;const sent=await appPrompt('Amount sent in KES:',String(amount||''));if(sent===null)return;api(`/api/my/bookings/${id}/manual-payment`,{method:'POST',body:JSON.stringify({confirmationCode:code.trim(),amount:Number(sent)})}).then(()=>portal()).then(()=>appAlert('Amy has been asked to verify your payment.')).catch(e=>appAlert(e.message))}
function clientReschedule(id){const b=(state.bookings?.privateBookings||[]).find(x=>x.id===id);if(!b)return;if(Number(b.reschedule_count||0)>=3)return appAlert('The three client-requested reschedules have been used. Please contact Amy.');state.rescheduleDraft={bookingId:id,booking:b,date:'',slots:[],selected:null};render()}
...
function exportReportCsv(){const rows=state.reports?.rows||[];if(!rows.length)return appAlert('Generate a report first.');const cols=Object.keys(rows[0]),q=v=>`"${String(v??'').replaceAll('"','""')}"`,csv=[cols.map(q).join(','),...rows.map(r=>cols.map(k=>q(r[k])).join(','))].join('\r\n');const blob=new Blob([csv],{type:'text/csv;charset=utf-8'}),x=document.createElement('a');x.href=URL.createObjectURL(blob);x.download=`cmc-${state.reportType}-${state.reportFrom||'report'}.csv`;x.click();setTimeout(()=>URL.revokeObjectURL(x.href),1000)}
