...
function addDogModal(){const fromBooking=!!state.addPetBookingContext;return `<div class="pet-add-overlay" role="dialog" aria-modal="true" aria-labelledby="addDogTitle"><div class="pet-add-card"><div class="pet-add-head"><div><div class="eyebrow">${fromBooking?"Booking":"Client profile"}</div><h3 id="addDogTitle">${fromBooking?"Add a dog to this booking":"Add a dog"}</h3></div><button class="close-btn close-light" aria-label="Close add dog form" onclick="closeAddDogModal()">×</button></div>${fromBooking?`<p class="small booking-dog-helper">Your booking choices stay exactly as they are. After saving, this dog will be selected automatically.</p>`:""}
 ...
 <p class="small">Photo and vaccination pages are optional now; you can add or replace them later.</p><div class="actions"><button class="secondary" onclick="closeAddDogModal()">Cancel</button><button class="primary" onclick="addPet()">Save dog</button></div></div></div>`}
...
async function addPet(){
 ...
 if(!name)return alert("Please add your dog's name.");
 fd.append("name",name);fd.append("breed",breed);fd.append("dateOfBirth",dob);fd.append("notes",notes);fd.append("gender",gender);fd.append("neuteredSpayed",neuteredSpayed?"1":"");fd.append("behaviorNotes",behaviorNotes);fd.append("medicalProcedures",medicalProcedures);
 const photo=document.getElementById("petPhoto"),vaccinations=document.getElementById("petVaccinations");
 if(photo?.files?.[0])fd.append("dogPhoto",photo.files[0]);
 for(const f of (vaccinations?.files||[]))fd.append("vaccinationPages",f);
 const bookingContext=state.addPetBookingContext;
 try{
  const p=await api("/api/my/pets",{method:"POST",body:fd});
  if(!state.profile)state.profile={pets:[]};if(!Array.isArray(state.profile.pets))state.profile.pets=[];
  state.profile.pets.push(p);
  if(bookingContext)state.selectedPet=Number(p.id);
  state.showAddPet=false;state.addPetBookingContext=null;render();
 }catch(e){alert(e.message)}
}
...
async function submitClassModal(){
 ...
 try{
   ...
   state.classes=refreshed[0];state.trainer=refreshed[1];state.classAdmin=refreshed[2];state.selectedClassAdmin=Number(created.id);
...
function classModalView(){
 ...
 <div class="eyebrow">Course planner</div><h2>Create a class course</h2>
 <p class="small">Choose the first date; weekly or fortnightly courses follow that weekday automatically. Saturdays are not assumed.</p>
