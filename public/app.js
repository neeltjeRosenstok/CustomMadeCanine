...
function applicationGo(step){state.applicationStep=Math.max(1,Math.min(5,Number(step)||1));render()}
...
async function submitNewClientApplication(){
 ...
 try{
  ...
  state.applicationCompletedName=d.name;state.applicationDraft=null;state.applicationDogPhoto=null;state.applicationVaccinationFiles=[];state.applicationVaccinationAccepted=false;state.applicationStep=5;state.view="application";render();
 }catch(e){appAlert(e.message);if(btn){btn.disabled=false;btn.textContent="Submit application"}}
}
